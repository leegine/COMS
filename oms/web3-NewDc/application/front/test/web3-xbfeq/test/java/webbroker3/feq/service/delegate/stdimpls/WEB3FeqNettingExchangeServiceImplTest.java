...
1.1
date	2011.04.07.02.17.13;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
public class WEB3FeqNettingExchangeServiceImplTest extends TestBaseForMock{
    ...
    public void preInstitutionAndBranch() throws Exception
    {
        ...
        TestDBUtility.deleteAll(BranchRow.TYPE);
        BranchParams l_branchParam = TestDBUtility.getBranchRow();
        l_branchParam.setInstitutionId(33);
        l_branchParam.setBranchCode("381");
        l_branchParam.setBranchId(33381);
        TestDBUtility.insertWithDel(l_branchParam);
        BranchParams l_branchParam1 = TestDBUtility.getBranchRow();
        l_branchParam1.setInstitutionId(33);
        l_branchParam1.setBranchCode("382");
        l_branchParam1.setBranchId(33382);
        TestDBUtility.insertWithDel(l_branchParam1);
        BranchParams l_branchParam2 = TestDBUtility.getBranchRow();
        l_branchParam2.setInstitutionId(33);
        l_branchParam2.setBranchCode("383");
        l_branchParam2.setBranchId(33383);
        TestDBUtility.insertWithDel(l_branchParam2);
    ...
    public void testExcuteCase001()
    {
        try
        {
            ...
            this.preGetDate();
            this.preGetList();
            this.preInstitutionAndBranch();
            WEB3FeqOrderUnit[] l_feqOrderUnits = new WEB3FeqOrderUnit[5];
            Constructor c = WEB3FeqOrderUnit.class.getDeclaredConstructor(new Class[]{long.class});
            c.setAccessible(true);
            TestDBUtility.deleteAllAndCommit(ProcessManagementRow.TYPE);
            TestDBUtility.deleteAllAndCommit(LoginUsernameRow.TYPE);
 ...
 class WEB3FeqNettingExchangeServiceImplMock extends WEB3FeqNettingExchangeServiceImpl
 {
         ...
         else
         {
             if (l_web3BackRequest instanceof WEB3FeqNettingExchangeRequest)
             {
                 ...
                 for (int i = 0; i < l_branches.length; i++)
                 {
                     ...
                     if (i==2)
                     {
                         l_processManagementParams.setBranchCode(l_branches[i-1].getBranchCode());
                     }
                     else
                     {
                     ...
                     try
                     {
                         WEB3FEQNettingInsertTransactionCallback l_insertCallback = 
                             new WEB3FEQNettingInsertTransactionCallback();
                        l_insertCallback.setProcessManagementParams(l_processManagementParams);
                         l_queryProcessor.doTransaction(TransactionalInterceptor.TX_JOIN_EXISTING, l_insertCallback);
     ...
     public class WEB3FEQNettingInsertTransactionCallback implements TransactionCallback
     {
         public WEB3FEQNettingInsertTransactionCallback()
         {

         }
         private ProcessManagementParams processManagementParams;
         public void setProcessManagementParams(ProcessManagementParams l_processManagementParams)
         {
             this.processManagementParams = l_processManagementParams;
         }
         public Object process()
         {
             ...
             {
                 QueryProcessor l_queryProcessor = Processors.getDefaultProcessor();
                 l_queryProcessor.doInsertQuery(processManagementParams);
             }
             catch(Exception l_ex)
             {
 ...
 {
     public void updateTransactionNettingAdoption(long[] ids, Date orderDate)  throws WEB3BaseException
     {
         try
         {
             System.out.println("*******************************ID is:"+ids[0]);
             LoginUsernameParams lp = TestDBUtility.getLoginUsernameRow();
             lp.setLoginId(ids[0]);
             lp.setUsername(ids[0]+"");
             
             QueryProcessor l_processor = Processors.getDefaultProcessor();
             l_processor.doInsertQuery(lp);
         }
         catch(Exception e)
         {
             throw new WEB3SystemLayerException(
                     WEB3ErrorCatalog.SYSTEM_ERROR_80005,
                     this.getClass().getName() + "." + "WEB3FeqPositionManagerHelperMock",
                     e.getMessage(),
                     e);
         }
