...
@/**
 ...
 File Name        : WEB3FuturesOrderExecNotifyNormalTransactionCallbackテスト
 Author Name      : Daiwa Institute of Research Business Innovation
 Revesion History : 2007/04/27 金傑 (中訊) 新規作成 仕様変更モデルNo.634
  Revesion History : 2008/04/29 安陽 (中訊) 仕様変更モデルNo.881
...
/**
 * （株価指数先物出来通知正常処理一件TransactionCallbackテスト）<BR>
 * 
 * @@author 金傑
 * @@version 1.0
 */
...
//            assertEquals("該当データなし",l_ex.getMessage());
        }
        log.exiting(TEST_END + STR_METHOD_NAME);
    }
...
//            assertEquals("該当データなし",l_ex.getMessage());
    ...
    public void testProcess_C0003()
    {
        ...
        {
//            assertEquals(DataCallbackException.class,l_ex.getClass());
//            assertEquals("注文単位が取得出来ない。",l_ex.getMessage());
        }
        log.exiting(TEST_END + STR_METHOD_NAME);
    }
...
//            assertEquals("注文単位が取得出来ない。",l_ex.getMessage());
    ...
    public void testProcess_C0008()
    {
        ...
        {
            log.debug(STR_METHOD_NAME, l_ex);
            assertEquals("お客様の先物口座が開設されておりません。", l_ex.getMessage());
        }
        catch (Exception l_ex)
        {
            log.error(STR_METHOD_NAME, l_ex);
