...
@/**
 ...
 Revision History : 2008/07/31 楊夫志(中訊) 新規作成 モデル 132
*/
...
public class WEB3MarginChangeCancelNotifyChangeUnitServiceImplTest extends TestBaseForMock
{
    ...
    // isストップ注文有効(EqTypeOrderUnit)=true
    //1.11. getInstitution()
    //1.13. getMainAccount()
    //1.14. getSubAccount()
    //OrderCategEnum.OPEN_MARGIN.equals(l_orderUnit.getOrderCateg()) 注文失効時概算代金 = null calc注文時建代金
    ...
    //isストップ注文有効(EqTypeOrderUnit)=false
    //1.11. getInstitution()
    //1.13. getMainAccount()
    //1.14. getSubAccount()
    //OrderCategEnum.OPEN_MARGIN.equals(l_orderUnit.getOrderCateg()) 注文失効時の場合 getストップ注文失効時概算代金計算結果
    //1.22. 通知キュー.訂正取消通知区分＝"訂正完了"の場合 ,l_result.isSuccessResult()
    ...
    //1.13. getMainAccount()
    //1.14. getSubAccount()
    //OrderCategEnum.CLOSE_MARGIN.equals(l_orderUnit.getOrderCateg())  adjust返済指定情報() setIs指値(true)
    //1.22. 通知キュー.訂正取消通知区分＝"訂正完了"の場合 ,l_result.isSuccessResult()
    //1.25.4 訂正後に全部約定となった場合
    ...
    //OrderCategEnum.OPEN_MARGIN.equals(l_orderUnit.getOrderCateg()) 注文失効時概算代金 = null calc注文時建代金
    //1.22. 通知キュー.訂正取消通知区分＝"訂正完了"の場合 ,l_result.isSuccessResult()
    //1.25.4 訂正後に全部約定となった場合 isFullyExecuted() = true
    ...
    //1.13. getMainAccount()
    //1.14. getSubAccount()
    //OrderCategEnum.CLOSE_MARGIN.equals(l_orderUnit.getOrderCateg())  adjust返済指定情報() setIs指値(false)
    //1.22. 通知キュー.訂正取消通知区分＝"訂正完了"の場合 ,l_result.isSuccessResult()
    //1.25.4 訂正後に全部約定となった場合
