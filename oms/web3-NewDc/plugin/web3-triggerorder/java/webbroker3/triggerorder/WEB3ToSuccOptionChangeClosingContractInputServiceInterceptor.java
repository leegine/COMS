...
1.1
date	2011.03.15.01.56.07;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccOptionChangeClosingContractInputServiceInterceptor.java;
...
File Name        :（連続）OP訂正返済入力サービスインタセプタ（WEB3ToSuccOptionChangeClosingContractInputServiceInterceptor.java）
...
/**
 * (（連続）OP訂正返済入力サービスインタセプタ)<BR>
 * （連続）株価指数オプション訂正返済入力サービスインタセプタ<BR>
 *
 * @@author 孟亞南(中訊)
 * @@version 1.0
 */
public class WEB3ToSuccOptionChangeClosingContractInputServiceInterceptor implements Interceptor
{
    /**
     * ログユーティリティ<BR>
     */
     private static WEB3LogUtility log =
         WEB3LogUtility.getInstance(WEB3ToSuccOptionChangeClosingContractInputServiceInterceptor.class);

    /**
     * @@roseuid 47FDBE3D0167
     */
    public WEB3ToSuccOptionChangeClosingContractInputServiceInterceptor()
    {
     ...
     * <BR>
     * (*1) 連続注文マネージャ.get先物OP予約注文単位(リクエストデータ.ＩＤ)<BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParams)
    {
        ...
        if (!(l_serviceParams[0] instanceof WEB3SuccOptionsCloseChangeInputRequest))
        {
        ...
        WEB3SuccOptionsCloseChangeInputRequest l_request =
            (WEB3SuccOptionsCloseChangeInputRequest)l_serviceParams[0];
