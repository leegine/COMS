...
1.1
date	2011.03.15.01.56.57;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccFuturesChangeOpenContractServiceInterceptor.java;
...
File Name        : （連続）先物訂正新規建サービスインタセプタ（WEB3ToSuccFuturesChangeOpenContractServiceInterceptor.java）
...
/**
 * (（連続）先物訂正新規建サービスインタセプタ)<BR>
 * （連続）先物訂正新規建サービスインタセプタ<BR>
 *
 * @@author トウ鋒鋼
 * @@version 1.0
 */
public class WEB3ToSuccFuturesChangeOpenContractServiceInterceptor implements Interceptor
{
    /**
     * ログ出力ユーティリティ。
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3ToSuccFuturesChangeOpenContractServiceInterceptor.class);

    /**
     * @@roseuid 47D7B0EF017C
     */
    public WEB3ToSuccFuturesChangeOpenContractServiceInterceptor()
    {
     ...
     * <BR>
     * １）　@リクエストデータの型が「（連続）株価指数先物訂正新規建完了リクエスト」の場合のみ、<BR>
     * 　@口座をロックする。<BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParams)
    {
        ...
        try
        {
            ...
            if (l_request instanceof WEB3SuccFuturesOpenChangeCompleteRequest)
            {
            ...
            {
                WEB3SuccFuturesOpenChangeConfirmRequest l_confirmRequest =
            ...
            {
                WEB3SuccFuturesOpenChangeCompleteRequest l_completeRequest =
