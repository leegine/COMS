...
1.1
date	2011.03.15.01.56.15;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccOptionChangeClosingContractServiceInterceptor.java;
...
File Name        :（連続）OP訂正返済サービスインタセプタ（WEB3ToSuccOptionChangeClosingContractServiceInterceptor.java）
...
/**
 * (（連続）OP訂正返済サービスインタセプタ)<BR>
 * （連続）OP訂正返済サービスインタセプタ<BR>
 *
 * @@author 孟亞南(中訊)
 * @@version 1.0
 */
public class WEB3ToSuccOptionChangeClosingContractServiceInterceptor implements Interceptor
{
    /**
     * ログユーティリティ<BR>
     */
     private static WEB3LogUtility log =
         WEB3LogUtility.getInstance(WEB3ToSuccOptionChangeClosingContractServiceInterceptor.class);

    /**
     * @@roseuid 47FDC51501C5
     */
    public WEB3ToSuccOptionChangeClosingContractServiceInterceptor()
    {
     ...
     * <BR>
     * １）　@リクエストデータの型が「（連続）株価指数オプション訂正返済完了リクエスト」の場合のみ、<BR>
     * 　@　@　@口座をロックする。 <BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParams)
    {
        ...
        try
        {
            ...
            if (l_request instanceof WEB3SuccOptionsCloseChangeCompleteRequest)
            {
            ...
            {
                WEB3SuccOptionsCloseChangeConfirmRequest l_confirmRequest =
            ...
            {
                WEB3SuccOptionsCloseChangeCompleteRequest l_completeRequest =
