...
1.1
date	2011.03.15.01.51.48;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccFuturesCancelOrderServiceImpl.java;
...
File Name        : （連続）先物取消注文サービスImpl(WEB3ToSuccFuturesCancelOrderServiceImpl.java)
Author Name      : Daiwa Institute of Research
Revision History : 2008/03/12 于瀟(中訊) 新規作成モデル268
Revision History : 2008/05/20 張騰宇 (中訊) 仕様変更 モデル353
*/
...
/**
 * (（連続）先物取消注文サービスImpl)<BR>
 * （連続）先物取消注文サービス実装クラス<BR>
 * <BR>
 * @@author 于瀟
 * @@version 1.0
 */
public class WEB3ToSuccFuturesCancelOrderServiceImpl
    extends WEB3FuturesClientRequestService
    implements WEB3ToSuccFuturesCancelOrderService
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3ToSuccFuturesCancelOrderServiceImpl.class);

    /**
     * @@roseuid 47D659330204
     */
    public WEB3ToSuccFuturesCancelOrderServiceImpl()
    {
     ...
     * <BR>
     * 　@　@[（連続）株価指数先物取消注文確認リクエストの場合] <BR>
     * 　@　@　@this.validate注文()をコールする。 <BR>
     * 　@　@[（連続）株価指数先物取消注文完了リクエストの場合] <BR>
     * 　@　@　@this.submit注文()をコールする。<BR>
     * @@param l_request - (リクエスト)<BR>
     * @@return WEB3GenResponse
     * @@throws WEB3BaseException
     * @@roseuid 47A9545C0312
     */
    public WEB3GenResponse execute(WEB3GenRequest l_request) throws WEB3BaseException
    {
        ...
        //[（連続）株価指数先物取消注文確認リクエストの場合]
        //this.validate注文()をコールする。
        if (l_request instanceof WEB3SuccFuturesCancelConfirmRequest)
        {
            l_response = this.validateOrder((WEB3SuccFuturesCancelConfirmRequest)l_request);
        }
        //[（連続）株価指数先物取消注文完了リクエストの場合] <BR>
        //this.submit注文()をコールする。
        else if (l_request instanceof WEB3SuccFuturesCancelCompleteRequest)
        {
            l_response = this.submitOrder((WEB3SuccFuturesCancelCompleteRequest)l_request);
        }
        else
        {
     ...
     * （連続）先物取消の発注審査を行う。<BR>
     * <BR>
     * 「（（連続）先物取消サービス）validate注文」参照。<BR>
     * @@param l_request - (リクエストデータ)<BR>
     * @@return WEB3SuccFuturesCancelConfirmResponse
     * @@throws WEB3BaseException
     * @@roseuid 47A9546F0373
     */
    protected WEB3SuccFuturesCancelConfirmResponse validateOrder(WEB3SuccFuturesCancelConfirmRequest l_request)
        throws WEB3BaseException
    {
        final String STR_METHOD_NAME = "validateOrder(WEB3SuccFuturesCancelConfirmRequest)";
        log.entering(STR_METHOD_NAME);
        ...
        double l_orderQuantity = l_ifoOrderUnit.getQuantity();
        ...
        //レスポンス.概算建代金（決済損益）= 先物OP予約注文単位.概算受渡代金
     ...
     * （連続）先物取消の注文を登録する。<BR>
     * <BR>
     * 「（（連続）先物取消サービス）submit注文」参照。<BR>
     * @@param l_request - (リクエストデータ)<BR>
     * @@return WEB3SuccFuturesCancelCompleteResponse
     * @@throws WEB3BaseException
     * @@roseuid 47A954AA00AD
     */
    protected WEB3SuccFuturesCancelCompleteResponse submitOrder(WEB3SuccFuturesCancelCompleteRequest l_request)
        throws WEB3BaseException
    {
        final String STR_METHOD_NAME = "submitOrder(WEB3SuccFuturesCancelCompleteRequest)";
        log.entering(STR_METHOD_NAME);
