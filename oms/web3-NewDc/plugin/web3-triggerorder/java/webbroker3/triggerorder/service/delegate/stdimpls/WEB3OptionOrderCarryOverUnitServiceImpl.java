...
File Name        : OP注文繰越UnitServiceImpl(WEB3OptionOrderCarryOverUnitServiceImpl.java)
Author Name      : Daiwa Institute of Research
Revesion History : 2004/06/17 張威 (中訊) 新規作成
              001: 2004/07/27 王暁傑 (中訊) 対応バッグ WEB3_IFO_UT-000027 タイトルとlogの定義を修正
              002: 2004/07/28 王暁傑 対応バッグ WEB3_IFO_UT-000048
              003: 2004/08/10 王暁傑 Review 修正
              004: 2004/08/13 呉艶飛　@(中訊) STBUG(IFO_ST-000079)を対応
              005: 2006/07/27 唐性峰 (中訊) モデルNo.509対応
Revesion History : 2007/06/22 孟亜南 (中訊) モデルNo.744対応
Revesion History : 2007/06/28 孟亜南 (中訊) モデルNo.763 No.764対応
Revesion History : 2007/07/17 趙林鵬 (中訊) モデルNo.776
...
/**
 * (OP注文繰越UnitServiceImpl)<BR>
 * OP注文繰越１件サービス実装クラス<BR>
 * <BR>
 * １件ごとの注文繰越処理を実施する。<BR>
 * <BR>
...
public class WEB3OptionOrderCarryOverUnitServiceImpl implements WEB3OptionOrderCarryOverUnitService
{
        ...
        throws WEB3BaseException
    {
        ...
        try
        {
            ...
            // (*3)プロパティセット
            // (*3)手数料オブジェクトに以下の通りプロパティをセットする。
            // 手数料.注文チャネル = 注文単位.初回注文の注文チャネル
            // 手数料.証券会社コード = 補助口座.get取引店()に該当する証券会社コード
            // 手数料.部店ID = 注文単位.部店ＩＤ
            ...
            // calc概算受渡代金(手数料, double, SubAccount, 先物OP取引銘柄, double, SideEnum, boolean, boolean)(OP注文マネージャ::calc概算受渡代金)
            //  [calc概算受渡代金()に指定する引数]
            //  手数料：　@手数料オブジェクト
                ...
                && WEB3OrderingConditionDef.W_LIMIT_PRICE.equals(l_openContractOrderSpec.getOrderCond()))
            {
                ...
                // (*5)プロパティセット
                //  (*5)W指値用手数料オブジェクトに以下の通りプロパティをセットする。
                //  W指値用手数料.注文チャネル = 注文単位.初回注文の注文チャネル
                //  W指値用手数料.証券会社ID = 注文単位.証券会社ＩＤ
                //  W指値用手数料.部店ID = 注文単位.部店ＩＤ
                ...
                //  W指値用手数料.原注文注文チャネル = 手数料.注文チャネル
                //  W指値用手数料.原注文手数料No = 手数料.手数料No
                //  W指値用手数料.原注文手数料No枝番 = 手数料.手数料No枝番
                //  W指値用手数料.原資産銘柄コード = 先物OP銘柄.get原資産銘柄コード()
                //  W指値手数料.数量 = 新規建注文内容.getQuantity()
            ...
            // 比較結果が高いほうの戻り値のものを使用する。
            if (l_openContractOrderSpec.isBuyToOpenOrder()
                && WEB3OrderingConditionDef.W_LIMIT_PRICE.equals(l_openContractOrderSpec.getOrderCond()))
            {
                // set手数料(手数料)(先物OP注文繰越更新インタセプタ::set手数料)
                //  ※買建、かつ、W指値注文の場合は、戻り値(*1)と戻り値(*2)の拘束売買代金を比較して、
                //  比較結果が高いほうの戻り値の手数料オブジェクトを使用する
                ...
                //  (先物OP注文繰越更新インタセプタ::set概算受渡代金計算結果)
                //  ※買建、かつ、W指値注文の場合は、戻り値(*1)と戻り値(*2)の拘束売買代金を比較して、
            ...
            //(*6) 証拠金・余力チェックでエラーになった場合
            if (!l_tradingPowerResult.isResultFlg())
            {
                String l_strErrorReasonCode = null;
                //  補助口座.補助口座タイプ != 7(株式オプション取引口座（先物証拠金）) かつ
                ...
                // update繰越元注文(OrderUnit, String)(OP注文繰越UnitServiceImpl::update繰越元注文)
                //  [update繰越元注文()に指定する引数]
                //  注文単位：　@注文単位
                //  注文エラー理由コード：
                //　@（預り金不足エラー）または（株価指数オプション残高不足エラー）
                //  ※ 注文エラー理由コードについては
                //  DB更新仕様
                //  OP注文繰越_注文単位テーブル仕様.xls
            ...
            //  (OP注文マネージャ::setThreadLocalPersistenceEventInterceptor)
            l_orderManager.setThreadLocalPersistenceEventInterceptor(l_updateInterceptor);

            // createNewOrderId( )(OP注文マネージャ::createNewOrderId)
            ...
            //  (OP注文マネージャ::submitOpenContractOrder)
            //  [submitOpenContractOrder()に指定する引数]
            //  補助口座：　@補助口座
            //  新規建注文内容：　@新規建注文内容
            //  注文ＩＤ：　@OP注文マネージャ.createNewOrderId()
            //  取引パスワード： 口座.取引パスワード
            //  isSkip発注審査：　@true
        ...
        throws WEB3BaseException
    {
        ...
        try
        {
            ...
            // validate返済注文(SubAccount, IfoSettleContractOrderSpec)(OP注文マネージャ::validate返済注文)
            //  [validate返済注文()に指定する引数]
            //  補助口座：　@get補助口座()の戻り値
        ...
        throws WEB3BaseException
    {
        ...
        //1.5 expireOrder(long)(OP注文マネージャ::expireOrder)
        //  [expireOrder()に指定する引数]
