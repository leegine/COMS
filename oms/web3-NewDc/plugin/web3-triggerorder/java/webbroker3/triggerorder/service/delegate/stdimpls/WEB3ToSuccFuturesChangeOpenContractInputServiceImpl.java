...
1.1
date	2011.03.15.01.52.04;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccFuturesChangeOpenContractInputServiceImpl.java;
...
File Name        : （連続）先物訂正新規建入力サービスImpl（WEB3ToSuccFuturesChangeOpenContractInputServiceImpl.java）
Author Name      : Daiwa Institute of Research
Revision History : 2008/03/18 トウ鋒鋼 (中訊) 新規作成 モデルNo.261、286
*/
...
/**
 * (（連続）先物訂正新規建入力サービスImpl)<BR>
 * （連続）先物訂正新規建入力サービス実装クラス<BR>
 *
 * @@author トウ鋒鋼
 * @@version 1.0
 */
public class WEB3ToSuccFuturesChangeOpenContractInputServiceImpl
    extends WEB3FuturesChangeOpenContractInputServiceImpl
    implements WEB3ToSuccFuturesChangeOpenContractInputService
{
    /**
     * ログ出力ユーティリティ。
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3ToSuccFuturesChangeOpenContractInputServiceImpl.class);

    /**
     * @@roseuid 47D6593400BB
     */
    public WEB3ToSuccFuturesChangeOpenContractInputServiceImpl()
    {
    ...
    public WEB3GenResponse execute(WEB3GenRequest l_request) throws WEB3BaseException
    {
        ...
        if (l_request instanceof WEB3SuccFuturesOpenChangeInputRequest)
        {
            l_response = this.createInputScreen((WEB3SuccFuturesOpenChangeInputRequest)l_request);
        }
        else
        {
     ...
     * 　@連続注文マネージャImpl.create先物OP注文単位(予約注文単位)をコールし、<BR>
     * 　@戻り値を返却する。<BR>
     * <BR>
     * 　@[引数]<BR>
     * 　@予約注文単位 ： １）の戻り値<BR>
     ...
     * 入力画面表示処理<BR>
     * <BR>
     * 「(（連続）先物訂正新規建入力)入力画面表示データ取得」参照<BR>
     * @@param l_request - (リクエストデータ)<BR>
     * リクエストデータ<BR>
     * @@return WEB3SuccFuturesOpenChangeInputResponse
     * @@throws WEB3BaseException
     * @@roseuid 47C252CA02BD
     */
    ...
    {
        final String STR_METHOD_NAME = "createInputScreen(WEB3SuccFuturesOpenChangeInputRequest)";
        log.entering(STR_METHOD_NAME);
        ...
        // create入力画面(リクエストデータ : 株価指数先物訂正新規建入力画面リクエスト)
        WEB3SuccFuturesOpenChangeInputResponse l_response =
            (WEB3SuccFuturesOpenChangeInputResponse)super.createInputScreen(l_request);
