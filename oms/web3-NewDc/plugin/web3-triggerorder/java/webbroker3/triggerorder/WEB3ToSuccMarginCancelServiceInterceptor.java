...
1.1
date	2011.03.15.01.55.33;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3ToSuccMarginCancelServiceInterceptor.java;
...
File Name        : （連続）信用取引取消サービスインタセプタ(WEB3ToSuccMarginCancelServiceInterceptor.java)
Author Name      : Daiwa Institute of Research
Revesion History : 2005/11/09 黄　@浩澎 (中訊) 作成
*/
...
/**
 * (（連続）信用取引取消サービスインタセプタ)<BR>
 * （連続）信用取引取消サービスインタセプタ<BR>
 * 
 * @@author 　@黄　@浩澎(中訊)
 * @@version 1.0 
 */
public class WEB3ToSuccMarginCancelServiceInterceptor implements Interceptor 
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3ToSuccMarginCancelServiceInterceptor.class);
    
    /**
     * @@roseuid 436ACB310222
     */
    public WEB3ToSuccMarginCancelServiceInterceptor() 
    {
     ...
     * <BR>
     * １）　@リクエストデータの型が「（連続）信用取引注文取消完了リクエスト」<BR>
     * の場合のみ、口座をロックする。<BR>
     ...
     * <BR>
     * 　@(*)リクエストデータオブジェクト.IDに該当する予約注文単位オブジェクトより編集。<BR>
     * 　@　@　@連続注文マネージャ.get予約注文単位(リクエストデータ.ID)で取得した<BR>
     * 　@　@　@予約注文単位より、取引カレンダコンテキストのプロパティをセットする。<BR>
     * <BR>
     * 　@　@　@○市場コード<BR>
     * 　@　@　@　@予約注文単位.get市場().市場コード<BR>
     * <BR>
     * 　@　@　@○受付時間区分<BR>
     * 　@　@　@　@予約注文単位.注文カテゴリ＝"現引・現渡"の場合：　@"現引・現渡"<BR>
     * 　@　@　@　@予約注文単位.注文カテゴリ≠"現引・現渡"の場合：　@"株式・信用"<BR>
     * <BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParam) 
    {
        ...
        try
        {
            ...
            //１）　@リクエストデータの型が「（連続）信用取引注文取消完了リクエスト」の場合のみ、口座をロックする。
            ...
            if (l_serviceParam[0] instanceof WEB3SuccMarginCancelCompleteRequest)
            {  
                l_accMgr.lockAccount(
                    l_strInstitutionCode, 
                    l_strBranchCode, 
                    l_mainAccount.getAccountCode());
                WEB3SuccMarginCancelCompleteRequest l_cancelCompleteRequest = 
                    (WEB3SuccMarginCancelCompleteRequest) l_serviceParam[0];
                l_strOrderId = l_cancelCompleteRequest.id;
            }
            else if (l_serviceParam[0] instanceof WEB3SuccMarginCancelConfirmRequest)
            {
                WEB3SuccMarginCancelConfirmRequest l_cancelConfirmRequest = 
                    (WEB3SuccMarginCancelConfirmRequest) l_serviceParam[0];
                l_strOrderId = l_cancelConfirmRequest.id;
            }
            else
            {
            ...
            //　@　@　@予約注文単位より、取引カレンダコンテキストのプロパティをセットする。
            ...
            //　@　@　@○受付時間区分
            //　@　@　@　@予約注文単位.注文カテゴリ＝"現引・現渡"の場合：　@"現引・現渡"
            //　@　@　@　@予約注文単位.注文カテゴリ≠"現引・現渡"の場合：　@"株式・信用"
