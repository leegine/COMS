...
1.1
date	2011.03.14.05.39.07;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	LoginHistoryPastRow.java;
...
/** 
 * LoginHistoryPastRowインタフェースは変更不可でリードオンリーである<em>login_history_past</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link LoginHistoryPastRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface LoginHistoryPastRow extends Row {


  /** 
   * この{@@link LoginHistoryPastRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "login_history_past", "session" );
