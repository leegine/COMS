...
1.1
date	2011.03.14.05.28.34;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	DocDeliveryManagementRow.java;
...
/** 
 * DocDeliveryManagementRowインタフェースは変更不可でリードオンリーである<em>doc_delivery_management</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link DocDeliveryManagementRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface DocDeliveryManagementRow extends Row {


  /** 
   * この{@@link DocDeliveryManagementRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "doc_delivery_management", "account" );
