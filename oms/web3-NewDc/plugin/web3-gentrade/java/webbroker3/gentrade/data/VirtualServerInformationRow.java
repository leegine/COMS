...
1.1
date	2011.03.14.05.18.22;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	VirtualServerInformationRow.java;
...
/** 
 * VirtualServerInformationRowインタフェースは変更不可でリードオンリーである<em>virtual_server_information</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link VirtualServerInformationRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface VirtualServerInformationRow extends Row {


  /** 
   * この{@@link VirtualServerInformationRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "virtual_server_information", "master" );
