...
1.1
date	2011.03.14.05.41.28;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	ExtMailProcTempParams.java;
...
/** 
 * ext_mail_proc_tempテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link ExtMailProcTempRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link ExtMailProcTempParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link ExtMailProcTempParams}が{@@link ExtMailProcTempRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class ExtMailProcTempParams extends Params implements ExtMailProcTempRow {
  ...
  public static final String PTYPE = "ext_mail_proc_temp";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = ExtMailProcTempRow.TYPE;
  ...
  public RowType getRowType() {
    return ExtMailProcTempRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のExtMailProcTempParamsオブジェクトを作成します。 
   */
  public ExtMailProcTempParams() {
  ...
  /** 
   * 指定のExtMailProcTempRowオブジェクトの値を利用してExtMailProcTempParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するExtMailProcTempRowオブジェクト 
   */
  public ExtMailProcTempParams( ExtMailProcTempRow p_row )
  {
  ...
  {
    if ( !( other instanceof ExtMailProcTempRow ) )
       return false;
    return fieldsEqual( (ExtMailProcTempRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のExtMailProcTempRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( ExtMailProcTempRow row )
  {
  ...
  {
    return new ExtMailProcTempPK(institution_code, branch_code, sendmail_div, discernment_id, account_code, mail_id, item_name);
  }
