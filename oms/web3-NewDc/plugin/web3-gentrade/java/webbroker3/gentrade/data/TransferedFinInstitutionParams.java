...
/** 
 * transfered_fin_institutionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link TransferedFinInstitutionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link TransferedFinInstitutionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link TransferedFinInstitutionParams}が{@@link TransferedFinInstitutionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
...
public class TransferedFinInstitutionParams extends Params implements TransferedFinInstitutionRow {
  ...
  /** 
   * 値が未設定のTransferedFinInstitutionParamsオブジェクトを作成します。 
   */
  ...
  /** 
   * 指定のTransferedFinInstitutionRowオブジェクトの値を利用してTransferedFinInstitutionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するTransferedFinInstitutionRowオブジェクト 
   */
   ...
   * @@param row 比較対照のTransferedFinInstitutionRow 
   ...
   *
   * @@param p_traderCode <em>trader_code</em>カラムの値をあらわす7桁以下のString値 
   */
