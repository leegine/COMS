...
1.1
date	2011.03.14.05.25.10;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	YellowCustomerRow.java;
...
/** 
 * YellowCustomerRowインタフェースは変更不可でリードオンリーである<em>yellow_customer</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link YellowCustomerRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface YellowCustomerRow extends Row {


  /** 
   * この{@@link YellowCustomerRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "yellow_customer", "master" );
