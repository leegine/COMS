...
1.1
date	2011.03.14.05.24.12;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	ExchangeRateParams.java;
...
/** 
 * exchange_rateテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link ExchangeRateRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link ExchangeRateParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link ExchangeRateParams}が{@@link ExchangeRateRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see ExchangeRateRow 
 */
public class ExchangeRateParams extends Params implements ExchangeRateRow {
  ...
  public static final String PTYPE = "exchange_rate";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = ExchangeRateRow.TYPE;
  ...
  /** 
   * 値が未設定のExchangeRateParamsオブジェクトを作成します。 
   */
  ...
  /** 
   * 指定のExchangeRateRowオブジェクトの値を利用してExchangeRateParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するExchangeRateRowオブジェクト 
   */
  public ExchangeRateParams( ExchangeRateRow p_row )
  {
  ...
  {
    if ( !( other instanceof ExchangeRateRow ) )
       return false;
    return fieldsEqual( (ExchangeRateRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のExchangeRateRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( ExchangeRateRow row )
  {
  ...
  {
    return new ExchangeRatePK(institution_code, currency_code);
  }
