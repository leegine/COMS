...
1.1
date	2011.03.14.05.36.29;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostOrderexecutionEndRow.java;
...
/** 
 * HostOrderexecutionEndRowインタフェースは変更不可でリードオンリーである<em>host_orderexecution_end</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostOrderexecutionEndRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostOrderexecutionEndRow extends Row {


  /** 
   * この{@@link HostOrderexecutionEndRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_orderexecution_end", "session" );
