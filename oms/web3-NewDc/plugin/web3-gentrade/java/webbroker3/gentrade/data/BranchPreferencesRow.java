...
1.1
date	2011.03.14.05.30.16;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BranchPreferencesRow.java;
...
/** 
 * BranchPreferencesRowインタフェースは変更不可でリードオンリーである<em>branch_preferences</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link BranchPreferencesRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface BranchPreferencesRow extends Row {


  /** 
   * この{@@link BranchPreferencesRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "branch_preferences", "master" );
