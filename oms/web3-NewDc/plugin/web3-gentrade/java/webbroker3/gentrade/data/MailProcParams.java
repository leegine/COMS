...
1.1
date	2011.03.14.05.32.50;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
/** 
 * mail_procテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MailProcRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MailProcParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MailProcParams}が{@@link MailProcRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see MailProcRow 
 */
public class MailProcParams extends Params implements MailProcRow {
  ...
  public static final RowType TYPE = MailProcRow.TYPE;
  ...
  /** 
   * 値が未設定のMailProcParamsオブジェクトを作成します。 
   */
  public MailProcParams() {
  ...
  /** 
   * 指定のMailProcRowオブジェクトの値を利用してMailProcParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMailProcRowオブジェクト 
   */
  public MailProcParams( MailProcRow p_row )
  {
  ...
  {
    if ( !( other instanceof MailProcRow ) )
       return false;
    return fieldsEqual( (MailProcRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMailProcRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MailProcRow row )
  {
  ...
  {
    return new MailProcPK(institution_code, branch_code, sendmail_div, discernment_id, account_code, mail_id);
  }
