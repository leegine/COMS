...
1.1
date	2011.03.14.05.24.08;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RequestCodesForReadParams.java;
...
/** 
 * request_codes_for_readテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RequestCodesForReadRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RequestCodesForReadParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RequestCodesForReadParams}が{@@link RequestCodesForReadRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RequestCodesForReadParams extends Params implements RequestCodesForReadRow {
  ...
  public static final RowType TYPE = RequestCodesForReadRow.TYPE;
  ...
  /** 
   * 値が未設定のRequestCodesForReadParamsオブジェクトを作成します。 
   */
  public RequestCodesForReadParams() {
  ...
  /** 
   * 指定のRequestCodesForReadRowオブジェクトの値を利用してRequestCodesForReadParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRequestCodesForReadRowオブジェクト 
   */
  public RequestCodesForReadParams( RequestCodesForReadRow p_row )
  {
  ...
  {
    if ( !( other instanceof RequestCodesForReadRow ) )
       return false;
    return fieldsEqual( (RequestCodesForReadRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRequestCodesForReadRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RequestCodesForReadRow row )
  {
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !submit_order_route_div_is_set )
			throw new IllegalArgumentException("non-nullable field 'submit_order_route_div' must be set before inserting.");
	}
  ...
  {
    return new RequestCodesForReadPK(request_code, ptype);
  }
