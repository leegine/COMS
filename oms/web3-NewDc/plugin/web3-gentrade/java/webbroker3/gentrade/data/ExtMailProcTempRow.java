...
1.1
date	2011.03.14.05.21.10;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	ExtMailProcTempRow.java;
...
/** 
 * ExtMailProcTempRowインタフェースは変更不可でリードオンリーである<em>ext_mail_proc_temp</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link ExtMailProcTempRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface ExtMailProcTempRow extends Row {


  /** 
   * この{@@link ExtMailProcTempRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "ext_mail_proc_temp", "session" );
