...
1.1
date	2011.03.14.05.20.33;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	FeqCalendarParams.java;
...
/** 
 * feq_calendarテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link FeqCalendarRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link FeqCalendarParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link FeqCalendarParams}が{@@link FeqCalendarRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see FeqCalendarRow 
 */
public class FeqCalendarParams extends Params implements FeqCalendarRow {
  ...
  public static final RowType TYPE = FeqCalendarRow.TYPE;
  ...
  /** 
   * 値が未設定のFeqCalendarParamsオブジェクトを作成します。 
   */
  public FeqCalendarParams() {
  ...
  /** 
   * 指定のFeqCalendarRowオブジェクトの値を利用してFeqCalendarParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するFeqCalendarRowオブジェクト 
   */
  public FeqCalendarParams( FeqCalendarRow p_row )
  {
  ...
  {
    if ( !( other instanceof FeqCalendarRow ) )
       return false;
    return fieldsEqual( (FeqCalendarRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のFeqCalendarRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( FeqCalendarRow row )
  {
  ...
  {
    return new FeqCalendarPK(institution_code, market_code, biz_date);
  }
