...
1.1
date	2011.03.14.05.35.02;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	ExclusiveUseAccountRow.java;
...
/** 
 * ExclusiveUseAccountRowインタフェースは変更不可でリードオンリーである<em>exclusive_use_account</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link ExclusiveUseAccountRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface ExclusiveUseAccountRow extends Row {


  /** 
   * この{@@link ExclusiveUseAccountRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "exclusive_use_account", "account" );
