...
/** 
 * DailyAssetRowインタフェースは変更不可でリードオンリーである<em>daily_asset</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link DailyAssetRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface DailyAssetRow extends Row {
  ...
  /** 
   * この{@@link DailyAssetRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
