...
1.1
date	2011.03.14.05.32.16;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
/** 
 * insiderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link InsiderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link InsiderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link InsiderParams}が{@@link InsiderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see InsiderRow 
 */
public class InsiderParams extends Params implements InsiderRow {
  ...
  public static final RowType TYPE = InsiderRow.TYPE;
   ...
   */
  public  java.sql.Timestamp  sonar_created_date;
  ...
  /** 
   * 値が未設定のInsiderParamsオブジェクトを作成します。 
   */
  public InsiderParams() {
  ...
  /** 
   * 指定のInsiderRowオブジェクトの値を利用してInsiderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するInsiderRowオブジェクト 
   */
  public InsiderParams( InsiderRow p_row )
  {
  ...
  {
    if ( !( other instanceof InsiderRow ) )
       return false;
    return fieldsEqual( (InsiderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のInsiderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( InsiderRow row )
  {
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !relation_div_is_set )
			throw new IllegalArgumentException("non-nullable field 'relation_div' must be set before inserting.");
   ...
   */
  public final java.sql.Timestamp getSonarCreatedDate()
  {
  ...
  {
    return new InsiderPK(institution_code, branch_id, account_id, product_id);
  }
   ...
   *
   * @@param p_relationDiv <em>relation_div</em>カラムの値をあらわす2桁以下のString値 
   */
   ...
   *
   * @@param p_officerName <em>officer_name</em>カラムの値をあらわす40桁以下のString値 
   */
   ...
   *
   * @@param p_postName <em>post_name</em>カラムの値をあらわす40桁以下のString値 
   */
   ...
   *
   * @@param p_sonarCreatedDate <em>sonar_created_date</em>カラムの値をあらわすjava.sql.Timestamp値
   */
  public final void setSonarCreatedDate( java.sql.Timestamp p_sonarCreatedDate )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
   ...
   /** 
   * 一般のjava.util.Dateオブジェクトを使って<em>sonar_created_date</em>カラムに値を設定します。 
   * @@param date 設定する値を含むjava.util.Date 
    */
  public final void setSonarCreatedDate( java.util.Date date )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
    sonar_created_date = (date==null? null: new java.sql.Timestamp( date.getTime() ));
    sonar_created_date_is_set = true;
    sonar_created_date_is_modified = true;
  }
    ...
    public Object getColumn( String name ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'r':
                ...
                break;
            case 's':
                if ( name.equals("sonar_created_date") ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 's':
                if ( name.equals("sonar_created_date") ) {
                    if ( value != null )
                      if ( !(value instanceof java.sql.Timestamp) )
                        throw new IllegalArgumentException( "value for 'sonar_created_date' must be java.sql.Timestamp: '"+value+"' is not." );
                    this.sonar_created_date = (java.sql.Timestamp) value;
