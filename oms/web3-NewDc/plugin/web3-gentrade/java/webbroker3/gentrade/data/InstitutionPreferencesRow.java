...
1.1
date	2011.03.14.05.32.08;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	InstitutionPreferencesRow.java;
...
/** 
 * InstitutionPreferencesRowインタフェースは変更不可でリードオンリーである<em>institution_preferences</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link InstitutionPreferencesRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface InstitutionPreferencesRow extends Row {


  /** 
   * この{@@link InstitutionPreferencesRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "institution_preferences", "master" );
