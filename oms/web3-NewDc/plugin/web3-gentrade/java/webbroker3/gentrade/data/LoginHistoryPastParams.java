...
1.1
date	2011.03.14.05.27.44;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	LoginHistoryPastParams.java;
...
/** 
 * login_history_pastテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link LoginHistoryPastRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link LoginHistoryPastParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link LoginHistoryPastParams}が{@@link LoginHistoryPastRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class LoginHistoryPastParams extends Params implements LoginHistoryPastRow {
  ...
  public static final String PTYPE = "login_history_past";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = LoginHistoryPastRow.TYPE;
  ...
  /** 
   * 値が未設定のLoginHistoryPastParamsオブジェクトを作成します。 
   */
  public LoginHistoryPastParams() {
  ...
  /** 
   * 指定のLoginHistoryPastRowオブジェクトの値を利用してLoginHistoryPastParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するLoginHistoryPastRowオブジェクト 
   */
  public LoginHistoryPastParams( LoginHistoryPastRow p_row )
  {
  ...
  {
    if ( !( other instanceof LoginHistoryPastRow ) )
       return false;
    return fieldsEqual( (LoginHistoryPastRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のLoginHistoryPastRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( LoginHistoryPastRow row )
  {
  ...
  {
    return new LoginHistoryPastPK(login_history_id);
  }
