...
filename	HostEqtypeSwapReceiptRow.java;
...
/** 
 * HostEqtypeSwapReceiptRowインタフェースは変更不可でリードオンリーである<em>host_eqtype_swap_receipt</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostEqtypeSwapReceiptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostEqtypeSwapReceiptRow extends Row {


  /** 
   * この{@@link HostEqtypeSwapReceiptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_eqtype_swap_receipt", "session" );
