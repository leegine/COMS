...
1.1
date	2011.03.14.07.01.01;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3SeshSleRequestPreparer.java;
...
@/**
 ...
 File Name        : WEB3SeshSleRequestPreparerクラス
 Author Name      : Daiwa Institute of Research
 Revision History : 2008/09/01 許 新規作成
 */
...
/**
 * 上海市場へ送信する前で電文変換処理を行うクラス.
 */
public class WEB3SeshSleRequestPreparer implements WEB3SleRequestPreparer
{
    /**
     * ログ出力ユーティリティ
     */
    private static final WEB3LogUtility m_log =
        WEB3LogUtility.getInstance(WEB3SeshSleRequestPreparer.class);
     ...
     */
    private static WEB3SeshSleRequestPreparer m_preparer = null;
    
    /**
     * コンストラクタ
     */
    public WEB3SeshSleRequestPreparer(WEB3SleProcessors wsp)
    {
        m_wsp = wsp;
    }
     ...
     * コンストラクタ
     */
    private WEB3SeshSleRequestPreparer() {
        ;
    }
     ...
     * Singletonインスタンス取得する
     */
    public static WEB3SeshSleRequestPreparer getInstance()
    {
        
        if(m_preparer == null)
        {
                m_preparer =  new WEB3SeshSleRequestPreparer(new WEB3SleProcessorsImpl());
        } 
        return m_preparer;
    }
    ...
    public GlSleRequest prepare(SleSendQRow l_sendqRow)
    {
        ...
        else if (SleSendqOpTypeEnum.CANCEL_ORDER.equals(opType))
        {
            ...
            //変更注文
            final String errMsg =
                "Invalid op_type in sle_send_q row for shanghai market. Row Values:" + l_sendqRow;
            m_log.error(errMsg);
        ...
        final GlSleRequest sleReq =
            new GlSleRequest(SleConstants.Markets.SESH.F_GL_REQ_ID, l_glData);
...
//        final String modality = getModality(l_sendqRow);　@//2006/10/6 孫　@修正
        l_glData.putString(SleConstants.Order2000MsgItem.F_MODALITY, SleConstants.Markets.SESH.Modality.LIMIT_PRICE);
        ...
        l_glData.putString(SleConstants.Order2000MsgItem.F_VALIDITY, SleConstants.Markets.SESH.Validity.ALL_DAY);
...
//        l_glData.putString(SleConstants.Order2000MsgItem.F_CUSTOMER_ACCOUNT, "C990064726");
...
//       l_glData.putString(
//                SleConstants.Order2000HK.F_USER_NUMBER,
 //               SleConstants.Markets.HK.DIR_HK_GL_NO);
//         l_glData.putBigDecimal(
//                   SleConstants.Order2000HK.F_USER_NUMBER,
//                  new BigDecimal(SleConstants.Markets.HK.DIR_HK_GL_NO));        
        //テスト用"101"に設定している
...
//                  new BigDecimal(getUserNo()));         
        ...
        l_glData.putString(
            ...
            SleConstants.Markets.SESH.OrderCategory.SINGLE);
...
//            SleConstants.Order2000MsgItem.F_LONG_NAME,
    ...
    private String getModality(SleSendQRow l_sendqRow)
    {
        ...
        {
            //成り行き
            return SleConstants.Markets.SESH.Modality.ANY_PRICE;
        }
        else{
            //指値(当面、'指値'のみ指定可能)
            return SleConstants.Markets.SESH.Modality.LIMIT_PRICE;
        }
    }
    ...
    private BigDecimal getSleSide(SleSendQRow l_sendqRow)
    {
        ...
        switch (l_sendqRow.getOrderType().intValue()) {
            ...
            case OrderTypeEnum.IntValues.FEQ_BUY:
                ...
                return new BigDecimal(SleConstants.Markets.SESH.Side.BUY);

                //外株売
            case OrderTypeEnum.IntValues.FEQ_SELL:
                m_log.exiting("getSleSide(SleSendQRow)");
                return new BigDecimal(SleConstants.Markets.SESH.Side.SELL);
