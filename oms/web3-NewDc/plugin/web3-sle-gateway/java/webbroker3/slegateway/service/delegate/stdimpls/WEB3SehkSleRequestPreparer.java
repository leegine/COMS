...
1.1
date	2011.03.14.07.00.56;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3SehkSleRequestPreparer.java;
...
@/**
 ...
 Revision History :	2006/05/31 李(FLJ) 電文仕様変更
 Revision History : 2006/6/8 李(FLJ)　@ソース精査
 Revision History : 2006/12/4 李(FLJ) クラスを Singletonインスタンスにするため改修
 */
...
/**
 * 市場へ送信する前で電文変換処理を行うクラス.
 */
public class WEB3SehkSleRequestPreparer implements WEB3SleRequestPreparer
{
	/**
	 * ログ出力ユーティリティ
	 */
	private static final WEB3LogUtility m_log =
		WEB3LogUtility.getInstance(WEB3SehkSleRequestPreparer.class);
	 ...
	 */
	private static WEB3SehkSleRequestPreparer m_preparer = null;
	
	/**
	 * コンストラクタ
	 */
	public WEB3SehkSleRequestPreparer(WEB3SleProcessors wsp)
	{
		m_wsp = wsp;
	}
	 ...
	 * コンストラクタ
	 */
	private WEB3SehkSleRequestPreparer() {
		;
	}
	 ...
	 * Singletonインスタンス取得する
	 */
	public static WEB3SehkSleRequestPreparer getInstance()
	{
		
		if(m_preparer == null)
		{
				m_preparer =  new WEB3SehkSleRequestPreparer(new WEB3SleProcessorsImpl());
		} 
		return m_preparer;
	}
	...
	public GlSleRequest prepare(SleSendQRow l_sendqRow)
	{
		...
		final GlSleRequest sleReq =
			new GlSleRequest(SleConstants.Markets.SEHK.F_GL_REQ_ID, l_glData);
...
//		final String modality = getModality(l_sendqRow);　@//2006/10/6 孫　@修正
		l_glData.putString(SleConstants.Order2000MsgItem.F_MODALITY, SleConstants.Markets.SEHK.Modality.LIMIT_PRICE);
        ...
        l_glData.putString(SleConstants.Order2000MsgItem.F_VALIDITY, SleConstants.Markets.SEHK.Validity.ALL_DAY);
...
//				  new BigDecimal(getUserNo()));         
		...
		l_glData.putString(
			...
			SleConstants.Markets.SEHK.OrderCategory.SINGLE);
...
//			SleConstants.Order2000MsgItem.F_LONG_NAME,
	...
	private String getModality(SleSendQRow l_sendqRow)
	{
		...
		{
			//成り行き
			return SleConstants.Markets.SEHK.Modality.ANY_PRICE;
		}
		else{
			//指値(当面、'指値'のみ指定可能)
			return SleConstants.Markets.SEHK.Modality.LIMIT_PRICE;
		}
	}
	...
	private BigDecimal getSleSide(SleSendQRow l_sendqRow)
	{
		...
		switch (l_sendqRow.getOrderType().intValue()) {
			...
			case OrderTypeEnum.IntValues.FEQ_BUY:
				...
				return new BigDecimal(SleConstants.Markets.SEHK.Side.BUY);

				//外株売
			case OrderTypeEnum.IntValues.FEQ_SELL:
				m_log.exiting("getSleSide(SleSendQRow)");
				return new BigDecimal(SleConstants.Markets.SEHK.Side.SELL);
