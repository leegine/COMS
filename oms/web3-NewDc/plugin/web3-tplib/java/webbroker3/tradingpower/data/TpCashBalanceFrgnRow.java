...
/** 
 * TpCashBalanceFrgnRowインタフェースは変更不可でリードオンリーである<em>tp_cash_balance_frgn</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link TpCashBalanceFrgnRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface TpCashBalanceFrgnRow extends Row {
  ...
  /** 
   * この{@@link TpCashBalanceFrgnRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
