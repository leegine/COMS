...
1.1
date	2011.03.15.01.00.31;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoDepositCalcLockParams.java;
...
/** 
 * ifo_deposit_calc_lockテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link IfoDepositCalcLockRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link IfoDepositCalcLockParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link IfoDepositCalcLockParams}が{@@link IfoDepositCalcLockRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class IfoDepositCalcLockParams extends Params implements IfoDepositCalcLockRow {
  ...
  public static final RowType TYPE = IfoDepositCalcLockRow.TYPE;
  ...
  private boolean ap_host_name_is_set = false;
  private boolean ap_host_name_is_modified = false;
  ...
  /** 
   * 値が未設定のIfoDepositCalcLockParamsオブジェクトを作成します。 
   */
  public IfoDepositCalcLockParams() {
  ...
  /** 
   * 指定のIfoDepositCalcLockRowオブジェクトの値を利用してIfoDepositCalcLockParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するIfoDepositCalcLockRowオブジェクト 
   */
  public IfoDepositCalcLockParams( IfoDepositCalcLockRow p_row )
  {
    ...
    ap_host_name = p_row.getApHostName();
    ap_host_name_is_set = p_row.getApHostNameIsSet();
    ap_host_name_is_modified = p_row.getApHostNameIsModified();
   ...
   */
    public void markAllValuesAsSet() {
      ...
      ap_host_name_is_set = true;
      ap_host_name_is_modified = true;
      status_is_set = true;
  ...
  {
    if ( !( other instanceof IfoDepositCalcLockRow ) )
       return false;
    return fieldsEqual( (IfoDepositCalcLockRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のIfoDepositCalcLockRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( IfoDepositCalcLockRow row )
  {
    ...
    if ( ap_host_name == null ) {
      if ( row.getApHostName() != null )
        return false;
    } else if ( !ap_host_name.equals( row.getApHostName() ) ) {
        return false;
    }
    if ( status == null ) {
  ...
  public int hashCode() {
      return  (service_name!=null? service_name.hashCode(): 0) 
        + ((int) thread_no)
        + (ap_host_name!=null? ap_host_name.hashCode(): 0) 
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !ap_host_name_is_set )
			throw new IllegalArgumentException("non-nullable field 'ap_host_name' must be set before inserting.");
	}
   ...
   */
	public java.util.Map toInsertMap() {
		...
		map.put("ap_host_name",ap_host_name);
		if ( status_is_set )
   ...
   * @@return Stringの値 
   */
  public final String getApHostName()
  {
    return ap_host_name;
  }
   ...
   * @@return 値が既に設定されている場合はtrue、そうでなければfalse 
   */
  public final boolean getApHostNameIsSet() {
    return ap_host_name_is_set;
  }
   ...
   * @@return 値が既に変更されている場合はtrue、そうでなければfalse 
   */
  public final boolean getApHostNameIsModified() {
    return ap_host_name_is_modified;
  }
  ...
  {
    return new IfoDepositCalcLockPK(service_name, thread_no);
  }
  ...
  /** 
   * <em>ap_host_name</em>カラムの値を設定します。 
   *
   * @@param p_apHostName <em>ap_host_name</em>カラムの値をあらわす100桁以下のString値 
   */
  public final void setApHostName( String p_apHostName )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    ap_host_name = p_apHostName;
    ap_host_name_is_set = true;
    ap_host_name_is_modified = true;
  }
            ...
            case 'a':
                if ( name.equals("ap_host_name") ) {
                    if ( !(value instanceof String) )
                        throw new IllegalArgumentException( "value for 'ap_host_name' must be String: '"+value+"' is not." );
                    this.ap_host_name = (String) value;
                    if (this.ap_host_name_is_set)
                        this.ap_host_name_is_modified = true;
                    this.ap_host_name_is_set = true;
                    return;
                }
                break;
            case 'c':
