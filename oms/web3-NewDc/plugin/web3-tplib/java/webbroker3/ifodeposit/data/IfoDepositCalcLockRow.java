...
1.1
date	2011.03.15.01.01.11;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoDepositCalcLockRow.java;
...
/** 
 * IfoDepositCalcLockRowインタフェースは変更不可でリードオンリーである<em>ifo_deposit_calc_lock</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link IfoDepositCalcLockRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface IfoDepositCalcLockRow extends Row {


  /** 
   * この{@@link IfoDepositCalcLockRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "ifo_deposit_calc_lock", "session" );
