...
1.1
date	2011.03.16.05.45.32;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	FxAccountCodeParams.java;
...
/** 
 * fx_account_codeテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link FxAccountCodeRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link FxAccountCodeParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link FxAccountCodeParams}が{@@link FxAccountCodeRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class FxAccountCodeParams extends Params implements FxAccountCodeRow {
  ...
  public static final RowType TYPE = FxAccountCodeRow.TYPE;
  ...
  private boolean fx_course_div_is_set = false;
  private boolean fx_course_div_is_modified = false;
  ...
  /** 
   * 値が未設定のFxAccountCodeParamsオブジェクトを作成します。 
   */
  public FxAccountCodeParams() {
    institution_code_is_modified = true;
    branch_code_is_modified = true;
    fx_system_code_is_modified = true;
    account_code_is_modified = true;
    fx_course_div_is_modified = true;
  }
  ...
  /** 
   * 指定のFxAccountCodeRowオブジェクトの値を利用してFxAccountCodeParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するFxAccountCodeRowオブジェクト 
   */
  public FxAccountCodeParams( FxAccountCodeRow p_row )
  {
    ...
    fx_course_div = p_row.getFxCourseDiv();
    fx_course_div_is_set = p_row.getFxCourseDivIsSet();
    fx_course_div_is_modified = p_row.getFxCourseDivIsModified();
  ...
  {
    if ( !( other instanceof FxAccountCodeRow ) )
       return false;
    return fieldsEqual( (FxAccountCodeRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のFxAccountCodeRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( FxAccountCodeRow row )
  {
    ...
    if ( fx_course_div == null ) {
      if ( row.getFxCourseDiv() != null )
        return false;
    } else if ( !fx_course_div.equals( row.getFxCourseDiv() ) ) {
        return false;
    }
   ...
   */
	public java.util.Map toInsertMap() {
		...
		map.put("fx_course_div",fx_course_div);
  ...
  /** 
   * <em>fx_course_div</em>カラムの値を取得します。
   * @@return Stringの値 
   */
  public final String getFxCourseDiv()
  {
    return fx_course_div;
  }


  /** 
   * <em>fx_course_div</em>カラムに値が設定されているかどうか調べます。
   * @@return 値が既に設定されている場合はtrue、そうでなければfalse 
   */
  public final boolean getFxCourseDivIsSet() {
    return fx_course_div_is_set;
  }


  /** 
   * <em>fx_course_div</em>カラムの値が変更されているかどうか調べます。
   * @@return 値が既に変更されている場合はtrue、そうでなければfalse 
   */
  public final boolean getFxCourseDivIsModified() {
    return fx_course_div_is_modified;
  }
  ...
  {
    return new FxAccountCodePK(institution_code, branch_code, fx_system_code, account_code, fx_course_div);
  }
  ...
  /** 
   * <em>fx_course_div</em>カラムの値を設定します。 
   *
   * @@param p_fxCourseDiv <em>fx_course_div</em>カラムの値をあらわす1桁以下のString値 
   */
  public final void setFxCourseDiv( String p_fxCourseDiv )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    fx_course_div = p_fxCourseDiv;
    fx_course_div_is_set = true;
    fx_course_div_is_modified = true;
  }
    ...
    public Object getColumn( String name ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'f':
                ...
                else if ( name.equals("fx_course_div") ) {
                    return this.fx_course_div;
                }
                else if ( name.equals("fx_account_code") ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'f':
                if ( name.equals("fx_system_code") ) {
                    ...
                    if ( !(value instanceof String) )
                        throw new IllegalArgumentException( "value for 'fx_course_div' must be String: '"+value+"' is not." );
                    this.fx_course_div = (String) value;
                    if (this.fx_course_div_is_set)
                        this.fx_course_div_is_modified = true;
                    this.fx_course_div_is_set = true;
                    return;
                }
                else if ( name.equals("fx_account_code") ) {
                    if ( !(value instanceof String) )
