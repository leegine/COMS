...
1.1
date	2011.03.16.02.52.03;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BankOrderRequestParams.java;
...
/** 
 * bank_order_requestテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link BankOrderRequestRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link BankOrderRequestParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link BankOrderRequestParams}が{@@link BankOrderRequestRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class BankOrderRequestParams extends Params implements BankOrderRequestRow {
  ...
  public static final String PTYPE = "bank_order_request";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = BankOrderRequestRow.TYPE;
  ...
  public RowType getRowType() {
    return BankOrderRequestRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のBankOrderRequestParamsオブジェクトを作成します。 
   */
  public BankOrderRequestParams() {
  ...
  /** 
   * 指定のBankOrderRequestRowオブジェクトの値を利用してBankOrderRequestParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するBankOrderRequestRowオブジェクト 
   */
  public BankOrderRequestParams( BankOrderRequestRow p_row )
  {
  ...
  {
    if ( !( other instanceof BankOrderRequestRow ) )
       return false;
    return fieldsEqual( (BankOrderRequestRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のBankOrderRequestRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( BankOrderRequestRow row )
  {
  ...
  {
    return new BankOrderRequestPK(rowid);
  }
