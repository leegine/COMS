...
1.1
date	2011.03.16.02.37.32;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostSecDeliveryTransferRow.java;
...
/** 
 * HostSecDeliveryTransferRowインタフェースは変更不可でリードオンリーである<em>host_sec_delivery_transfer</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostSecDeliveryTransferRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostSecDeliveryTransferRow extends Row {


  /** 
   * この{@@link HostSecDeliveryTransferRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_sec_delivery_transfer", "session" );
