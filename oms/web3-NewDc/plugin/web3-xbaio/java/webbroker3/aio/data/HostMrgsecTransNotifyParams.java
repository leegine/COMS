...
1.1
date	2011.03.16.05.37.59;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostMrgsecTransNotifyParams.java;
...
/** 
 * host_mrgsec_trans_notifyテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link HostMrgsecTransNotifyRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link HostMrgsecTransNotifyParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link HostMrgsecTransNotifyParams}が{@@link HostMrgsecTransNotifyRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class HostMrgsecTransNotifyParams extends Params implements HostMrgsecTransNotifyRow {
  ...
  public static final String PTYPE = "host_mrgsec_trans_notify";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = HostMrgsecTransNotifyRow.TYPE;
  ...
  public RowType getRowType() {
    return HostMrgsecTransNotifyRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のHostMrgsecTransNotifyParamsオブジェクトを作成します。 
   */
  public HostMrgsecTransNotifyParams() {
  }
  ...
  /** 
   * 指定のHostMrgsecTransNotifyRowオブジェクトの値を利用してHostMrgsecTransNotifyParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するHostMrgsecTransNotifyRowオブジェクト 
   */
  public HostMrgsecTransNotifyParams( HostMrgsecTransNotifyRow p_row )
  {
  ...
  {
    if ( !( other instanceof HostMrgsecTransNotifyRow ) )
       return false;
    return fieldsEqual( (HostMrgsecTransNotifyRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のHostMrgsecTransNotifyRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( HostMrgsecTransNotifyRow row )
  {
  ...
  {
    throw new UnsupportedOperationException("Table host_mrgsec_trans_notify has no primary key.");
  }
