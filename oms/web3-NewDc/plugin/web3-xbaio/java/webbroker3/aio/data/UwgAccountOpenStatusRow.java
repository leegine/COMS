...
1.1
date	2011.03.16.05.45.37;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	UwgAccountOpenStatusRow.java;
...
/** 
 * UwgAccountOpenStatusRowインタフェースは変更不可でリードオンリーである<em>uwg_account_open_status</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link UwgAccountOpenStatusRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface UwgAccountOpenStatusRow extends Row {


  /** 
   * この{@@link UwgAccountOpenStatusRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "uwg_account_open_status", "session" );
