...
1.1
date	2011.03.16.05.46.09;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	OtherOrderExecutedCountParams.java;
...
/** 
 * other_order_executed_countテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link OtherOrderExecutedCountRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link OtherOrderExecutedCountParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link OtherOrderExecutedCountParams}が{@@link OtherOrderExecutedCountRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class OtherOrderExecutedCountParams extends Params implements OtherOrderExecutedCountRow {
  ...
  public static final RowType TYPE = OtherOrderExecutedCountRow.TYPE;
  ...
  /** 
   * 値が未設定のOtherOrderExecutedCountParamsオブジェクトを作成します。 
   */
  public OtherOrderExecutedCountParams() {
  ...
  /** 
   * 指定のOtherOrderExecutedCountRowオブジェクトの値を利用してOtherOrderExecutedCountParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するOtherOrderExecutedCountRowオブジェクト 
   */
  public OtherOrderExecutedCountParams( OtherOrderExecutedCountRow p_row )
  {
  ...
  {
    if ( !( other instanceof OtherOrderExecutedCountRow ) )
       return false;
    return fieldsEqual( (OtherOrderExecutedCountRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のOtherOrderExecutedCountRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( OtherOrderExecutedCountRow row )
  {
  ...
  {
    return new OtherOrderExecutedCountPK(institution_code, branch_code, record_div, product_div, pay_scheme_id, order_chanel, order_root_div, biz_date);
  }
