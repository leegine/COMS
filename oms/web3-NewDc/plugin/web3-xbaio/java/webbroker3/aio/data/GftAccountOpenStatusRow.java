...
/** 
 * GftAccountOpenStatusRowインタフェースは変更不可でリードオンリーである<em>gft_account_open_status</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link GftAccountOpenStatusRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface GftAccountOpenStatusRow extends Row {
  ...
  /** 
   * この{@@link GftAccountOpenStatusRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
