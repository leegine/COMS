...
1.1
date	2011.03.16.05.44.55;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostCashTransferRow.java;
...
/** 
 * HostCashTransferRowインタフェースは変更不可でリードオンリーである<em>host_cash_transfer</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostCashTransferRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostCashTransferRow extends Row {


  /** 
   * この{@@link HostCashTransferRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_cash_transfer", "session" );
