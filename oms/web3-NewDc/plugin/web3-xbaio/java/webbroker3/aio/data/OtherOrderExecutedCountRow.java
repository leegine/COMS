...
1.1
date	2011.03.16.05.37.53;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	OtherOrderExecutedCountRow.java;
...
/** 
 * OtherOrderExecutedCountRowインタフェースは変更不可でリードオンリーである<em>other_order_executed_count</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link OtherOrderExecutedCountRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface OtherOrderExecutedCountRow extends Row {


  /** 
   * この{@@link OtherOrderExecutedCountRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "other_order_executed_count", "master" );
