...
1.1
date	2011.03.16.05.44.02;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostTransferAcceptParams.java;
...
/** 
 * host_transfer_acceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link HostTransferAcceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link HostTransferAcceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link HostTransferAcceptParams}が{@@link HostTransferAcceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class HostTransferAcceptParams extends Params implements HostTransferAcceptRow {
  ...
  public static final String PTYPE = "host_transfer_accept";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = HostTransferAcceptRow.TYPE;
  ...
  public RowType getRowType() {
    return HostTransferAcceptRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のHostTransferAcceptParamsオブジェクトを作成します。 
   */
  public HostTransferAcceptParams() {
  ...
  /** 
   * 指定のHostTransferAcceptRowオブジェクトの値を利用してHostTransferAcceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するHostTransferAcceptRowオブジェクト 
   */
  public HostTransferAcceptParams( HostTransferAcceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof HostTransferAcceptRow ) )
       return false;
    return fieldsEqual( (HostTransferAcceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のHostTransferAcceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( HostTransferAcceptRow row )
  {
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !error_message_is_set )
			throw new IllegalArgumentException("non-nullable field 'error_message' must be set before inserting.");
		if ( !status_is_set )
  ...
  {
    return new HostTransferAcceptPK(institution_code, branch_code, account_code, order_request_number);
  }
