...
1.1
date	2011.03.16.02.30.54;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3AccTransChangeAcceptUnitServiceInterceptor.java;
...
File Name        : 振替受付UnitServiceインタセプタ(WEB3AccTransChangeAcceptUnitServiceInterceptor)
...
/**
 * (振替受付UnitServiceインタセプタ)<BR>
 * 振替受付UnitServiceインタセプタクラス<BR>
 * 
 * @@author 于美麗(中訊)
 * @@version 1.0
 */
public class WEB3AccTransChangeAcceptUnitServiceInterceptor
    implements Interceptor
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3AccTransChangeAcceptUnitServiceInterceptor.class);
     ...
     * <BR>
     * (*1)受付時間区分の取得方法@ <BR>
     * 　@　@注文単位.注文種別が以下のいずれかの場合、”23：為替保証金” <BR>
     ...
     *      ”09：信用保証金への振替” <BR>
     *  注文単位.注文種別 = 1006：振替注文（信用保証金から預り金） の場合、<BR>
     *      ”10：信用保証金からの振替” <BR>
     *  注文単位.注文種別 = 1007：振替注文（預り金から株先証拠金） の場合、<BR>
     *      ”12：先OP証拠金への振替” <BR>
     *  注文単位.注文種別 = 1008：振替注文（株先証拠金から預り金） の場合、<BR>
     *      ”13：先OP証拠金からの振替” <BR>
     *  注文単位.注文種別が以下のいずれかの場合、”23：為替保証金” <BR>
     * 　@　@　@1011(為替保証金振替注文(預かり金から為替保証金)) <BR>
     * 　@　@　@1012(為替保証金振替注文(為替保証金から預かり金)) <BR>
