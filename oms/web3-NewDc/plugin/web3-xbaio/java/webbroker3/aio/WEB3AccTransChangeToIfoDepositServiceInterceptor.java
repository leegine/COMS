...
1.1
date	2011.03.16.02.27.47;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3AccTransChangeToIfoDepositServiceInterceptor.java;
...
File Name        : 証拠金への振替サービスインタセプタ(WEB3AccTransChangeToIfoDepositServiceInterceptor)
...
/**
 * (証拠金への振替サービスインタセプタ)<BR>
 * 証拠金への振替サービスインタセプタクラス<BR>
 * 
 * @@author 于美麗(中訊)
 * @@version 1.0
 */
public class WEB3AccTransChangeToIfoDepositServiceInterceptor
    implements Interceptor
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3AccTransChangeToIfoDepositServiceInterceptor.class);
     ...
     * 　@取引カレンダコンテキスト.注文受付商品 = ”12：先OP証拠金への振替” <BR>
...
　@   *  －拡張アカウントマネージャ.lock口座(証券会社コード, 部店コード, 口座コード)をコールする。<BR> 
    ...
    public Object onCall(Method l_method, Object[] l_serviceParams)
    {
        ...
        try
        {
            ...
            //取引カレンダコンテキスト.注文受付商品 = ”12：先OP証拠金への振替”
            ...
            //３）　@リクエストデータの型が「証拠金への振替完了リクエスト」の場合のみ、口座をロックする。 
            //　@－拡張アカウントマネージャ.lock口座(証券会社コード, 部店コード, 口座コード)をコールする。 
            //※引数はOpLoginSecurityServiceより編集。.
            Object l_request = l_serviceParams[0];
            
            if (l_request instanceof WEB3AccTransChangeToIfoDepositCompleteRequest)
            {
