...
1.1
date	2011.03.16.02.21.26;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3FXTransFromFXServiceInterceptor.java;
...
@/**
 ...
 File Name        : FXから振替サービスインタセプタ(WEB3FXTransFromFXServiceInterceptor)
 Author Name      : Daiwa Institute of Research
 Revesion History : 2005/01/21 黄建(中訊) 新規作成
 Revesion History : 2008/09/23 馮海濤 (中訊) 仕様変更・モデルNo.995、1024、1079
 Revesion History : 2009/03/11 柴双紅 (中訊) 仕様変更・モデルNo.1111
 */
...
public class WEB3FXTransFromFXServiceInterceptor implements Interceptor
{
    /**
     * ログユーティリティ
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3FXTransFromFXServiceInterceptor.class);
     ...
     *<BR>
     * ５）リクエストデータの型が「FXから振替完了リクエスト」の場合のみ、口座をロックする。<BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParam)
    {
        ...
        if (l_serviceParam[0] instanceof WEB3FXTransFromFXConfirmRequest)
        {
            WEB3FXTransFromFXConfirmRequest l_request =
                (WEB3FXTransFromFXConfirmRequest)l_serviceParam[0];
            l_strFxSystemCode = l_request.fxSystemCode;
        }
        else if (l_serviceParam[0] instanceof WEB3FXTransFromFXCompleteSoapRequest)
        {
            WEB3FXTransFromFXCompleteSoapRequest l_request =
                (WEB3FXTransFromFXCompleteSoapRequest)l_serviceParam[0];
            l_strFxSystemCode = l_request.fxSystemCode;
        }
        else if (l_serviceParam[0] instanceof WEB3FXTransFromFXAskingRequest)
        {
            WEB3FXTransFromFXAskingRequest l_request =
                (WEB3FXTransFromFXAskingRequest)l_serviceParam[0];
            l_strFxSystemCode = l_request.fxSystemCode;
        }
        else if (l_serviceParam[0] instanceof WEB3FXTransFromFXCompleteRequest)
        {
            WEB3FXTransFromFXCompleteRequest l_request =
                (WEB3FXTransFromFXCompleteRequest)l_serviceParam[0];
            l_strFxSystemCode = l_request.fxSystemCode;
        }
        ...
        //２－１）リクエストデータ.FXシステムコード がnullでない場合
        //　@[条件]  
        //　@証券会社コード = OpLoginSecurityServiceより編集 
        //　@部店コード = OpLoginSecurityServiceより編集  
        //　@FXシステムコード = １）で取得したリクエストデータ.FXシステムコード 
        ...
        try
        {
            ...
            //５）リクエストデータの型が「FXから振替完了リクエスト」の場合のみ、口座をロックする。
            //　@　@拡張アカウントマネージャ.lock口座(証券会社コード, 部店コード, 口座コード)をコールする。
            //　@　@※引数はOpLoginSecurityServiceより編集。
            WEB3GentradeAccountManager l_accManager = 
                (WEB3GentradeAccountManager)l_finApp.getAccountManager();
            if (l_serviceParam[0] instanceof WEB3FXTransFromFXCompleteRequest)
            {
