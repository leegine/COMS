...
/** 
 * EqtypeProductConditionRowインタフェースは変更不可でリードオンリーである<em>eqtype_product_condition</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link EqtypeProductConditionRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface EqtypeProductConditionRow extends Row {
  ...
  /** 
   * この{@@link EqtypeProductConditionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
