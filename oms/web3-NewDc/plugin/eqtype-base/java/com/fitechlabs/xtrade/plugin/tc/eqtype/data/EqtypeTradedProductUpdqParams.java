...
1.1
date	2011.03.14.03.37.59;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeTradedProductUpdqParams.java;
...
/** 
 * eqtype_traded_product_updqテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link EqtypeTradedProductUpdqRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link EqtypeTradedProductUpdqParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link EqtypeTradedProductUpdqParams}が{@@link EqtypeTradedProductUpdqRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class EqtypeTradedProductUpdqParams extends Params implements EqtypeTradedProductUpdqRow {
  ...
  public static final RowType TYPE = EqtypeTradedProductUpdqRow.TYPE;
  ...
  /** 
   * 値が未設定のEqtypeTradedProductUpdqParamsオブジェクトを作成します。 
   */
  public EqtypeTradedProductUpdqParams() {
  ...
  /** 
   * 指定のEqtypeTradedProductUpdqRowオブジェクトの値を利用してEqtypeTradedProductUpdqParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するEqtypeTradedProductUpdqRowオブジェクト 
   */
  public EqtypeTradedProductUpdqParams( EqtypeTradedProductUpdqRow p_row )
  {
  ...
  {
    if ( !( other instanceof EqtypeTradedProductUpdqRow ) )
       return false;
    return fieldsEqual( (EqtypeTradedProductUpdqRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のEqtypeTradedProductUpdqRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( EqtypeTradedProductUpdqRow row )
  {
  ...
  {
    return new EqtypeTradedProductUpdqPK(traded_product_id, valid_until_biz_date);
  }
