...
1.1
date	2011.03.14.03.39.01;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeOrderUnitRow.java;
...
/** 
 * EqtypeOrderUnitRowインタフェースは変更不可でリードオンリーである<em>eqtype_order_unit</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link EqtypeOrderUnitRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface EqtypeOrderUnitRow extends Row {


  /** 
   * この{@@link EqtypeOrderUnitRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "eqtype_order_unit", "account" );
