...
1.1
date	2011.03.14.03.39.05;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeOrderExecutionRow.java;
...
/** 
 * EqtypeOrderExecutionRowインタフェースは変更不可でリードオンリーである<em>eqtype_order_execution</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link EqtypeOrderExecutionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface EqtypeOrderExecutionRow extends Row {


  /** 
   * この{@@link EqtypeOrderExecutionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "eqtype_order_execution", "account" );
