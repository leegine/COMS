...
1.1
date	2011.03.14.03.39.18;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeOrderParams.java;
...
/** 
 * eqtype_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link EqtypeOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link EqtypeOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link EqtypeOrderParams}が{@@link EqtypeOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see EqtypeOrderRow 
 */
public class EqtypeOrderParams extends Params implements EqtypeOrderRow {
  ...
  public static final RowType TYPE = EqtypeOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のEqtypeOrderParamsオブジェクトを作成します。 
   */
  public EqtypeOrderParams() {
  ...
  /** 
   * 指定のEqtypeOrderRowオブジェクトの値を利用してEqtypeOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するEqtypeOrderRowオブジェクト 
   */
  public EqtypeOrderParams( EqtypeOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof EqtypeOrderRow ) )
       return false;
    return fieldsEqual( (EqtypeOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のEqtypeOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( EqtypeOrderRow row )
  {
  ...
  {
    return new EqtypeOrderPK(order_id);
  }
