...
1.1
date	2011.03.14.03.38.28;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeTradedProductUpdqRow.java;
...
/** 
 * EqtypeTradedProductUpdqRowインタフェースは変更不可でリードオンリーである<em>eqtype_traded_product_updq</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link EqtypeTradedProductUpdqRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface EqtypeTradedProductUpdqRow extends Row {


  /** 
   * この{@@link EqtypeTradedProductUpdqRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "eqtype_traded_product_updq", "master" );
