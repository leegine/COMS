...
1.1
date	2011.03.14.03.37.47;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeFinTransactionParams.java;
...
/** 
 * eqtype_fin_transactionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link EqtypeFinTransactionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link EqtypeFinTransactionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link EqtypeFinTransactionParams}が{@@link EqtypeFinTransactionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class EqtypeFinTransactionParams extends Params implements EqtypeFinTransactionRow {
  ...
  public static final RowType TYPE = EqtypeFinTransactionRow.TYPE;
  ...
  /** 
   * <em>capital_gain_status</em>カラムの値 
   */
  public  String  capital_gain_status;
  ...
  private boolean capital_gain_status_is_set = false;
  private boolean capital_gain_status_is_modified = false;
  ...
  public String toString() {
    return "{"
      ...
      + "," + "capital_gain_status=" +capital_gain_status
      + "}";
  }


  /** 
   * 値が未設定のEqtypeFinTransactionParamsオブジェクトを作成します。 
   */
  public EqtypeFinTransactionParams() {
  ...
  /** 
   * 指定のEqtypeFinTransactionRowオブジェクトの値を利用してEqtypeFinTransactionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するEqtypeFinTransactionRowオブジェクト 
   */
  public EqtypeFinTransactionParams( EqtypeFinTransactionRow p_row )
  {
    ...
    if ( !p_row.getImportedNameTransferFeeIsNull() )
      imported_name_transfer_fee = new Double( p_row.getImportedNameTransferFee() );
    ...
    if ( !p_row.getImportedNameTransferFeeTaxIsNull() )
      imported_name_transfer_fee_tax = new Double( p_row.getImportedNameTransferFeeTax() );
    ...
    if ( !p_row.getContractIdIsNull() )
      contract_id = new Long( p_row.getContractId() );
    ...
    if ( !p_row.getImportedPayInterestFeeIsNull() )
      imported_pay_interest_fee = new Double( p_row.getImportedPayInterestFee() );
    ...
    if ( !p_row.getImportedPayInterestFeeTaxIsNull() )
      imported_pay_interest_fee_tax = new Double( p_row.getImportedPayInterestFeeTax() );
    ...
    if ( !p_row.getImportedLoanEquityFeeIsNull() )
      imported_loan_equity_fee = new Double( p_row.getImportedLoanEquityFee() );
    ...
    if ( !p_row.getImportedOtherIsNull() )
      imported_other = new Double( p_row.getImportedOther() );
    ...
    capital_gain_status = p_row.getCapitalGainStatus();
    capital_gain_status_is_set = p_row.getCapitalGainStatusIsSet();
    capital_gain_status_is_modified = p_row.getCapitalGainStatusIsModified();
  }
   ...
   */
    public void markAllValuesAsSet() {
      ...
      capital_gain_status_is_set = true;
      capital_gain_status_is_modified = true;
    }
  ...
  {
    if ( !( other instanceof EqtypeFinTransactionRow ) )
       return false;
    return fieldsEqual( (EqtypeFinTransactionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のEqtypeFinTransactionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( EqtypeFinTransactionRow row )
  {
    ...
    if ( imported_setup_fee_tax != row.getImportedSetupFeeTax() )
      return false;
    if ( imported_name_transfer_fee == null ) {
      if ( !row.getImportedNameTransferFeeIsNull() )
        return false;
    } else if ( row.getImportedNameTransferFeeIsNull() || ( imported_name_transfer_fee.doubleValue() != row.getImportedNameTransferFee() ) ) {
        return false;
    }
    if ( imported_name_transfer_fee_tax == null ) {
      if ( !row.getImportedNameTransferFeeTaxIsNull() )
        return false;
    } else if ( row.getImportedNameTransferFeeTaxIsNull() || ( imported_name_transfer_fee_tax.doubleValue() != row.getImportedNameTransferFeeTax() ) ) {
        return false;
    }
    ...
    if ( contract_id == null ) {
      if ( !row.getContractIdIsNull() )
        return false;
    } else if ( row.getContractIdIsNull() || ( contract_id.longValue() != row.getContractId() ) ) {
        return false;
    }
    ...
    if ( imported_interest_fee_tax != row.getImportedInterestFeeTax() )
      return false;
    if ( imported_pay_interest_fee == null ) {
      if ( !row.getImportedPayInterestFeeIsNull() )
        return false;
    } else if ( row.getImportedPayInterestFeeIsNull() || ( imported_pay_interest_fee.doubleValue() != row.getImportedPayInterestFee() ) ) {
        return false;
    }
    if ( imported_pay_interest_fee_tax == null ) {
      if ( !row.getImportedPayInterestFeeTaxIsNull() )
        return false;
    } else if ( row.getImportedPayInterestFeeTaxIsNull() || ( imported_pay_interest_fee_tax.doubleValue() != row.getImportedPayInterestFeeTax() ) ) {
        return false;
    }
    if ( imported_loan_equity_fee == null ) {
      if ( !row.getImportedLoanEquityFeeIsNull() )
        return false;
    } else if ( row.getImportedLoanEquityFeeIsNull() || ( imported_loan_equity_fee.doubleValue() != row.getImportedLoanEquityFee() ) ) {
        return false;
    }
    if ( imported_other == null ) {
      if ( !row.getImportedOtherIsNull() )
        return false;
    } else if ( row.getImportedOtherIsNull() || ( imported_other.doubleValue() != row.getImportedOther() ) ) {
        return false;
    }
    ...
    if ( capital_gain_status == null ) {
      if ( row.getCapitalGainStatus() != null )
        return false;
    } else if ( !capital_gain_status.equals( row.getCapitalGainStatus() ) ) {
        return false;
    }
    return true;
  }
  ...
  public int hashCode() {
      return  ((int) fin_transaction_id)
        ...
        + (imported_name_transfer_fee!=null? imported_name_transfer_fee.hashCode(): 0) 
        + (imported_name_transfer_fee_tax!=null? imported_name_transfer_fee_tax.hashCode(): 0) 
        + ((int) capital_gain)
        + ((int) capital_gain_tax)
        + (contract_id!=null? contract_id.hashCode(): 0) 
        + ((int) imported_management_fee)
        + ((int) imported_management_fee_tax)
        + ((int) imported_interest_fee)
        + ((int) imported_interest_fee_tax)
        + (imported_pay_interest_fee!=null? imported_pay_interest_fee.hashCode(): 0) 
        + (imported_pay_interest_fee_tax!=null? imported_pay_interest_fee_tax.hashCode(): 0) 
        + (imported_loan_equity_fee!=null? imported_loan_equity_fee.hashCode(): 0) 
        + (imported_other!=null? imported_other.hashCode(): 0) 
        ...
        + (capital_gain_status!=null? capital_gain_status.hashCode(): 0) 
      ;
  }
   ...
   */
	public java.util.Map toInsertMap() {
		...
		if ( imported_name_transfer_fee != null )
			map.put("imported_name_transfer_fee",imported_name_transfer_fee);
		if ( imported_name_transfer_fee_tax != null )
			map.put("imported_name_transfer_fee_tax",imported_name_transfer_fee_tax);
		...
		if ( imported_pay_interest_fee != null )
			map.put("imported_pay_interest_fee",imported_pay_interest_fee);
		if ( imported_pay_interest_fee_tax != null )
			map.put("imported_pay_interest_fee_tax",imported_pay_interest_fee_tax);
		if ( imported_loan_equity_fee != null )
			map.put("imported_loan_equity_fee",imported_loan_equity_fee);
		if ( imported_other != null )
			map.put("imported_other",imported_other);
		...
		if ( capital_gain_status_is_set )
			map.put("capital_gain_status",capital_gain_status);
		return map;
	}
   ...
   */
	public java.util.Map toUpdateMap() {
		...
		if ( last_updated_timestamp_is_modified )
			map.put("last_updated_timestamp",last_updated_timestamp);
		if ( capital_gain_status_is_modified )
			map.put("capital_gain_status",capital_gain_status);
		if (map.size() == 0) {
			if ( account_id_is_set )
				map.put("account_id",new Long(account_id));
  ...
  {
    return ( imported_name_transfer_fee==null? ((double)0): imported_name_transfer_fee.doubleValue() );
  }


  /** 
   * <em>imported_name_transfer_fee</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedNameTransferFeeIsNull()
  {
    return imported_name_transfer_fee == null;
  }
  ...
  {
    return ( imported_name_transfer_fee_tax==null? ((double)0): imported_name_transfer_fee_tax.doubleValue() );
  }


  /** 
   * <em>imported_name_transfer_fee_tax</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedNameTransferFeeTaxIsNull()
  {
    return imported_name_transfer_fee_tax == null;
  }
  ...
  {
    return ( contract_id==null? ((long)0): contract_id.longValue() );
  }


  /** 
   * <em>contract_id</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getContractIdIsNull()
  {
  ...
  {
    return ( imported_pay_interest_fee==null? ((double)0): imported_pay_interest_fee.doubleValue() );
  }


  /** 
   * <em>imported_pay_interest_fee</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedPayInterestFeeIsNull()
  {
    return imported_pay_interest_fee == null;
  }
  ...
  {
    return ( imported_pay_interest_fee_tax==null? ((double)0): imported_pay_interest_fee_tax.doubleValue() );
  }


  /** 
   * <em>imported_pay_interest_fee_tax</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedPayInterestFeeTaxIsNull()
  {
    return imported_pay_interest_fee_tax == null;
  }
  ...
  {
    return ( imported_loan_equity_fee==null? ((double)0): imported_loan_equity_fee.doubleValue() );
  }


  /** 
   * <em>imported_loan_equity_fee</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedLoanEquityFeeIsNull()
  {
    return imported_loan_equity_fee == null;
  }
  ...
  {
    return ( imported_other==null? ((double)0): imported_other.doubleValue() );
  }


  /** 
   * <em>imported_other</em>カラムの値がnullかどうか調べます。
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getImportedOtherIsNull()
  {
    return imported_other == null;
  }
  ...
  /** 
   * <em>capital_gain_status</em>カラムの値を取得します。
   * @@return Stringの値 
   */
  public final String getCapitalGainStatus()
  {
    return capital_gain_status;
  }


  /** 
   * <em>capital_gain_status</em>カラムに値が設定されているかどうか調べます。
   * @@return 値が既に設定されている場合はtrue、そうでなければfalse 
   */
  public final boolean getCapitalGainStatusIsSet() {
    return capital_gain_status_is_set;
  }


  /** 
   * <em>capital_gain_status</em>カラムの値が変更されているかどうか調べます。
   * @@return 値が既に変更されている場合はtrue、そうでなければfalse 
   */
  public final boolean getCapitalGainStatusIsModified() {
    return capital_gain_status_is_modified;
  }
  ...
  {
    return new EqtypeFinTransactionPK(fin_transaction_id);
  }
  ...
  public final void setImportedNameTransferFee( double p_importedNameTransferFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_name_transfer_fee = new Double(p_importedNameTransferFee);
    imported_name_transfer_fee_is_set = true;
    imported_name_transfer_fee_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_name_transfer_fee</em>カラムに値を設定します。 
   */
  public final void setImportedNameTransferFee( Double p_importedNameTransferFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setImportedNameTransferFeeTax( double p_importedNameTransferFeeTax )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_name_transfer_fee_tax = new Double(p_importedNameTransferFeeTax);
    imported_name_transfer_fee_tax_is_set = true;
    imported_name_transfer_fee_tax_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_name_transfer_fee_tax</em>カラムに値を設定します。 
   */
  public final void setImportedNameTransferFeeTax( Double p_importedNameTransferFeeTax )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setContractId( long p_contractId )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    contract_id = new Long(p_contractId);
    contract_id_is_set = true;
    contract_id_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>contract_id</em>カラムに値を設定します。 
   */
  public final void setContractId( Long p_contractId )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setImportedPayInterestFee( double p_importedPayInterestFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_pay_interest_fee = new Double(p_importedPayInterestFee);
    imported_pay_interest_fee_is_set = true;
    imported_pay_interest_fee_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_pay_interest_fee</em>カラムに値を設定します。 
   */
  public final void setImportedPayInterestFee( Double p_importedPayInterestFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setImportedPayInterestFeeTax( double p_importedPayInterestFeeTax )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_pay_interest_fee_tax = new Double(p_importedPayInterestFeeTax);
    imported_pay_interest_fee_tax_is_set = true;
    imported_pay_interest_fee_tax_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_pay_interest_fee_tax</em>カラムに値を設定します。 
   */
  public final void setImportedPayInterestFeeTax( Double p_importedPayInterestFeeTax )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setImportedLoanEquityFee( double p_importedLoanEquityFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_loan_equity_fee = new Double(p_importedLoanEquityFee);
    imported_loan_equity_fee_is_set = true;
    imported_loan_equity_fee_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_loan_equity_fee</em>カラムに値を設定します。 
   */
  public final void setImportedLoanEquityFee( Double p_importedLoanEquityFee )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  public final void setImportedOther( double p_importedOther )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    imported_other = new Double(p_importedOther);
    imported_other_is_set = true;
    imported_other_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>imported_other</em>カラムに値を設定します。 
   */
  public final void setImportedOther( Double p_importedOther )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
  ...
  /** 
   * <em>capital_gain_status</em>カラムの値を設定します。 
   *
   * @@param p_capitalGainStatus <em>capital_gain_status</em>カラムの値をあらわす1桁以下のString値 
   */
  public final void setCapitalGainStatus( String p_capitalGainStatus )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    capital_gain_status = p_capitalGainStatus;
    capital_gain_status_is_set = true;
    capital_gain_status_is_modified = true;
  }
    ...
    public Object getColumn( String name ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'c':
                ...
                else if ( name.equals("capital_gain_status") ) {
                    return this.capital_gain_status;
                }
                break;
            case 'd':
            ...
            case 'i':
                if ( name.equals("imported_paid_value") ) {
                    ...
                    return this.imported_name_transfer_fee;
                }
                else if ( name.equals("imported_name_transfer_fee_tax") ) {
                    return this.imported_name_transfer_fee_tax;
                }
                    ...
                    return this.imported_pay_interest_fee;
                }
                else if ( name.equals("imported_pay_interest_fee_tax") ) {
                    return this.imported_pay_interest_fee_tax;
                }
                else if ( name.equals("imported_loan_equity_fee") ) {
                    return this.imported_loan_equity_fee;
                }
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'c':
                if ( name.equals("commission_fee") ) {
                    ...
                    if ( !(value instanceof String) )
                        throw new IllegalArgumentException( "value for 'capital_gain_status' must be String: '"+value+"' is not." );
                    this.capital_gain_status = (String) value;
                    if (this.capital_gain_status_is_set)
                        this.capital_gain_status_is_modified = true;
                    this.capital_gain_status_is_set = true;
                    return;
                }
                break;
            case 'd':
            ...
            case 'i':
                if ( name.equals("imported_paid_value") ) {
                    ...
                    this.imported_name_transfer_fee = (Double) value;
                    ...
                    this.imported_name_transfer_fee_tax = (Double) value;
                    ...
                    this.imported_pay_interest_fee = (Double) value;
                    ...
                    this.imported_pay_interest_fee_tax = (Double) value;
                    ...
                    this.imported_loan_equity_fee = (Double) value;
                    ...
                    this.imported_other = (Double) value;
