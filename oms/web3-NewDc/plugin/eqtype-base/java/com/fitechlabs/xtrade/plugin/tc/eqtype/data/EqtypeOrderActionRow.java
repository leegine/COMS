...
1.1
date	2011.03.14.03.39.34;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	EqtypeOrderActionRow.java;
...
/** 
 * EqtypeOrderActionRowインタフェースは変更不可でリードオンリーである<em>eqtype_order_action</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link EqtypeOrderActionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface EqtypeOrderActionRow extends Row {


  /** 
   * この{@@link EqtypeOrderActionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "eqtype_order_action", "account" );
