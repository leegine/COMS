...
filename	ForeignCostParams.java;
...
/** 
 * foreign_costテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link ForeignCostRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link ForeignCostParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link ForeignCostParams}が{@@link ForeignCostRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see ForeignCostRow 
 */
public class ForeignCostParams extends Params implements ForeignCostRow {
  ...
  public static final RowType TYPE = ForeignCostRow.TYPE;
  ...
  /** 
   * 値が未設定のForeignCostParamsオブジェクトを作成します。 
   */
  public ForeignCostParams() {
  ...
  /** 
   * 指定のForeignCostRowオブジェクトの値を利用してForeignCostParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するForeignCostRowオブジェクト 
   */
  public ForeignCostParams( ForeignCostRow p_row )
  {
  ...
  {
    if ( !( other instanceof ForeignCostRow ) )
       return false;
    return fieldsEqual( (ForeignCostRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のForeignCostRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( ForeignCostRow row )
  {
  ...
  {
    return new ForeignCostPK(institution_code, market_code, cost_div, appli_start_date, amount_from, side_div);
  }
