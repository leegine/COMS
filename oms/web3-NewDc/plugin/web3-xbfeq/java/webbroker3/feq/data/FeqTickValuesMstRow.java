...
filename	FeqTickValuesMstRow.java;
...
/** 
 * FeqTickValuesMstRowインタフェースは変更不可でリードオンリーである<em>feq_tick_values_mst</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link FeqTickValuesMstRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface FeqTickValuesMstRow extends Row {


  /** 
   * この{@@link FeqTickValuesMstRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "feq_tick_values_mst", "master" );
