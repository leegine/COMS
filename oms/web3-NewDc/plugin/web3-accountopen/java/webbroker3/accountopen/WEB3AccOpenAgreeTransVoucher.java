...
1.1
date	2011.03.14.03.28.04;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3AccOpenAgreeTransVoucher.java;
...
File Name           : 保振同意伝票(WEB3AccOpenAgreeTransVoucher.java)
...
import webbroker3.accountopen.data.HostAgreeTransVoucherParams;
...
import webbroker3.common.define.WEB3InquiryReportDivDef;
import webbroker3.common.define.WEB3OrgConsignDivDef;
import webbroker3.common.define.WEB3StatusDef;
import webbroker3.common.define.WEB3StockholderReportDivDef;
 ...
 * @@version 1.0 
 */
public class WEB3AccOpenAgreeTransVoucher extends WEB3AccOpenVoucher
{
    ...
    private static WEB3LogUtility log =
            WEB3LogUtility.getInstance(WEB3AccOpenAgreeTransVoucher.class);
            
    /**
     * @@roseuid 41B45E6C007D
     */
    public WEB3AccOpenAgreeTransVoucher()
    {
    ...
    /**
     * (getInstance)<BR>
     * 口座開設見込客オブジェクトを指定し、保振同意伝票インスタンスを作成する。<BR>
     * <BR>
     * １）　@インスタンス生成<BR>
     * 　@保振同意伝票インスタンスを生成する。<BR>
     * <BR>
     ...
     *
     * @@return webbroker3.accountopen.WEB3AccOpenAgreeTransVoucher
     * @@roseuid 41930EAF0362
     */
    public static WEB3AccOpenAgreeTransVoucher getInstance(WEB3AccOpenExpAccountOpen l_accOpenExpAccountOpen) throws WEB3BaseException
    {
        final String STR_METHOD_NAME = " getInstance(WEB3AccOpenExpAccountOpen)";
        log.entering(STR_METHOD_NAME);
        //１）　@インスタンス生成
        WEB3AccOpenAgreeTransVoucher l_accOpenAgreeTransVoucher = new WEB3AccOpenAgreeTransVoucher();
        
        //２）　@口座開設見込客プロパティセット
        l_accOpenAgreeTransVoucher.setAccOpenExpAccountOpen(l_accOpenExpAccountOpen);
        
        //３）　@口座開設伝票マスタプロパティセット
        l_accOpenAgreeTransVoucher.setAccOpenVoucherMaster();
        
        log.exiting(STR_METHOD_NAME);
        return l_accOpenAgreeTransVoucher;
    }
     ...
     * （getデータコード()の実装）<BR>
     * <BR>
     * データコード.保振同意（GI822）を返却する。<BR>
     * @@return String
     * @@roseuid 41930EAF0373
     */
    public String getRequestCode()
    {
        ...
        log.exiting(STR_METHOD_NAME);
        return WEB3HostRequestCodeDef.ACCOPEN_AGREE_TRANSFER;
    }
    ...
    public String getVoucherCode()
    {
        ...
        log.exiting(STR_METHOD_NAME);
        return WEB3AccountOpenVoucherCodeDef.AGREE_TRANS_VOUCHER_CODE;
    }
     ...
     * <BR>
     * 　@　@作成可能でない場合（is作成可能伝票() == false）のみ、<BR>
     * ２－２）を実施する。<BR>
     ...
     * 　@　@保振同意伝票（GA300）キューテーブルの各項目について、<BR>
     * 下記の処理を実施する。<BR>
     * <BR>
     ...
     * 　@　@伝票通番：　@口座開設伝票マスタ行[index].伝票通番<BR>
     * 　@　@伝票出力項目物理名：　@（※1 保振同意伝票（GA300）キューテーブルの<BR>
     * 処理対象項目）<BR>
     * 　@　@伝票参照項目初期値：　@（※2 保振同意伝票（GA300）キューテーブルの<BR>
     * 処理対象項目デフォルト設定値）<BR>
     * <BR>
     * 　@　@（※2）　@保振同意伝票（GA300）キューテーブルの処理対象項目<BR>
     * デフォルト設定値<BR>
     * 　@　@DBレイアウト 「保振同意伝票（GA300）キューテーブル.xls#<BR>
     * デフォルトDB設定論理」シートを参照し、<BR>
     * 　@　@該当項目の説明欄に、口座開設見込客テーブルから編集する指定<BR>
     * があれば、指定項目の列物理名配列。<BR>
     * 　@　@以外は、null。<BR>
     * <BR>
    ...
    public String[] getConfirmedItemName() throws WEB3BaseException
    {
        ...
        for (int i = 0; i < l_intLength; i++)
        {
            ...
            {                
                ...
                //機@構委託
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.ORG_CONSIGN_DIV, null);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
                ...
                //株主報告
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.STOCKHOLDER_REPORT_DIV, null);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
                ...
                //照会報告
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.INQUIRY_REPORT_DIV, null);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
     ...
     * ３）　@DB更新<BR>
     * 　@３－１）　@既存行削除<BR>
     * 　@　@以下の条件にて保振同意伝票（GA300）キューテーブルを検索する。<BR>
    ...
    public void saveVoucherRow(String l_strSerialNo) throws WEB3BaseException
    {
        ...
        log.entering(STR_METHOD_NAME);
        //デフォルト設定行生成 
        HostAgreeTransVoucherParams l_hostAgreeTransVoucherParams = new HostAgreeTransVoucherParams();
        ...
        {             
            ...
            //データコード
            l_hostAgreeTransVoucherParams.setRequestCode(WEB3HostRequestCodeDef.ACCOPEN_AGREE_TRANSFER);
            //証券会社コード
            l_hostAgreeTransVoucherParams.setInstitutionCode(super.getStringByByteNumber(l_strInstitutionCode, 3));
            //部店コード                    
            l_hostAgreeTransVoucherParams.setBranchCode(super.getStringByByteNumber(l_strBranchCode, 3));
            //顧客コード
            l_hostAgreeTransVoucherParams.setAccountCode(super.getStringByByteNumber(l_strAccountCode, 7));
            ...
            l_hostAgreeTransVoucherParams.setTraderCode(super.getStringByByteNumber(l_strSonarTraderCode, 5));
            
            //識別コード（口座開設見込客）
            l_hostAgreeTransVoucherParams.setAccOpenRequestNumber(super.getStringByByteNumber(l_strRequestNumber, 13));
            //伝票通番
            l_hostAgreeTransVoucherParams.setSerialNo(super.getStringByByteNumber(l_strSerialNo, 3));
            //機@構委託
            l_hostAgreeTransVoucherParams.setOrgConsignDiv(WEB3OrgConsignDivDef.AGREE);
            //株主報告
            l_hostAgreeTransVoucherParams.setStockholderReportDiv(WEB3StockholderReportDivDef.AGREE);
            //照会報告
            l_hostAgreeTransVoucherParams.setInquiryReportDiv(WEB3InquiryReportDivDef.AGREE);
            
            //処理区分
            l_hostAgreeTransVoucherParams.setStatus(WEB3StatusDef.NOT_DEAL);
            Timestamp l_tsSystemTimestamp = GtlUtils.getTradingSystem().getSystemTimestamp();
            //送信日時
            l_hostAgreeTransVoucherParams.setSendTimestamp(null);
            //作成日時
            l_hostAgreeTransVoucherParams.setCreatedTimestamp(l_tsSystemTimestamp);
            //更新日時
            l_hostAgreeTransVoucherParams.setLastUpdatedTimestamp(l_tsSystemTimestamp);
            ...
            if (l_intSize > 0)
            {
                ...
                for (int i = 0; i < l_intSize; i++)
                {
                    ...
                    {
                        l_hostAgreeTransVoucherParams.setRequestCode(super.getStringByByteNumber(l_strValue, 5));   
                    }
                    ...
                    if (WEB3AccountOpenOutputItemSymbolNameDef.INSTITUTION_CODE.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        ...
                        l_hostAgreeTransVoucherParams.setInstitutionCode(super.getStringByByteNumber(l_strValue, 3));     
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setBranchCode(super.getStringByByteNumber(l_strValue, 3));   
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setAccountCode(super.getStringByByteNumber(l_strValue, 7));       
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setTraderCode(super.getStringByByteNumber(l_strValue, 5));     
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる                        
                        l_hostAgreeTransVoucherParams.setAccOpenRequestNumber(super.getStringByByteNumber(l_strValue, 13));    
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる                        
                        l_hostAgreeTransVoucherParams.setSerialNo(super.getStringByByteNumber(l_strValue, 3));      
                    }
                        
                    //機@構委託
                    if (WEB3AccountOpenOutputItemSymbolNameDef.ORG_CONSIGN_DIV.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setOrgConsignDiv(super.getStringByByteNumber(l_strValue, 1));      
                    }
                        
                    //株主報告
                    if (WEB3AccountOpenOutputItemSymbolNameDef.STOCKHOLDER_REPORT_DIV.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setStockholderReportDiv(super.getStringByByteNumber(l_strValue, 1));      
                    }
                        
                    //照会報告
                    if (WEB3AccountOpenOutputItemSymbolNameDef.INQUIRY_REPORT_DIV.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる 
                        l_hostAgreeTransVoucherParams.setInquiryReportDiv(super.getStringByByteNumber(l_strValue, 1));    
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostAgreeTransVoucherParams.setStatus(super.getStringByByteNumber(l_strValue, 1));     
                    }    
                }
            }
            ...
            l_hostAgreeTransVoucherParams.setOrderRequestNumber(l_strNewNumber);
            ...
            //３－２）　@伝票行挿入 
            l_queryProcesser.doInsertQuery(l_hostAgreeTransVoucherParams);  
        }    
     ...
     * 口座開設伝票を１件削除する。<BR>
     * <BR>
     * 保振同意伝票（GA300）キューテーブルの以下の条件に当てはまる行を取得する。<BR>
     * <BR>
     * 　@[条件]<BR>
     * 　@証券会社コード =  this.get証券会社コード() And<BR>
     * 　@識別コード = this.get識別コード() And<BR>
