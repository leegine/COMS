...
File Name           : 契約書徴収伝票(WEB3AccOpenContMrgVoucher.java)
...
public class WEB3AccOpenContMrgVoucher extends WEB3AccOpenVoucher
{
    ...
    /**
     * (getInstance)<BR>
     * 口座開設見込客オブジェクトを指定し、契約書徴収伝票インスタンスを作成する。<BR>
     * <BR>
     * １）　@インスタンス生成<BR>
     * 　@契約書徴収伝票インスタンスを生成する。<BR>
     * <BR>
     ...
     * 　@　@契約書徴収伝票（G1151）キューテーブルの各項目について、<BR>
     * 下記の処理を実施する。<BR>
     * <BR>
     ...
     * 　@　@伝票通番：　@口座開設伝票マスタ行[index].伝票通番<BR>
     * 　@　@伝票出力項目物理名：　@（※1 契約書徴収伝票（G1151）キューテーブルの<BR>
     * 処理対象項目）<BR>
     * 　@　@伝票参照項目初期値：　@（※2 契約書徴収伝票（G1151）キューテーブルの<BR>
     * 処理対象項目デフォルト設定値）<BR>
     * <BR>
     * 　@　@（※2）　@契約書徴収伝票（G1151）キューテーブルの処理対象項目<BR>
     * デフォルト設定値<BR>
     * 　@　@DBレイアウト 「契約書徴収伝票（G1151）キューテーブル.xls#デフォルトDB設定論理」<BR>
     * シートを参照し、<BR>
    ...
    public String[] getConfirmedItemName() throws WEB3BaseException
    {
        ...
        log.debug("this.口座開設伝票マスタ行[]の各要素毎に、２－１）～２－３）の処理を実施する");
        int l_intLength = 0;
     ...
     * <BR>
     * １）　@デフォルト設定行生成<BR>
     * 　@契約書徴収伝票（G1151）キューテーブル行オブジェクトを生成し、<BR>
     * 　@デフォルト設定（※1）の通りにプロパティをセットする。<BR>
     * <BR>
     * 　@文字列をセットする場合、出力項目のデータサイズを超えた場合は、<BR>
     * データサイズ以降の文字を切り捨てる。<BR>
     * <BR>
     * 　@（※1）　@契約書徴収伝票（G1151）キューテーブルの処理対象項目デフォルト設定<BR>
     * 　@DBレイアウト
     * 「契約書徴収伝票（G1151）キューテーブル.xls#デフォルトDB設定論理」<BR>
     * シート参照。<BR>
     * <BR>
     * ２）　@カスタマイズ項目セット <BR>
     ...
     * ３）　@DB更新<BR>
     * 　@３－１）　@既存行削除<BR>
     * 　@　@以下の条件にて契約書徴収伝票（G1151）キューテーブルを検索する。<BR>
     ...
     * 口座開設伝票を１件削除する。<BR>
     * <BR>
     * 契約書徴収伝票（G1151）キューテーブルの以下の条件に当てはまる行を取得する。<BR>
     * <BR>
     * 　@[条件]<BR>
     * 　@証券会社コード =  this.get証券会社コード() And<BR>
     * 　@識別コード = this.get識別コード() And<BR>
