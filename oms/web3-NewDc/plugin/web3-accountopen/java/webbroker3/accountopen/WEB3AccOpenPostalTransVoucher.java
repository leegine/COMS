...
1.1
date	2011.03.14.03.29.14;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3AccOpenPostalTransVoucher.java;
...
File Name           : 振替申込（郵貯）伝票(WEB3AccOpenPostalTransVoucher.java)
...
import webbroker3.common.define.WEB3PostalTransCommissionDef;
 ...
 * @@version 1.0 
 */
public class WEB3AccOpenPostalTransVoucher extends WEB3AccOpenVoucher
{
    ...
    private static WEB3LogUtility log =
            WEB3LogUtility.getInstance(WEB3AccOpenPostalTransVoucher.class);
    
    /**
     * @@roseuid 41B45E6E0148
     */
    public WEB3AccOpenPostalTransVoucher()
    {
    ...
    /**
     * (getInstance)<BR>
     * 口座開設見込客オブジェクトを指定し、振替申込（郵貯）伝票インスタンスを<BR>
     * 作成する。<BR>
     * <BR>
     * １）　@インスタンス生成<BR>
     * 　@振替申込（郵貯）伝票インスタンスを生成する。<BR>
     * <BR>
     ...
     *
     * @@return webbroker3.accountopen.WEB3AccOpenPostalTransVoucher
     * @@roseuid 41930C8C0381
     */
    public static WEB3AccOpenPostalTransVoucher getInstance(WEB3AccOpenExpAccountOpen l_accOpenExpAccountOpen) throws WEB3BaseException
    {
        final String STR_METHOD_NAME = " getInstance(WEB3AccOpenExpAccountOpen)";
        log.entering(STR_METHOD_NAME);
        //１）　@インスタンス生成
        WEB3AccOpenPostalTransVoucher l_accOpenPostalTransVoucher = new WEB3AccOpenPostalTransVoucher();
        
        //２）　@口座開設見込客プロパティセット
        l_accOpenPostalTransVoucher.setAccOpenExpAccountOpen(l_accOpenExpAccountOpen);
        
        //３）　@口座開設伝票マスタプロパティセット
        l_accOpenPostalTransVoucher.setAccOpenVoucherMaster();

        log.exiting(STR_METHOD_NAME);
        return l_accOpenPostalTransVoucher;
    }
    ...
    public String getRequestCode()
    {
        ...
        log.exiting(STR_METHOD_NAME);
        return WEB3HostRequestCodeDef.ACCOPEN_TRANSFER_POSTAL;
    }
     ...
     * 　@　@振替申込（郵貯）伝票（G26）キューテーブルの各項目について、<BR>
     * 下記の処理を実施する。<BR>
     * <BR>
     ...
     * 　@　@伝票通番：　@口座開設伝票マスタ行[index].伝票通番<BR>
     * 　@　@伝票出力項目物理名：　@（※1 振替申込（郵貯）伝票（G26）キューテーブルの<BR>
     * 処理対象項目）<BR>
     * 　@　@伝票参照項目初期値：　@（※2 振替申込（郵貯）伝票（G26）キューテーブルの<BR>
     * 処理対象項目デフォルト設定値）<BR>
     * <BR>
     * 　@　@（※2）　@振替申込（郵貯）伝票（G26）キューテーブルの処理対象項目<BR>
     * デフォルト設定値<BR>
     * 　@　@DBレイアウト 「振替申込（郵貯）伝票（G26）キューテーブル.xls#<BR>
    ...
    public String[] getConfirmedItemName() throws WEB3BaseException
    {
        ...
        for (int i = 0; i < l_intLength; i++)
        {
            ...
            {                
                ...
                //２－２）　@カスタマイズ参照項目列物理名取得
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.TRANS_COMMISSION, null);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
                ...
                //通帳記号
                //２－２）　@カスタマイズ参照項目列物理名取得
                l_strValues[0] = WEB3AccountOpenExpAccountOpenSymbolNameDef.POSTAL_SAVE_CODE;
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.POSTAL_SAVE_CODE, l_strValues);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
                ...
                //通帳番号
                //２－２）　@カスタマイズ参照項目列物理名取得
                l_strValues[0] = WEB3AccountOpenExpAccountOpenSymbolNameDef.POSTAL_SAVE_NO;
                l_strCustomizingRefItem = this.getCustomizingRefItem(l_strSerialNo, 
                    WEB3AccountOpenOutputItemSymbolNameDef.POSTAL_SAVE_NO, l_strValues);  
                //２－３）　@伝票使用項目Table（：Hashtable）に追加
                l_int = 0;
                if (l_strCustomizingRefItem != null)
                {
     ...
     * ３）　@DB更新<BR>
     * 　@３－１）　@既存行削除<BR>
     * 　@　@以下の条件にて振替申込（郵貯）伝票（G26）キューテーブルを検索する。<BR>
    ...
    public void saveVoucherRow(String l_strSerialNo) throws WEB3BaseException
    {
        ...
        {             
            ...
            //証券会社コード
            l_hostPostalTransVoucherParams.setInstitutionCode(
                super.getStringByByteNumber(l_strInstitutionCode, 3));
            //部店コード                    
            l_hostPostalTransVoucherParams.setBranchCode(
                super.getStringByByteNumber(l_strBranchCode, 3));
            //顧客コード
            l_hostPostalTransVoucherParams.setAccountCode(
            ...
            l_hostPostalTransVoucherParams.setTraderCode(
                super.getStringByByteNumber(l_strSonarTraderCode, 5));
            
            //識別コード（口座開設見込客）
            l_hostPostalTransVoucherParams.setAccOpenRequestNumber(
                super.getStringByByteNumber(l_strRequestNumber, 13));
            //伝票通番
            l_hostPostalTransVoucherParams.setSerialNo(
                super.getStringByByteNumber(l_strSerialNo, 3));
            //振出範囲
            l_hostPostalTransVoucherParams.setTransferRange(WEB3TransferRangeDef.SALE_TURNOVER);
            //指定商品コード
            l_hostPostalTransVoucherParams.setProductTypeCodeSpec(null);
            
            //指定銘柄コード
            l_hostPostalTransVoucherParams.setProductCodeSpec(null);           
            //登録区分
            l_hostPostalTransVoucherParams.setRegistDiv(WEB3RegDivDef.NEW);          
            //振替区分
            l_hostPostalTransVoucherParams.setTransferDiv(WEB3TransferDivDef.POSTAL_TRANSFER);
            //振替手数料区分
            l_hostPostalTransVoucherParams.setTransCommission(WEB3PostalTransCommissionDef.FREE);
            
            //通帳記号
            l_hostPostalTransVoucherParams.setPostalSaveCode(
                super.getStringByByteNumber(l_expAccountOpenRow.getPostalSaveCode(), 5));
            //通帳番号
            l_hostPostalTransVoucherParams.setPostalSaveNo(
                super.getStringByByteNumber(l_expAccountOpenRow.getPostalSaveNo(), 8));
            //口座名義
            ...
            //処理区分
            l_hostPostalTransVoucherParams.setStatus(WEB3StatusDef.NOT_DEAL);
            ...
            //作成日時
            l_hostPostalTransVoucherParams.setCreatedTimestamp(l_tsSystemTimestamp);
            //更新日時
            l_hostPostalTransVoucherParams.setLastUpdatedTimestamp(l_tsSystemTimestamp);
            ...
            if (l_intSize > 0)
            {
                ...
                for (int i = 0; i < l_intSize; i++)
                {
                    ...
                    {
                        l_hostPostalTransVoucherParams.setRequestCode(super.getStringByByteNumber(l_strValue, 5));
                    }
                    ...
                    if (WEB3AccountOpenOutputItemSymbolNameDef.INSTITUTION_CODE.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        ...
                        l_hostPostalTransVoucherParams.setInstitutionCode(super.getStringByByteNumber(l_strValue, 3));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setBranchCode(super.getStringByByteNumber(l_strValue, 3));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setAccountCode(super.getStringByByteNumber(l_strValue, 7));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setTraderCode(super.getStringByByteNumber(l_strValue, 5));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setAccOpenRequestNumber(super.getStringByByteNumber(l_strValue, 13));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setSerialNo(super.getStringByByteNumber(l_strValue, 3));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setTransferRange(super.getStringByByteNumber(l_strValue, 1));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setProductTypeCodeSpec(super.getStringByByteNumber(l_strValue, 1));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setProductCodeSpec(super.getStringByByteNumber(l_strValue, 9));
                    } 
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setRegistDiv(super.getStringByByteNumber(l_strValue, 1));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setTransferDiv(super.getStringByByteNumber(l_strValue, 1));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setTransCommission(super.getStringByByteNumber(l_strValue, 1));
                    }
            
                    //通帳記号
                    if (WEB3AccountOpenOutputItemSymbolNameDef.POSTAL_SAVE_CODE.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setPostalSaveCode(super.getStringByByteNumber(l_strValue, 5));
                    }

                    //通帳番号
                    if (WEB3AccountOpenOutputItemSymbolNameDef.POSTAL_SAVE_NO.equals(l_accOpenVoucherItemRow.getOutputItemSymbolName()))
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setPostalSaveNo(super.getStringByByteNumber(l_strValue, 8));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setFinAccountName(super.getStringByByteNumber(l_strValue, 19));
                    }
                    ...
                    {
                        //文字列をセットする場合、出力項目のデータサイズを超えた場合は、データサイズ以降の文字を切り捨てる
                        l_hostPostalTransVoucherParams.setStatus(super.getStringByByteNumber(l_strValue, 1));
                    }    
                }
            }
            ...
            l_hostPostalTransVoucherParams.setOrderRequestNumber(l_strNewNumber);
            ...
            //３－２）　@伝票行挿入 
            l_queryProcesser.doInsertQuery(l_hostPostalTransVoucherParams);  
        }    
     ...
     * 口座開設伝票を１件削除する。<BR>
     * <BR>
     * 振替申込（郵貯）伝票（G26）キューテーブルの以下の条件に<BR>
     * 当てはまる行を取得する。<BR>
     * <BR>
     * 　@[条件]<BR>
     * 　@証券会社コード =  this.get証券会社コード() And<BR>
