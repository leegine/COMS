...
1.1
date	2011.03.14.03.16.30;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BatchPreferencesParams.java;
...
/** 
 * batch_preferencesテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link BatchPreferencesRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link BatchPreferencesParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link BatchPreferencesParams}が{@@link BatchPreferencesRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class BatchPreferencesParams extends Params implements BatchPreferencesRow {
  ...
  public static final RowType TYPE = BatchPreferencesRow.TYPE;
  ...
  /** 
   * 値が未設定のBatchPreferencesParamsオブジェクトを作成します。 
   */
  public BatchPreferencesParams() {
  ...
  /** 
   * 指定のBatchPreferencesRowオブジェクトの値を利用してBatchPreferencesParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するBatchPreferencesRowオブジェクト 
   */
  public BatchPreferencesParams( BatchPreferencesRow p_row )
  {
  ...
  {
    if ( !( other instanceof BatchPreferencesRow ) )
       return false;
    return fieldsEqual( (BatchPreferencesRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のBatchPreferencesRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( BatchPreferencesRow row )
  {
  ...
  {
    return new BatchPreferencesPK(institution_code, branch_code, name, name_serial_no);
  }
