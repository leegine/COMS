...
1.1
date	2011.03.14.03.11.23;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostAccOpenAcceptRow.java;
...
/** 
 * HostAccOpenAcceptRowインタフェースは変更不可でリードオンリーである<em>host_acc_open_accept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostAccOpenAcceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostAccOpenAcceptRow extends Row {


  /** 
   * この{@@link HostAccOpenAcceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_acc_open_accept", "session" );
