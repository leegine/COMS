...
1.1
date	2011.03.14.03.18.10;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BatchPreferencesRow.java;
...
/** 
 * BatchPreferencesRowインタフェースは変更不可でリードオンリーである<em>batch_preferences</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link BatchPreferencesRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface BatchPreferencesRow extends Row {


  /** 
   * この{@@link BatchPreferencesRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "batch_preferences", "session" );
