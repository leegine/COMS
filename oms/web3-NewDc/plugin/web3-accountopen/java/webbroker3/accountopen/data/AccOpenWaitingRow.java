...
/** 
 * AccOpenWaitingRowインタフェースは変更不可でリードオンリーである<em>acc_open_waiting</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link AccOpenWaitingRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface AccOpenWaitingRow extends Row {
  ...
  /** 
   * この{@@link AccOpenWaitingRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
