...
1.1
date	2011.03.14.03.12.38;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	AccOpenItemAttributeRow.java;
...
/** 
 * AccOpenItemAttributeRowインタフェースは変更不可でリードオンリーである<em>acc_open_item_attribute</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link AccOpenItemAttributeRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface AccOpenItemAttributeRow extends Row {


  /** 
   * この{@@link AccOpenItemAttributeRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "acc_open_item_attribute", "master" );
