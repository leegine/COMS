...
1.1
date	2011.03.14.03.18.14;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostAccOpenAcceptParams.java;
...
/** 
 * host_acc_open_acceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link HostAccOpenAcceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link HostAccOpenAcceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link HostAccOpenAcceptParams}が{@@link HostAccOpenAcceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class HostAccOpenAcceptParams extends Params implements HostAccOpenAcceptRow {
  ...
  public static final RowType TYPE = HostAccOpenAcceptRow.TYPE;
  ...
  /** 
   * 値が未設定のHostAccOpenAcceptParamsオブジェクトを作成します。 
   */
  public HostAccOpenAcceptParams() {
  ...
  /** 
   * 指定のHostAccOpenAcceptRowオブジェクトの値を利用してHostAccOpenAcceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するHostAccOpenAcceptRowオブジェクト 
   */
  public HostAccOpenAcceptParams( HostAccOpenAcceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof HostAccOpenAcceptRow ) )
       return false;
    return fieldsEqual( (HostAccOpenAcceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のHostAccOpenAcceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( HostAccOpenAcceptRow row )
  {
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !accept_status_is_set )
			throw new IllegalArgumentException("non-nullable field 'accept_status' must be set before inserting.");
		if ( !status_is_set )
  ...
  {
    return new HostAccOpenAcceptPK(rowid);
  }
