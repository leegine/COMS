...
1.1
date	2011.03.14.03.18.48;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	ExpAccountOpenRow.java;
...
/** 
 * ExpAccountOpenRowインタフェースは変更不可でリードオンリーである<em>exp_account_open</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link ExpAccountOpenRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface ExpAccountOpenRow extends Row {


  /** 
   * この{@@link ExpAccountOpenRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "exp_account_open", "session" );
