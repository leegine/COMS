...
1.1
date	2011.03.14.03.21.38;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostRealnameRegVoucherRow.java;
...
/** 
 * HostRealnameRegVoucherRowインタフェースは変更不可でリードオンリーである<em>host_realname_reg_voucher</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostRealnameRegVoucherRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostRealnameRegVoucherRow extends Row {


  /** 
   * この{@@link HostRealnameRegVoucherRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_realname_reg_voucher", "session" );
