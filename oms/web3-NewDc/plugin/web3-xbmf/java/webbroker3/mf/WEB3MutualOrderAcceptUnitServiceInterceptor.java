...
filename	WEB3MutualOrderAcceptUnitServiceInterceptor.java;
...
File Name        : 投資信託注文受付UnitServiceインタセプタクラス(WEB3MutualOrderAcceptUnitServiceInterceptor)
...
/**
 * 投資信託注文受付UnitServiceインタセプタクラス<BR>
 * 
 * @@author 黄建(中訊)
 * @@version 1.0
 */

public class WEB3MutualOrderAcceptUnitServiceInterceptor 
    implements Interceptor
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3MutualOrderAcceptUnitServiceInterceptor.class);
     ...
     * <BR>
     * 　@－証券会社オブジェクトの取得 <BR>
     * 　@　@取得した部店オブジェクト.getInstitution()をコールして証券会社オブジェクトを取得する。 <BR>
     * <BR>
     * 　@－取得した投信注文単位オブジェクト.getDataSourceObject()<BR>
     * をコールし、投信注文単位Paramsを取得する。 <BR>
     * <BR>
     * 　@－拡張投信銘柄オブジェクトを取得する。<BR>
     * 　@　@拡張投信銘柄マネージャ.getProduct()をコールして、Productオブジェクトを取得<BR>
     * 　@　@する。<BR>
     * 　@　@［getProductに渡すパラメタ］<BR>
     * 　@　@　@銘柄ID： 取得した投信注文単位Params.getProductId()の戻り値<BR>
     * 　@　@拡張投信銘柄マネージャ.to銘柄()をコールして、拡張投信銘柄オブジェクトを取得<BR>
     * 　@　@する。<BR>
     * 　@　@［to銘柄に渡すパラメタ]<BR>
     * 　@　@　@Rowオブジェクト： 取得したProductオブジェクト.getDataSourceObject()の戻り値<BR>
     * <BR>
    ...
    public Object onCall(Method l_method, Object[] l_serviceParam)
    {
        ...
        try
        {
            ...
            MutualFundOrderUnitRow l_mutualOrderUnitRow =
                (MutualFundOrderUnitRow) l_mutualOrderUnit.getDataSourceObject();  
  

            //－拡張投信銘柄オブジェクトを取得する。
            //拡張投信銘柄マネージャ.getProduct()をコールして、Productオブジェクトを取得する。
            ...
            WEB3MutualFundProduct l_mutualFundProduct =
                (WEB3MutualFundProduct) l_mutualManager.getProduct(
                    l_mutualOrderUnitRow.getProductId());
            ...
            //  証券会社オブジェクト.getInstitutionCode()
            l_context.setInstitutionCode(
                l_institution.getInstitutionCode());
 
            //取引カレンダコンテキスト.部店コード = 
            //  部店オブジェクト.getBranchCode()
            ...
            //取引カレンダコンテキスト.銘柄コード = 
            //  拡張投信銘柄オブジェクト.getProductCode()
            l_context.setProductCode(
            ...
            l_context.setOrderAcceptProduct(
                WEB3OrderAccProductDef.MUTUAL_FUND);
            ...
            String l_strAccountCode = l_accMgr.getMainAccount(
                    l_mutualOrderUnitRow.getAccountId()).getAccountCode();      
