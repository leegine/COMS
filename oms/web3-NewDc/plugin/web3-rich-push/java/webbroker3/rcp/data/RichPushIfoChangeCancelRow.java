...
/** 
 * RichPushIfoChangeCancelRowインタフェースは変更不可でリードオンリーである<em>rich_push_ifo_change_cancel</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushIfoChangeCancelRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface RichPushIfoChangeCancelRow extends Row {
  ...
  /** 
   * この{@@link RichPushIfoChangeCancelRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
