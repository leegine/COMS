...
1.1
date	2011.03.17.01.25.41;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushEqOrderacceptParams.java;
...
/** 
 * qtp_rich_push_eq_orderacceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushEqOrderacceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushEqOrderacceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushEqOrderacceptParams}が{@@link QtpRichPushEqOrderacceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushEqOrderacceptParams extends Params implements QtpRichPushEqOrderacceptRow {
  ...
  public static final String PTYPE = "qtp_rich_push_eq_orderaccept";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = QtpRichPushEqOrderacceptRow.TYPE;
  ...
  public RowType getRowType() {
    return QtpRichPushEqOrderacceptRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のQtpRichPushEqOrderacceptParamsオブジェクトを作成します。 
   */
  public QtpRichPushEqOrderacceptParams() {
  ...
  /** 
   * 指定のQtpRichPushEqOrderacceptRowオブジェクトの値を利用してQtpRichPushEqOrderacceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushEqOrderacceptRowオブジェクト 
   */
  public QtpRichPushEqOrderacceptParams( QtpRichPushEqOrderacceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushEqOrderacceptRow ) )
       return false;
    return fieldsEqual( (QtpRichPushEqOrderacceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushEqOrderacceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushEqOrderacceptRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushEqOrderacceptPK(serlnum);
  }
