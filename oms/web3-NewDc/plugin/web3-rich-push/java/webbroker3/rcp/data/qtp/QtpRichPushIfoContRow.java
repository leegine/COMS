...
1.1
date	2011.03.17.01.28.05;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoContRow.java;
...
/** 
 * QtpRichPushIfoContRowインタフェースは変更不可でリードオンリーである<em>qtp_rich_push_ifo_cont</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link QtpRichPushIfoContRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface QtpRichPushIfoContRow extends Row {


  /** 
   * この{@@link QtpRichPushIfoContRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "qtp_rich_push_ifo_cont", "session" );
