...
1.1
date	2011.03.17.01.27.23;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushEquityLapseParams.java;
...
/** 
 * qtp_rich_push_equity_lapseテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushEquityLapseRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushEquityLapseParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushEquityLapseParams}が{@@link QtpRichPushEquityLapseRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushEquityLapseParams extends Params implements QtpRichPushEquityLapseRow {
  ...
  public static final RowType TYPE = QtpRichPushEquityLapseRow.TYPE;
  ...
  /** 
   * 値が未設定のQtpRichPushEquityLapseParamsオブジェクトを作成します。 
   */
  public QtpRichPushEquityLapseParams() {
  ...
  /** 
   * 指定のQtpRichPushEquityLapseRowオブジェクトの値を利用してQtpRichPushEquityLapseParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushEquityLapseRowオブジェクト 
   */
  public QtpRichPushEquityLapseParams( QtpRichPushEquityLapseRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushEquityLapseRow ) )
       return false;
    return fieldsEqual( (QtpRichPushEquityLapseRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushEquityLapseRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushEquityLapseRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushEquityLapsePK(serlnum);
  }
