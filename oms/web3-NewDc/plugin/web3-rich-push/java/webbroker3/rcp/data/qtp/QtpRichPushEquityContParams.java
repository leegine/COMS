...
1.1
date	2011.03.17.01.26.15;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushEquityContParams.java;
...
/** 
 * qtp_rich_push_equity_contテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushEquityContRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushEquityContParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushEquityContParams}が{@@link QtpRichPushEquityContRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushEquityContParams extends Params implements QtpRichPushEquityContRow {
  ...
  public static final String PTYPE = "qtp_rich_push_equity_cont";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = QtpRichPushEquityContRow.TYPE;
  ...
  public RowType getRowType() {
    return QtpRichPushEquityContRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のQtpRichPushEquityContParamsオブジェクトを作成します。 
   */
  public QtpRichPushEquityContParams() {
  ...
  /** 
   * 指定のQtpRichPushEquityContRowオブジェクトの値を利用してQtpRichPushEquityContParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushEquityContRowオブジェクト 
   */
  public QtpRichPushEquityContParams( QtpRichPushEquityContRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushEquityContRow ) )
       return false;
    return fieldsEqual( (QtpRichPushEquityContRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushEquityContRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushEquityContRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushEquityContPK(serlnum);
  }
