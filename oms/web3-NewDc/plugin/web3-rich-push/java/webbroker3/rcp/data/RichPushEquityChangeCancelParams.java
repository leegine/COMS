...
1.1
date	2011.03.17.01.30.42;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushEquityChangeCancelParams.java;
...
/** 
 * rich_push_equity_change_cancelテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushEquityChangeCancelRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushEquityChangeCancelParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushEquityChangeCancelParams}が{@@link RichPushEquityChangeCancelRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushEquityChangeCancelParams extends Params implements RichPushEquityChangeCancelRow {
  ...
  public static final RowType TYPE = RichPushEquityChangeCancelRow.TYPE;
  ...
  /** 
   * 値が未設定のRichPushEquityChangeCancelParamsオブジェクトを作成します。 
   */
  public RichPushEquityChangeCancelParams() {
  ...
  /** 
   * 指定のRichPushEquityChangeCancelRowオブジェクトの値を利用してRichPushEquityChangeCancelParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushEquityChangeCancelRowオブジェクト 
   */
  public RichPushEquityChangeCancelParams( RichPushEquityChangeCancelRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushEquityChangeCancelRow ) )
       return false;
    return fieldsEqual( (RichPushEquityChangeCancelRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushEquityChangeCancelRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushEquityChangeCancelRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new RichPushEquityChangeCancelPK(serlnum);
  }
