...
1.1
date	2011.03.17.01.30.30;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushIfoOrderAcceptParams.java;
...
/** 
 * rich_push_ifo_order_acceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushIfoOrderAcceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushIfoOrderAcceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushIfoOrderAcceptParams}が{@@link RichPushIfoOrderAcceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushIfoOrderAcceptParams extends Params implements RichPushIfoOrderAcceptRow {
  ...
  public static final RowType TYPE = RichPushIfoOrderAcceptRow.TYPE;
  ...
  /** 
   * 値が未設定のRichPushIfoOrderAcceptParamsオブジェクトを作成します。 
   */
  public RichPushIfoOrderAcceptParams() {
  ...
  /** 
   * 指定のRichPushIfoOrderAcceptRowオブジェクトの値を利用してRichPushIfoOrderAcceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushIfoOrderAcceptRowオブジェクト 
   */
  public RichPushIfoOrderAcceptParams( RichPushIfoOrderAcceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushIfoOrderAcceptRow ) )
       return false;
    return fieldsEqual( (RichPushIfoOrderAcceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushIfoOrderAcceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushIfoOrderAcceptRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new RichPushIfoOrderAcceptPK(serlnum);
  }
