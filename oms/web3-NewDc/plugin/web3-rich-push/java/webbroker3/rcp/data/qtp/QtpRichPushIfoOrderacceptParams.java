...
1.1
date	2011.03.17.01.26.58;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoOrderacceptParams.java;
...
/** 
 * qtp_rich_push_ifo_orderacceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushIfoOrderacceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushIfoOrderacceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushIfoOrderacceptParams}が{@@link QtpRichPushIfoOrderacceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushIfoOrderacceptParams extends Params implements QtpRichPushIfoOrderacceptRow {
  ...
  public static final RowType TYPE = QtpRichPushIfoOrderacceptRow.TYPE;
  ...
  /** 
   * 値が未設定のQtpRichPushIfoOrderacceptParamsオブジェクトを作成します。 
   */
  public QtpRichPushIfoOrderacceptParams() {
  ...
  /** 
   * 指定のQtpRichPushIfoOrderacceptRowオブジェクトの値を利用してQtpRichPushIfoOrderacceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushIfoOrderacceptRowオブジェクト 
   */
  public QtpRichPushIfoOrderacceptParams( QtpRichPushIfoOrderacceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushIfoOrderacceptRow ) )
       return false;
    return fieldsEqual( (QtpRichPushIfoOrderacceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushIfoOrderacceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushIfoOrderacceptRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushIfoOrderacceptPK(serlnum);
  }
