...
1.1
date	2011.03.17.01.31.04;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushIfoOrderAcceptRow.java;
...
/** 
 * RichPushIfoOrderAcceptRowインタフェースは変更不可でリードオンリーである<em>rich_push_ifo_order_accept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushIfoOrderAcceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushIfoOrderAcceptRow extends Row {


  /** 
   * この{@@link RichPushIfoOrderAcceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_ifo_order_accept", "session" );
