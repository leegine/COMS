...
1.1
date	2011.03.17.01.29.05;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushHistoryParams.java;
...
/** 
 * rich_push_historyテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushHistoryRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushHistoryParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushHistoryParams}が{@@link RichPushHistoryRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushHistoryParams extends Params implements RichPushHistoryRow {
  ...
  public static final String PTYPE = "rich_push_history";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RichPushHistoryRow.TYPE;
  ...
  public RowType getRowType() {
    return RichPushHistoryRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のRichPushHistoryParamsオブジェクトを作成します。 
   */
  public RichPushHistoryParams() {
  ...
  /** 
   * 指定のRichPushHistoryRowオブジェクトの値を利用してRichPushHistoryParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushHistoryRowオブジェクト 
   */
  public RichPushHistoryParams( RichPushHistoryRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushHistoryRow ) )
       return false;
    return fieldsEqual( (RichPushHistoryRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushHistoryRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushHistoryRow row )
  {
  ...
  {
    return new RichPushHistoryPK(serlnum);
  }
