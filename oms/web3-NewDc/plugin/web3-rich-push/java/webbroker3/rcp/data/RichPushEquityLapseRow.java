...
1.1
date	2011.03.17.01.28.52;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushEquityLapseRow.java;
...
/** 
 * RichPushEquityLapseRowインタフェースは変更不可でリードオンリーである<em>rich_push_equity_lapse</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushEquityLapseRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushEquityLapseRow extends Row {


  /** 
   * この{@@link RichPushEquityLapseRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_equity_lapse", "session" );
