...
1.1
date	2011.03.17.01.31.16;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushEquityContParams.java;
...
/** 
 * rich_push_equity_contテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushEquityContRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushEquityContParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushEquityContParams}が{@@link RichPushEquityContRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushEquityContParams extends Params implements RichPushEquityContRow {
  ...
  public static final String PTYPE = "rich_push_equity_cont";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RichPushEquityContRow.TYPE;
  ...
  public RowType getRowType() {
    return RichPushEquityContRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のRichPushEquityContParamsオブジェクトを作成します。 
   */
  public RichPushEquityContParams() {
  ...
  /** 
   * 指定のRichPushEquityContRowオブジェクトの値を利用してRichPushEquityContParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushEquityContRowオブジェクト 
   */
  public RichPushEquityContParams( RichPushEquityContRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushEquityContRow ) )
       return false;
    return fieldsEqual( (RichPushEquityContRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushEquityContRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushEquityContRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new RichPushEquityContPK(serlnum);
  }
