...
1.1
date	2011.03.17.01.31.12;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushEquityLapseParams.java;
...
/** 
 * rich_push_equity_lapseテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushEquityLapseRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushEquityLapseParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushEquityLapseParams}が{@@link RichPushEquityLapseRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushEquityLapseParams extends Params implements RichPushEquityLapseRow {
  ...
  public static final String PTYPE = "rich_push_equity_lapse";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RichPushEquityLapseRow.TYPE;
  ...
  public RowType getRowType() {
    return RichPushEquityLapseRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のRichPushEquityLapseParamsオブジェクトを作成します。 
   */
  public RichPushEquityLapseParams() {
  ...
  /** 
   * 指定のRichPushEquityLapseRowオブジェクトの値を利用してRichPushEquityLapseParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushEquityLapseRowオブジェクト 
   */
  public RichPushEquityLapseParams( RichPushEquityLapseRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushEquityLapseRow ) )
       return false;
    return fieldsEqual( (RichPushEquityLapseRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushEquityLapseRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushEquityLapseRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new RichPushEquityLapsePK(serlnum);
  }
