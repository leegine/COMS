...
1.1
date	2011.03.17.01.27.19;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoChangecancelParams.java;
...
/** 
 * qtp_rich_push_ifo_changecancelテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushIfoChangecancelRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushIfoChangecancelParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushIfoChangecancelParams}が{@@link QtpRichPushIfoChangecancelRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushIfoChangecancelParams extends Params implements QtpRichPushIfoChangecancelRow {
  ...
  public static final RowType TYPE = QtpRichPushIfoChangecancelRow.TYPE;
  ...
  /** 
   * 値が未設定のQtpRichPushIfoChangecancelParamsオブジェクトを作成します。 
   */
  public QtpRichPushIfoChangecancelParams() {
  ...
  /** 
   * 指定のQtpRichPushIfoChangecancelRowオブジェクトの値を利用してQtpRichPushIfoChangecancelParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushIfoChangecancelRowオブジェクト 
   */
  public QtpRichPushIfoChangecancelParams( QtpRichPushIfoChangecancelRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushIfoChangecancelRow ) )
       return false;
    return fieldsEqual( (QtpRichPushIfoChangecancelRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushIfoChangecancelRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushIfoChangecancelRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushIfoChangecancelPK(serlnum);
  }
