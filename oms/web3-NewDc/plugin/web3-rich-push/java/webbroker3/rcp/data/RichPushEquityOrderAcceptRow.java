...
1.1
date	2011.03.17.01.29.47;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushEquityOrderAcceptRow.java;
...
/** 
 * RichPushEquityOrderAcceptRowインタフェースは変更不可でリードオンリーである<em>rich_push_equity_order_accept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushEquityOrderAcceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushEquityOrderAcceptRow extends Row {


  /** 
   * この{@@link RichPushEquityOrderAcceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_equity_order_accept", "session" );
