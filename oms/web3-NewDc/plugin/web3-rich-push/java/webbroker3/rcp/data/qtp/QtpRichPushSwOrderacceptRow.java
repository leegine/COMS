...
1.1
date	2011.03.17.01.26.41;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushSwOrderacceptRow.java;
...
/** 
 * QtpRichPushSwOrderacceptRowインタフェースは変更不可でリードオンリーである<em>qtp_rich_push_sw_orderaccept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link QtpRichPushSwOrderacceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface QtpRichPushSwOrderacceptRow extends Row {


  /** 
   * この{@@link QtpRichPushSwOrderacceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "qtp_rich_push_sw_orderaccept", "session" );
