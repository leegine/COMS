...
1.1
date	2011.03.17.01.27.32;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoLapseRow.java;
...
/** 
 * QtpRichPushIfoLapseRowインタフェースは変更不可でリードオンリーである<em>qtp_rich_push_ifo_lapse</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link QtpRichPushIfoLapseRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface QtpRichPushIfoLapseRow extends Row {


  /** 
   * この{@@link QtpRichPushIfoLapseRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "qtp_rich_push_ifo_lapse", "session" );
