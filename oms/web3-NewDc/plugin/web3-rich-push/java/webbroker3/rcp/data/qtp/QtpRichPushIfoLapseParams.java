...
1.1
date	2011.03.17.01.25.50;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoLapseParams.java;
...
/** 
 * qtp_rich_push_ifo_lapseテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushIfoLapseRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushIfoLapseParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushIfoLapseParams}が{@@link QtpRichPushIfoLapseRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushIfoLapseParams extends Params implements QtpRichPushIfoLapseRow {
  ...
  public static final RowType TYPE = QtpRichPushIfoLapseRow.TYPE;
  ...
  /** 
   * 値が未設定のQtpRichPushIfoLapseParamsオブジェクトを作成します。 
   */
  public QtpRichPushIfoLapseParams() {
  ...
  /** 
   * 指定のQtpRichPushIfoLapseRowオブジェクトの値を利用してQtpRichPushIfoLapseParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushIfoLapseRowオブジェクト 
   */
  public QtpRichPushIfoLapseParams( QtpRichPushIfoLapseRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushIfoLapseRow ) )
       return false;
    return fieldsEqual( (QtpRichPushIfoLapseRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushIfoLapseRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushIfoLapseRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushIfoLapsePK(serlnum);
  }
