...
1.1
date	2011.03.17.01.30.13;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushIfoChangeCancelParams.java;
...
/** 
 * rich_push_ifo_change_cancelテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RichPushIfoChangeCancelRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RichPushIfoChangeCancelParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RichPushIfoChangeCancelParams}が{@@link RichPushIfoChangeCancelRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RichPushIfoChangeCancelParams extends Params implements RichPushIfoChangeCancelRow {
  ...
  public static final String PTYPE = "rich_push_ifo_change_cancel";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RichPushIfoChangeCancelRow.TYPE;
  ...
  public RowType getRowType() {
    return RichPushIfoChangeCancelRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のRichPushIfoChangeCancelParamsオブジェクトを作成します。 
   */
  public RichPushIfoChangeCancelParams() {
  ...
  /** 
   * 指定のRichPushIfoChangeCancelRowオブジェクトの値を利用してRichPushIfoChangeCancelParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRichPushIfoChangeCancelRowオブジェクト 
   */
  public RichPushIfoChangeCancelParams( RichPushIfoChangeCancelRow p_row )
  {
  ...
  {
    if ( !( other instanceof RichPushIfoChangeCancelRow ) )
       return false;
    return fieldsEqual( (RichPushIfoChangeCancelRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRichPushIfoChangeCancelRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RichPushIfoChangeCancelRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new RichPushIfoChangeCancelPK(serlnum);
  }
