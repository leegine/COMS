...
1.1
date	2011.03.17.01.28.10;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoContParams.java;
...
/** 
 * qtp_rich_push_ifo_contテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushIfoContRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushIfoContParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushIfoContParams}が{@@link QtpRichPushIfoContRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushIfoContParams extends Params implements QtpRichPushIfoContRow {
  ...
  public static final String PTYPE = "qtp_rich_push_ifo_cont";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = QtpRichPushIfoContRow.TYPE;
  ...
  public RowType getRowType() {
    return QtpRichPushIfoContRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のQtpRichPushIfoContParamsオブジェクトを作成します。 
   */
  public QtpRichPushIfoContParams() {
  ...
  /** 
   * 指定のQtpRichPushIfoContRowオブジェクトの値を利用してQtpRichPushIfoContParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushIfoContRowオブジェクト 
   */
  public QtpRichPushIfoContParams( QtpRichPushIfoContRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushIfoContRow ) )
       return false;
    return fieldsEqual( (QtpRichPushIfoContRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushIfoContRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushIfoContRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushIfoContPK(serlnum);
  }
