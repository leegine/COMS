...
1.1
date	2011.03.17.01.31.29;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushIfoLapseRow.java;
...
/** 
 * RichPushIfoLapseRowインタフェースは変更不可でリードオンリーである<em>rich_push_ifo_lapse</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushIfoLapseRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushIfoLapseRow extends Row {


  /** 
   * この{@@link RichPushIfoLapseRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_ifo_lapse", "session" );
