...
1.1
date	2011.03.17.01.28.01;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushSwOrderacceptParams.java;
...
/** 
 * qtp_rich_push_sw_orderacceptテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link QtpRichPushSwOrderacceptRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link QtpRichPushSwOrderacceptParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link QtpRichPushSwOrderacceptParams}が{@@link QtpRichPushSwOrderacceptRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class QtpRichPushSwOrderacceptParams extends Params implements QtpRichPushSwOrderacceptRow {
  ...
  public static final RowType TYPE = QtpRichPushSwOrderacceptRow.TYPE;
  ...
  /** 
   * 値が未設定のQtpRichPushSwOrderacceptParamsオブジェクトを作成します。 
   */
  public QtpRichPushSwOrderacceptParams() {
  ...
  /** 
   * 指定のQtpRichPushSwOrderacceptRowオブジェクトの値を利用してQtpRichPushSwOrderacceptParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するQtpRichPushSwOrderacceptRowオブジェクト 
   */
  public QtpRichPushSwOrderacceptParams( QtpRichPushSwOrderacceptRow p_row )
  {
  ...
  {
    if ( !( other instanceof QtpRichPushSwOrderacceptRow ) )
       return false;
    return fieldsEqual( (QtpRichPushSwOrderacceptRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のQtpRichPushSwOrderacceptRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( QtpRichPushSwOrderacceptRow row )
  {
    if ( tpk == null ) {
      if ( row.getTpk() != null )
        return false;
  ...
  {
    return new QtpRichPushSwOrderacceptPK(serlnum);
  }
