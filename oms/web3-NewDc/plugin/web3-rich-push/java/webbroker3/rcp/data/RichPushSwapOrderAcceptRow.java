...
1.1
date	2011.03.17.01.29.18;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushSwapOrderAcceptRow.java;
...
/** 
 * RichPushSwapOrderAcceptRowインタフェースは変更不可でリードオンリーである<em>rich_push_swap_order_accept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushSwapOrderAcceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushSwapOrderAcceptRow extends Row {


  /** 
   * この{@@link RichPushSwapOrderAcceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_swap_order_accept", "session" );
