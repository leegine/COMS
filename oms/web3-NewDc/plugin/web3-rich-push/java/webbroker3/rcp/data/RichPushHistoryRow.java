...
1.1
date	2011.03.17.01.31.21;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RichPushHistoryRow.java;
...
/** 
 * RichPushHistoryRowインタフェースは変更不可でリードオンリーである<em>rich_push_history</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RichPushHistoryRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RichPushHistoryRow extends Row {


  /** 
   * この{@@link RichPushHistoryRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rich_push_history", "session" );
