...
1.1
date	2011.03.17.01.25.07;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QtpRichPushIfoOrderacceptRow.java;
...
/** 
 * QtpRichPushIfoOrderacceptRowインタフェースは変更不可でリードオンリーである<em>qtp_rich_push_ifo_orderaccept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link QtpRichPushIfoOrderacceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface QtpRichPushIfoOrderacceptRow extends Row {


  /** 
   * この{@@link QtpRichPushIfoOrderacceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "qtp_rich_push_ifo_orderaccept", "session" );
