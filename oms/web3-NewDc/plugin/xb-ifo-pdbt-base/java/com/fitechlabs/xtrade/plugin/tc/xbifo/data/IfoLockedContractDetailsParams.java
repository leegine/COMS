...
1.1
date	2011.03.22.09.09.13;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoLockedContractDetailsParams.java;
...
/** 
 * ifo_locked_contract_detailsテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link IfoLockedContractDetailsRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link IfoLockedContractDetailsParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link IfoLockedContractDetailsParams}が{@@link IfoLockedContractDetailsRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class IfoLockedContractDetailsParams extends Params implements IfoLockedContractDetailsRow {
  ...
  public static final RowType TYPE = IfoLockedContractDetailsRow.TYPE;
  ...
  /** 
   * 値が未設定のIfoLockedContractDetailsParamsオブジェクトを作成します。 
   */
  public IfoLockedContractDetailsParams() {
  ...
  /** 
   * 指定のIfoLockedContractDetailsRowオブジェクトの値を利用してIfoLockedContractDetailsParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するIfoLockedContractDetailsRowオブジェクト 
   */
  public IfoLockedContractDetailsParams( IfoLockedContractDetailsRow p_row )
  {
  ...
  {
    if ( !( other instanceof IfoLockedContractDetailsRow ) )
       return false;
    return fieldsEqual( (IfoLockedContractDetailsRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のIfoLockedContractDetailsRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( IfoLockedContractDetailsRow row )
  {
  ...
  {
    return new IfoLockedContractDetailsPK(contract_id);
  }
