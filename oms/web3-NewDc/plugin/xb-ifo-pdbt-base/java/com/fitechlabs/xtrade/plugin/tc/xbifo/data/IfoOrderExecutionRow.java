...
1.1
date	2011.03.22.09.07.40;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoOrderExecutionRow.java;
...
/** 
 * IfoOrderExecutionRowインタフェースは変更不可でリードオンリーである<em>ifo_order_execution</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link IfoOrderExecutionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface IfoOrderExecutionRow extends Row {


  /** 
   * この{@@link IfoOrderExecutionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "ifo_order_execution", "account" );
