...
1.1
date	2011.03.22.09.08.47;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoFinTransactionParams.java;
...
/** 
 * ifo_fin_transactionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link IfoFinTransactionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link IfoFinTransactionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link IfoFinTransactionParams}が{@@link IfoFinTransactionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class IfoFinTransactionParams extends Params implements IfoFinTransactionRow {
  ...
  public static final RowType TYPE = IfoFinTransactionRow.TYPE;
  ...
  /** 
   * 値が未設定のIfoFinTransactionParamsオブジェクトを作成します。 
   */
  public IfoFinTransactionParams() {
  ...
  /** 
   * 指定のIfoFinTransactionRowオブジェクトの値を利用してIfoFinTransactionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するIfoFinTransactionRowオブジェクト 
   */
  public IfoFinTransactionParams( IfoFinTransactionRow p_row )
  {
  ...
  {
    if ( !( other instanceof IfoFinTransactionRow ) )
       return false;
    return fieldsEqual( (IfoFinTransactionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のIfoFinTransactionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( IfoFinTransactionRow row )
  {
  ...
  {
    return new IfoFinTransactionPK(fin_transaction_id);
  }
