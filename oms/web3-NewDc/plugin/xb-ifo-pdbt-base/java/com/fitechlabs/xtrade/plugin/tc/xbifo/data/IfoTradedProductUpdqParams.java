...
1.1
date	2011.03.22.09.09.21;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoTradedProductUpdqParams.java;
...
/** 
 * ifo_traded_product_updqテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link IfoTradedProductUpdqRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link IfoTradedProductUpdqParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link IfoTradedProductUpdqParams}が{@@link IfoTradedProductUpdqRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class IfoTradedProductUpdqParams extends Params implements IfoTradedProductUpdqRow {
  ...
  public static final RowType TYPE = IfoTradedProductUpdqRow.TYPE;
  ...
  /** 
   * 値が未設定のIfoTradedProductUpdqParamsオブジェクトを作成します。 
   */
  public IfoTradedProductUpdqParams() {
  ...
  /** 
   * 指定のIfoTradedProductUpdqRowオブジェクトの値を利用してIfoTradedProductUpdqParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するIfoTradedProductUpdqRowオブジェクト 
   */
  public IfoTradedProductUpdqParams( IfoTradedProductUpdqRow p_row )
  {
    ...
    if ( !p_row.getUnitSizeIsNull() )
      unit_size = new Double( p_row.getUnitSize() );
    ...
    if ( !p_row.getPerOrderMaxUnitsIsNull() )
      per_order_max_units = new Double( p_row.getPerOrderMaxUnits() );
  ...
  {
    if ( !( other instanceof IfoTradedProductUpdqRow ) )
       return false;
    return fieldsEqual( (IfoTradedProductUpdqRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のIfoTradedProductUpdqRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( IfoTradedProductUpdqRow row )
  {
    ...
    if ( product_id != row.getProductId() )
      return false;
    if ( unit_size == null ) {
      if ( !row.getUnitSizeIsNull() )
        return false;
    } else if ( row.getUnitSizeIsNull() || ( unit_size.doubleValue() != row.getUnitSize() ) ) {
        return false;
    }
    if ( unit_margin != row.getUnitMargin() )
      return false;
    if ( per_order_max_units == null ) {
      if ( !row.getPerOrderMaxUnitsIsNull() )
        return false;
    } else if ( row.getPerOrderMaxUnitsIsNull() || ( per_order_max_units.doubleValue() != row.getPerOrderMaxUnits() ) ) {
        return false;
    }
    ...
    if ( indication_price != row.getIndicationPrice() )
      return false;
    if ( last_liquidation_price != row.getLastLiquidationPrice() )
      return false;
    if ( target_spot_price != row.getTargetSpotPrice() )
      return false;
    if ( liquidation_price != row.getLiquidationPrice() )
      return false;
  ...
  public int hashCode() {
      return  ((int) traded_product_id)
        ...
        + ((int) market_id)
        + ((int) product_id)
        + (unit_size!=null? unit_size.hashCode(): 0) 
        + ((int) unit_margin)
        + (per_order_max_units!=null? per_order_max_units.hashCode(): 0) 
        ...
        + ((int) indication_price)
        + ((int) last_liquidation_price)
        + ((int) target_spot_price)
   ...
   */
	protected void assertValidForInsert() throws IllegalArgumentException {
		...
		if ( !list_flag_is_set )
			throw new IllegalArgumentException("non-nullable field 'list_flag' must be set before inserting.");
		if ( !list_date_is_set )
			throw new IllegalArgumentException("non-nullable field 'list_date' must be set before inserting.");
		if ( !unlisted_date_is_set )
			throw new IllegalArgumentException("non-nullable field 'unlisted_date' must be set before inserting.");
		if ( !exercise_stop_is_set )
			throw new IllegalArgumentException("non-nullable field 'exercise_stop' must be set before inserting.");
		if ( !actual_recieve_stop_is_set )
			throw new IllegalArgumentException("non-nullable field 'actual_recieve_stop' must be set before inserting.");
		if ( !actual_delivary_stop_is_set )
			throw new IllegalArgumentException("non-nullable field 'actual_delivary_stop' must be set before inserting.");
		if ( !reserve_stop_is_set )
			throw new IllegalArgumentException("non-nullable field 'reserve_stop' must be set before inserting.");
		if ( !indication_price_is_set )
			throw new IllegalArgumentException("non-nullable field 'indication_price' must be set before inserting.");
		if ( !last_liquidation_price_is_set )
			throw new IllegalArgumentException("non-nullable field 'last_liquidation_price' must be set before inserting.");
		if ( !target_spot_price_is_set )
			throw new IllegalArgumentException("non-nullable field 'target_spot_price' must be set before inserting.");
		if ( !liquidation_price_is_set )
			throw new IllegalArgumentException("non-nullable field 'liquidation_price' must be set before inserting.");
	}
   ...
   */
	public java.util.Map toInsertMap() {
		...
		if ( per_order_max_units != null )
			map.put("per_order_max_units",per_order_max_units);
		if ( order_close_time != null )
		...
		map.put("indication_price",new Double(indication_price));
		map.put("last_liquidation_price",new Double(last_liquidation_price));
		map.put("target_spot_price",new Double(target_spot_price));
		map.put("liquidation_price",new Double(liquidation_price));
  ...
  {
    return ( unit_size==null? ((double)0): unit_size.doubleValue() );
  }
   ...
   */
  public final boolean getUnitSizeIsNull()
  {
    return unit_size == null;
  }
  ...
  {
    return ( per_order_max_units==null? ((double)0): per_order_max_units.doubleValue() );
  }
   ...
   * @@return 対象のデータベースカラムの値がnullの場合はtrue 
   */
  public final boolean getPerOrderMaxUnitsIsNull()
  {
    return per_order_max_units == null;
  }
  ...
  {
    return new IfoTradedProductUpdqPK(traded_product_id, valid_for_biz_date);
  }
  ...
  public final void setUnitSize( double p_unitSize )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    unit_size = new Double(p_unitSize);
    unit_size_is_set = true;
    unit_size_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>unit_size</em>カラムに値を設定します。 
   */
  public final void setUnitSize( Double p_unitSize )  {
    if(!mutable())
  ...
  public final void setPerOrderMaxUnits( double p_perOrderMaxUnits )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object cann't be modified. Use copy contructor to create a new Params object for modification.");
    per_order_max_units = new Double(p_perOrderMaxUnits);
    per_order_max_units_is_set = true;
    per_order_max_units_is_modified = true;
  }
  ...
  /** 
   * 指定のオブジェクト（場合によってはnull）を使って<em>per_order_max_units</em>カラムに値を設定します。 
   */
  public final void setPerOrderMaxUnits( Double p_perOrderMaxUnits )  {
    if(!mutable())
      throw new UnsupportedOperationException("This Params object can't be modified. Use copy contructor to create a new Params object for modification.");
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'i':
                if ( name.equals("institution_code") ) {
                    ...
                    return new Double( indication_price );
                }
                break;
            case 'l':
                if ( name.equals("last_closing_price") ) {
                    ...
                    return new Double( last_liquidation_price );
                }
                else if ( name.equals("liquidation_price") ) {
                    return new Double( liquidation_price );
                }
            ...
            case 'p':
                if ( name.equals("product_id") ) {
                    ...
                    return this.per_order_max_units;
                }
                break;
            case 'r':
            ...
            case 't':
                if ( name.equals("traded_product_id") ) {
                    ...
                    return new Double( target_spot_price );
                }
                break;
            case 'u':
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'i':
                if ( name.equals("institution_code") ) {
                    ...
                    this.indication_price = ((Double) value).doubleValue();
            ...
            case 'l':
                if ( name.equals("last_closing_price") ) {
                    ...
                    this.last_liquidation_price = ((Double) value).doubleValue();
                    ...
                    this.liquidation_price = ((Double) value).doubleValue();
            ...
            case 'p':
                if ( name.equals("product_id") ) {
                    ...
                    this.per_order_max_units = (Double) value;
            ...
            case 't':
                if ( name.equals("traded_product_id") ) {
                    ...
                    this.target_spot_price = ((Double) value).doubleValue();
