...
1.1
date	2011.03.22.09.07.52;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	IfoClosingContractSpecRow.java;
...
/** 
 * IfoClosingContractSpecRowインタフェースは変更不可でリードオンリーである<em>ifo_closing_contract_spec</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link IfoClosingContractSpecRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface IfoClosingContractSpecRow extends Row {


  /** 
   * この{@@link IfoClosingContractSpecRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "ifo_closing_contract_spec", "account" );
