...
1.1
date	2011.03.22.08.01.08;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3AdminPointBranchCodeComparator.java;
...
File Name        : 部店コードComparator(WEB3AdminPointBranchCodeComparator.java)
...
/**
 * (部店コードComparator)<BR>
 * 部店コードComparatorクラス<BR>
 *
 * @@author 鄭海良
 * @@version 1.0
 */
public class WEB3AdminPointBranchCodeComparator implements Comparator 
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3AdminPointBranchCodeComparator.class);        
     ...
     * 　@D：降順<BR>
     * 
     * @@roseuid 419C052502C8
     */
    public WEB3AdminPointBranchCodeComparator(String l_strOrderBy) 
    {
     ...
     * 　@・（ポイント申込明細１.部店コード < ポイント申込明細２.部店コード）の場合、<BR>
     * 負の整数（-1）を返却する。<BR>
     * 　@・（ポイント申込明細１.部店コード = ポイント申込明細２.部店コード）の場合、<BR>
     * 0を返却する。<BR>
     * 　@・（ポイント申込明細１.部店コード > ポイント申込明細２.部店コード）の場合、<BR>
    ...
    public int compare(Object l_pointApplyDetail1, Object l_pointApplyDetail2) 
    {
        ...
        {
            if (l_pointApplyDetail1_alias.branchCode == null && l_pointApplyDetail2_alias.branchCode != null)
            {
            ...
            if (l_pointApplyDetail1_alias.branchCode.compareTo(l_pointApplyDetail2_alias.branchCode) < 0)
            {
                log.exiting(STR_METHOD_NAME);
                return -1;
            }
