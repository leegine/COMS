...
1.1
date	2011.03.22.08.55.45;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
/** 
 * aio_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link AioOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link AioOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link AioOrderParams}が{@@link AioOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see AioOrderRow 
 */
public class AioOrderParams extends Params implements AioOrderRow {
  ...
  public static final String PTYPE = "aio_order";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = AioOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のAioOrderParamsオブジェクトを作成します。 
   */
  public AioOrderParams() {
  ...
  /** 
   * 指定のAioOrderRowオブジェクトの値を利用してAioOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するAioOrderRowオブジェクト 
   */
  public AioOrderParams( AioOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof AioOrderRow ) )
       return false;
    return fieldsEqual( (AioOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のAioOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( AioOrderRow row )
  {
  ...
  {
    return new AioOrderPK(order_id);
  }
