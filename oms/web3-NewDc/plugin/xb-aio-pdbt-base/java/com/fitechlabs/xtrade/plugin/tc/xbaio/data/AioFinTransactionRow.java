...
1.1
date	2011.03.22.08.56.10;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	AioFinTransactionRow.java;
...
/** 
 * AioFinTransactionRowインタフェースは変更不可でリードオンリーである<em>aio_fin_transaction</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link AioFinTransactionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface AioFinTransactionRow extends Row {


  /** 
   * この{@@link AioFinTransactionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "aio_fin_transaction", "account" );
