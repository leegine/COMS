...
File Name        : 入金請求顧客検索一覧リクエスト(WEB3AdminTPPaymentRequisitionListRequest.java)
Author Name      : Daiwa Institute of Research
Revision History : 2008/10/08 孟亞南 (中訊) 新規作成 モデルNo.027
Revision History : 2008/10/17 姜丹 (中訊) モデルNo.039
*/
...
public class WEB3AdminTPPaymentRequisitionListRequest extends WEB3AdminTPPaymentRequisitionCommonRequest
{
     ...
     * <BR>
     * ６）要求ページ番号のチェック<BR>
     * 　@要求ページ番号がnull　@or　@要求ページ番号に半角数字以外の文字がある場合<BR>
     * 　@例外をスローする。<BR>
     * 　@class: WEB3BusinessLayerException<BR>
     * 　@　@tag: BUSINESS_ERROR_00786<BR>
     * <BR>
     * ７）ページ内表示行数のチェック<BR>
     * 　@ページ内表示行数がnull　@or　@ページ内表示行数に半角数字以外の文字がある場合<BR>
     * 　@例外をスローする。<BR>
     * 　@class: WEB3BusinessLayerException<BR>
    ...
    public void validate() throws WEB3BaseException
    {
        ...
        //６）要求ページ番号のチェック
        //　@要求ページ番号がnull　@or　@要求ページ番号に半角数字以外の文字がある場合
        //　@例外をスローする。
        if (this.pageIndex == null
            || !WEB3StringTypeUtility.isDigit(this.pageIndex))
        {
            log.debug("要求ページ番号の入力が不正です。");
            log.exiting(STR_METHOD_NAME);
            throw new WEB3BusinessLayerException(
        ...
        //７）ページ内表示行数のチェック
        //　@ページ内表示行数がnull　@or　@ページ内表示行数に半角数字以外の文字がある場合
        //　@例外をスローする。
