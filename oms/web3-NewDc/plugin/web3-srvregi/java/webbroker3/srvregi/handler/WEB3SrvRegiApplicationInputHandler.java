...
File Name        : サービス利用申込入力ハンドラ(WEB3SrvRegiApplicationInputHandler.java)
...
public class WEB3SrvRegiApplicationInputHandler implements MessageHandler 
{
    ...
    /**
     * (利用申込入力リクエスト)<BR>
     * サービス利用申込入力照会処理を行う。<BR>
     * <BR>
     * サービス利用申込入力サービスを取得し、execute( )メソッドをコールする。<BR>
    ...
    public WEB3SrvRegiApplyInputResponse useAppliInputRequest(WEB3SrvRegiApplyInputRequest l_request) 
    {
        ...
        catch (Exception l_ex)
        {
            ...
            log.error(
                l_request,
                "サービス利用申込入力サービスの取得に失敗しました。",
                l_response.errorInfo,l_ex);
            log.exiting(STR_METHOD_NAME);
            return l_response;      
        }
        
        //サービス利用申込入力サービス.execute()メソッドをコールする
        try
        {
            l_response =
