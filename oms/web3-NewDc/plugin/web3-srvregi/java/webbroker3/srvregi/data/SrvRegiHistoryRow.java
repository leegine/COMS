...
1.1
date	2011.03.14.06.42.13;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	SrvRegiHistoryRow.java;
...
/** 
 * SrvRegiHistoryRowインタフェースは変更不可でリードオンリーである<em>srv_regi_history</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link SrvRegiHistoryRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface SrvRegiHistoryRow extends Row {


  /** 
   * この{@@link SrvRegiHistoryRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "srv_regi_history", "master" );
