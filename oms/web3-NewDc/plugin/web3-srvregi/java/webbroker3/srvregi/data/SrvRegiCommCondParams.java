...
1.1
date	2011.03.14.06.43.36;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	SrvRegiCommCondParams.java;
...
/** 
 * srv_regi_comm_condテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link SrvRegiCommCondRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link SrvRegiCommCondParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link SrvRegiCommCondParams}が{@@link SrvRegiCommCondRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class SrvRegiCommCondParams extends Params implements SrvRegiCommCondRow {
  ...
  public static final RowType TYPE = SrvRegiCommCondRow.TYPE;
  ...
  /** 
   * 値が未設定のSrvRegiCommCondParamsオブジェクトを作成します。 
   */
  public SrvRegiCommCondParams() {
  ...
  /** 
   * 指定のSrvRegiCommCondRowオブジェクトの値を利用してSrvRegiCommCondParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するSrvRegiCommCondRowオブジェクト 
   */
  public SrvRegiCommCondParams( SrvRegiCommCondRow p_row )
  {
  ...
  {
    if ( !( other instanceof SrvRegiCommCondRow ) )
       return false;
    return fieldsEqual( (SrvRegiCommCondRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のSrvRegiCommCondRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( SrvRegiCommCondRow row )
  {
  ...
  {
    return new SrvRegiCommCondPK(institution_code, srv_div, order_acc_product);
  }
