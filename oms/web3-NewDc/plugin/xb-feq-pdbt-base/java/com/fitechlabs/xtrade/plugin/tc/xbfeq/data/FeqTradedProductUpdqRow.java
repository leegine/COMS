...
1.1
date	2011.03.22.09.03.25;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	FeqTradedProductUpdqRow.java;
...
/** 
 * FeqTradedProductUpdqRowインタフェースは変更不可でリードオンリーである<em>feq_traded_product_updq</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link FeqTradedProductUpdqRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface FeqTradedProductUpdqRow extends Row {


  /** 
   * この{@@link FeqTradedProductUpdqRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "feq_traded_product_updq", "master" );
