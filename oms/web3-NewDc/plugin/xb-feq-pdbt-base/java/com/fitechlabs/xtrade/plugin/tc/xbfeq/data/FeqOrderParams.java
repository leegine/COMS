...
1.1
date	2011.03.22.09.02.35;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
/** 
 * feq_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link FeqOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link FeqOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link FeqOrderParams}が{@@link FeqOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see FeqOrderRow 
 */
public class FeqOrderParams extends Params implements FeqOrderRow {
  ...
  public static final RowType TYPE = FeqOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のFeqOrderParamsオブジェクトを作成します。 
   */
  public FeqOrderParams() {
  ...
  /** 
   * 指定のFeqOrderRowオブジェクトの値を利用してFeqOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するFeqOrderRowオブジェクト 
   */
  public FeqOrderParams( FeqOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof FeqOrderRow ) )
       return false;
    return fieldsEqual( (FeqOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のFeqOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( FeqOrderRow row )
  {
  ...
  {
    return new FeqOrderPK(order_id);
  }
