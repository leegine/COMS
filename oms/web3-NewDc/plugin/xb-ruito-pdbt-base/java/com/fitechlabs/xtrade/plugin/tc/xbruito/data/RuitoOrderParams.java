...
1.1
date	2011.03.22.09.18.11;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RuitoOrderParams.java;
...
/** 
 * ruito_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RuitoOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RuitoOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RuitoOrderParams}が{@@link RuitoOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see RuitoOrderRow 
 */
public class RuitoOrderParams extends Params implements RuitoOrderRow {
  ...
  public static final String PTYPE = "ruito_order";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RuitoOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のRuitoOrderParamsオブジェクトを作成します。 
   */
  public RuitoOrderParams() {
  ...
  /** 
   * 指定のRuitoOrderRowオブジェクトの値を利用してRuitoOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRuitoOrderRowオブジェクト 
   */
  public RuitoOrderParams( RuitoOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof RuitoOrderRow ) )
       return false;
    return fieldsEqual( (RuitoOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRuitoOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RuitoOrderRow row )
  {
  ...
  {
    return new RuitoOrderPK(order_id);
  }
