...
1.1
date	2011.03.22.09.18.02;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RuitoTradedProductUpdqRow.java;
...
/** 
 * RuitoTradedProductUpdqRowインタフェースは変更不可でリードオンリーである<em>ruito_traded_product_updq</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RuitoTradedProductUpdqRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RuitoTradedProductUpdqRow extends Row {


  /** 
   * この{@@link RuitoTradedProductUpdqRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "ruito_traded_product_updq", "master" );
