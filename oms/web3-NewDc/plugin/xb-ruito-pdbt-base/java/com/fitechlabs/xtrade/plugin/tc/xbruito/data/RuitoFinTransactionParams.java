...
1.1
date	2011.03.22.09.16.55;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RuitoFinTransactionParams.java;
...
/** 
 * ruito_fin_transactionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RuitoFinTransactionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RuitoFinTransactionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RuitoFinTransactionParams}が{@@link RuitoFinTransactionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RuitoFinTransactionParams extends Params implements RuitoFinTransactionRow {
  ...
  public static final RowType TYPE = RuitoFinTransactionRow.TYPE;
  ...
  /** 
   * 値が未設定のRuitoFinTransactionParamsオブジェクトを作成します。 
   */
  public RuitoFinTransactionParams() {
  ...
  /** 
   * 指定のRuitoFinTransactionRowオブジェクトの値を利用してRuitoFinTransactionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRuitoFinTransactionRowオブジェクト 
   */
  public RuitoFinTransactionParams( RuitoFinTransactionRow p_row )
  {
  ...
  {
    if ( !( other instanceof RuitoFinTransactionRow ) )
       return false;
    return fieldsEqual( (RuitoFinTransactionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRuitoFinTransactionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RuitoFinTransactionRow row )
  {
  ...
  {
    return new RuitoFinTransactionPK(fin_transaction_id);
  }
