...
/** 
 * RuitoOrderUnitRowインタフェースは変更不可でリードオンリーである<em>ruito_order_unit</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RuitoOrderUnitRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface RuitoOrderUnitRow extends Row {
  ...
  /** 
   * この{@@link RuitoOrderUnitRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
