...
1.1
date	2011.03.22.09.16.30;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RuitoOrderExecutionParams.java;
...
/** 
 * ruito_order_executionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RuitoOrderExecutionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RuitoOrderExecutionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RuitoOrderExecutionParams}が{@@link RuitoOrderExecutionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RuitoOrderExecutionParams extends Params implements RuitoOrderExecutionRow {
  ...
  public static final RowType TYPE = RuitoOrderExecutionRow.TYPE;
  ...
  /** 
   * 値が未設定のRuitoOrderExecutionParamsオブジェクトを作成します。 
   */
  public RuitoOrderExecutionParams() {
  ...
  /** 
   * 指定のRuitoOrderExecutionRowオブジェクトの値を利用してRuitoOrderExecutionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRuitoOrderExecutionRowオブジェクト 
   */
  public RuitoOrderExecutionParams( RuitoOrderExecutionRow p_row )
  {
  ...
  {
    if ( !( other instanceof RuitoOrderExecutionRow ) )
       return false;
    return fieldsEqual( (RuitoOrderExecutionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRuitoOrderExecutionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RuitoOrderExecutionRow row )
  {
  ...
  {
    return new RuitoOrderExecutionPK(order_execution_id);
  }
