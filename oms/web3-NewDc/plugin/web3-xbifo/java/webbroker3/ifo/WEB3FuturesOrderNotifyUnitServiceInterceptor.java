...
1.1
date	2011.03.16.03.45.34;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3FuturesOrderNotifyUnitServiceInterceptor.java;
...
File Name        : 先物注文通知UnitServiceインタセプタ(WEB3FuturesOrderNotifyUnitServiceInterceptor.java)
...
/**
 * (先物注文通知UnitServiceインタセプタ)<BR>
 * 株価指数先物注文通知１件サービスインタセプタクラス<BR>
 * <BR>
 * Plugin時に先物注文通知UnitServiceに対して設定する。<BR>
 * @@author  : 凌建平
 * @@version : 1.0
 */
public class WEB3FuturesOrderNotifyUnitServiceInterceptor implements Interceptor 
{
    ...
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3FuturesOrderNotifyUnitServiceInterceptor.class);
            
    /**
     * @@roseuid 41AD588901B5
     */
    public WEB3FuturesOrderNotifyUnitServiceInterceptor() 
    {
     ...
     * （xTradeカーネルより[先物注文通知Unitサービス]実行前に呼び出される）<BR>
     * <BR>
     ...
     * <BR>
     * ３）　@受付日時として、SONAR側での発注日時(=発注日時(biz_datetime))を使用する為、
     *  <BR>
     * 　@　@　@　@取引時間管理.setTimestamp()にて設定された受付日時を上書きする。（※注　@?
     * 参照） <BR>
     * <BR>
     * 　@－ThreadLocalSystemAttributesRegistry.setAttribute( 
