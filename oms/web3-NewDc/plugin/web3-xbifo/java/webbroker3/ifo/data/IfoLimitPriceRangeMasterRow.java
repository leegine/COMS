...
/** 
 * IfoLimitPriceRangeMasterRowインタフェースは変更不可でリードオンリーである<em>ifo_limit_price_range_master</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link IfoLimitPriceRangeMasterRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface IfoLimitPriceRangeMasterRow extends Row {
  ...
  /** 
   * この{@@link IfoLimitPriceRangeMasterRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
