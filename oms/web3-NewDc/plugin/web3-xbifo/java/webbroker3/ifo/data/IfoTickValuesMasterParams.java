...
/** 
 * ifo_tick_values_masterテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link IfoTickValuesMasterRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link IfoTickValuesMasterParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link IfoTickValuesMasterParams}が{@@link IfoTickValuesMasterRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
...
public class IfoTickValuesMasterParams extends Params implements IfoTickValuesMasterRow {
  ...
  /** 
   * 値が未設定のIfoTickValuesMasterParamsオブジェクトを作成します。 
   */
  ...
  /** 
   * 指定のIfoTickValuesMasterRowオブジェクトの値を利用してIfoTickValuesMasterParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するIfoTickValuesMasterRowオブジェクト 
   */
   ...
   * @@param row 比較対照のIfoTickValuesMasterRow 
