...
1.1
date	2011.03.16.03.18.23;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3FuturesExecuteReferenceRequest.java;
...
File Name        : 株価指数先物注文約定照会リクエストクラス(WEB3FuturesExecuteReferenceRequest.java)
Author Name      : Daiwa Institute of Research
Revesion History : 2004/07/21 李強(中訊) 新規作成
Revesion History : 2007/10/16 張騰宇(中訊)モデル 788
*/
...
/**
 * (株価指数先物注文約定照会リクエスト)<BR>
 * 株価指数先物注文約定照会リクエストクラス
 * @@author 李強
 * @@version 1.0
 */
public class WEB3FuturesExecuteReferenceRequest extends WEB3GenRequest 
{
    /**
      * ログ出力ユーティリティ。
      */
     private static WEB3LogUtility log =
         WEB3LogUtility.getInstance(WEB3FuturesExecuteReferenceRequest.class);
    ...
    /**
     * (照会区分)<BR>
     * 0：注文約定照会　@1：訂正取消一覧（訂正取消可能なもののみ表示）<BR>
     */
     ...
     * <BR>
     * 初回は会社ごとに決められた初期値をセット（デフォルトは注文時間の降順）<BR>
     * モバイルでは「注文時間の降順」固定<BR>
     */
    ...
    /**
     * @@roseuid 40F7AE0F029F
     */
    public WEB3FuturesExecuteReferenceRequest() 
    {
     ...
     * 　@　@　@　@繰り返してチェックを行う。<BR>
     * 　@　@３－３－１）株価指数先物オプションソートキー.キー項目 ＝ null の場合、<BR>
     * 例外をスローする。<BR>
     *           class: WEB3BusinessLayerException<BR>
     *           tag:   BUSINESS_ERROR_00085<BR>
     * 　@　@３－３－２）株価指数先物オプションソートキー.キー項目に<BR>
     * 以下の項目名以外の値が存在したら例外をスローする。<BR>
     * 　@　@　@・銘柄コード<BR>
     * 　@　@　@・取引市場<BR>
     * 　@　@　@・取引区分<BR>
     * 　@　@　@・注文時間<BR>
     ...
     * 　@　@　@・注文有効期限<BR>
     *           class: WEB3BusinessLayerException<BR>
     *           tag:   BUSINESS_ERROR_00280<BR>
     * 　@　@３－３－３）株価指数先物オプションソートキー.昇順／降順 ＝ null <BR>
     * の場合、例外をスローする。<BR>
     *           class: WEB3BusinessLayerException<BR>
     *           tag:   BUSINESS_ERROR_00318<BR>
     * 　@　@３－３－４）株価指数先物オプションソートキー.昇順／降順が<BR>
    ...
    public void validate() throws WEB3BaseException 
    {
        ...
        //      class: WEB3BusinessLayerException<BR>
        //      tag:   BUSINESS_ERROR_00082<BR>
        if (!WEB3IfoReferenceTypeDef.ORDER_PROMISE.equals(this.referenceType) && !WEB3IfoReferenceTypeDef.CORRECTION_LIST.equals(this.referenceType))
        {
            throw new WEB3BusinessLayerException(
        ...
        for (int i = 0; i < l_intOpSortKeysLength; i++)
        {
            ...
            //          class: WEB3BusinessLayerException<BR>
            //          tag:   BUSINESS_ERROR_00280<BR>          
            if (!WEB3IfoKeyItemDef.FUTPRODUCTCODE.equals(this.futOpSortKeys[i].keyItem)
        ...
        // ６－２）銘柄選択時に取引市場,指数種別,限月のいずれかの項目が              
        // 設定されていない場合、例外をスローする。             
        if((marketCode==null)
            &&(targetProductCode==null)
            &&(delivaryMonth==null))
     ...
     * @@roseuid 40F7AE0F02CE
     */
    public WEB3GenResponse createResponse() 
    {
        return new WEB3FuturesExecuteReferenceResponse(this);
    }
}
@
