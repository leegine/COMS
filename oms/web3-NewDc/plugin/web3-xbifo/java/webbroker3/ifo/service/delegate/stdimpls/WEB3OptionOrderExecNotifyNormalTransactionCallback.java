...
filename	WEB3OptionOrderExecNotifyNormalTransactionCallback.java;
...
File Name        : 株価指数オプション出来通知正常処理一件TransactionCallback(WEB3OptionOrderExecNotifyNormalTransactionCallback.java)
...
/**
 * （株価指数オプション出来通知正常処理一件TransactionCallback）。<BR>
 * <BR>
 * トランザクション処理を実施する内部クラス。<BR>
 * @@author 李志強
 * @@version 1.0
 */
public class WEB3OptionOrderExecNotifyNormalTransactionCallback implements TransactionCallback
{
    /**
     * ログ出力ユーティリティ。<BR>
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3OptionOrderExecNotifyNormalTransactionCallback.class);
     ...
     */
    public WEB3OptionOrderExecNotifyNormalTransactionCallback(
        ...
        throws DataQueryException, DataNetworkException, DataCallbackException
    {
        ...
        try
        {
            String l_strInstitutionCode = hostOptionOrderExecNotifyRow.getInstitutionCode();
            String l_strBranchCode = hostOptionOrderExecNotifyRow.getBranchCode();

            //1.2 get注文単位(String, String, ProductTypeEnum, String)
            //  [引数]
            //  証券会社コード：先物OP訂正取消通知キューテーブル.証券会社コード
            //  部店コード    ：先物OP訂正取消通知キューテーブル.部店コード
            //  商品タイプ    ：”先物オプション”
            //  識別コード    ：先物OP訂正取消通知キューテーブル.識別コード
            ...
            catch (WEB3BaseException l_ex)
            {
                if (l_ex instanceof WEB3SystemLayerException)
                {
                        ...
                        l_wre.getException() == null)
                    {
                        ...
                        Date l_datAddDate = WEB3DateUtility.addSecond(
                            hostOptionOrderExecNotifyRow.getCreatedTimestamp(), 
                            l_lngPreference);
            ...
            //  約定日時：　@出来通知キュー.約定日時
            Timestamp l_tsExecTimestamp = hostOptionOrderExecNotifyRow.getExecTimestamp();

            //  約定数量：　@出来通知キュー.約定数量
            double l_dblExecQuantity = hostOptionOrderExecNotifyRow.getExecQuantity();

            //  約定単価：　@出来通知キュー.約定単価
            double l_dblExecPrice = hostOptionOrderExecNotifyRow.getExecPrice();

            //  出来通知区分：　@出来通知キュー.出来通知区分
            String l_strDealedType = hostOptionOrderExecNotifyRow.getDealedType();

            //  OP出来通知UnitServiceImpl
            WEB3OptionOrderExecNotifyUnitService l_orderExecNotifyUnitService =
                (WEB3OptionOrderExecNotifyUnitService)Services.getService(WEB3OptionOrderExecNotifyUnitService.class);

            // 処理区分
            String l_strStatus = null;
            //1.3 (*2) 約定の場合のみ処理実施
            //  処理対象出来通知キューレコード.出来通知区分 == ”全部約定”または”一部約定”の場合処理実施。
            if (WEB3DealedTypeDef.FULLY_EXECUTED.equals(l_strDealedType)
                || WEB3DealedTypeDef.PARTIALLY_EXECUTED.equals(l_strDealedType))
            {
                log.debug("出来通知区分 == ”全部約定”または”一部約定”の場合");
                //1.3.1 notify約定(OrderUnit, Timestamp, double, double, String)
                //  [notify約定()に指定する引数]
                //  注文単位：　@注文単位
                //  約定日時：　@出来通知キュー.約定日時
                //  約定数量：　@出来通知キュー.約定数量
                ...
                //1.3.2 isPartiallyExecuted()
                //  (IfoBaseOrderUnitImpl::isPartiallyExecuted)
                l_orderUnit.isPartiallyExecuted();

                //1.3.3(*3)注文単位.失効区分==”失効中”の場合のみ実施
                WEB3GentradeAccountManager l_accountManager = (WEB3GentradeAccountManager)l_finApp.getAccountManager();
                IfoOrderUnitRow l_orderUnitRow = (IfoOrderUnitRow)l_orderUnit.getDataSourceObject();
                log.debug("注文単位 失効区分 = " + l_orderUnitRow.getExpirationStatus());
                if (OrderExpirationStatusEnum.EXPIRING.equals(l_orderUnitRow.getExpirationStatus()))
                {
                    log.debug("注文単位.失効区分==”失効中”の場合");
                    try
                    {
                        //1.3.3.1 (*4)失効通知キューテーブルに”処理中”が存在する場合
                        //(*4)以下の条件で先物OP失効通知キューテーブルを検索する。
                        //行が取得できた場合、失効通知処理をコールし、処理対象キューテーブルの処理区分を更新する。
                        //[条件]
                        //先物OP失効通知キューテーブル.データコード = "EI813"（株価指数オプション失効通知）
                        ...
                        l_sbWhereClose.append(" request_code = ? ");            // "EI813"（株価指数オプション失効通知）
                        ...
                        List l_lisSearchResultClose = l_QueryProcessor.doFindAllQuery(
                        ...
                        if (l_intNumClose == 1)
                        {
                            //1.3.3.1.1 notify失効(OrderUnit, double, String, String)
                    ...
                    || OrderStatusEnum.CANCEL_ACCEPTED.equals(l_orderUnitRow.getOrderStatus()))
                {
                    ...
                    try
                    {
                        //1.3.3.2 (*6)訂正取消通知キューテーブルに"処理中"が存在する場合
                        //(*6) 以下の条件で先物OP訂正取消通知キューテーブルを検索する。
                        //行が取得できた場合、取消通知処理をコールし、処理対象キューテーブルの処理区分を更新する。
                        //[条件]
                        //先物OP訂正取消通知キューテーブル.データコード = ”EI812”（株価指数オプション訂正取消通知）
                        ...
                        l_sbWhereCancel.append(" request_code = ? ");             // "EI812"（株価指数オプション失効通知）
                        ...
                        List l_lisSearchResultCancel = l_QueryProcessor.doFindAllQuery(
                        ...
                        if (l_intNumCancel == 1)
                        {
                            ...
                            //1.3.2.2  notify取消(OrderUnit, 先物OP訂正取消通知更新インタセプタ)(OP訂正取消通知UnitServiceImpl::notify取消)
                            //[notify取消()に指定する引数]
                            //注文単位    ： 注文単位
                            //インタセプタ：（生成したOP訂正取消通知インタセプタオブジェクト）
                            log.debug("============= notify取消 開始=============");
                            WEB3OptionChangeCancelNotifyUnitService l_unitService =
                                (WEB3OptionChangeCancelNotifyUnitService)Services.getService(
                                    WEB3OptionChangeCancelNotifyUnitService.class);
                            l_strStatus = l_unitService.notifyCancel(l_orderUnit, l_interceptor);
                            log.debug("============= notify取消 終了=============");

                            //1.3.2.3 処理対象訂正取消通知キューレコード.処理区分を
                            //[更新内容]
                            //”エラー”：上記処理で例外が発生した場合。
                            //notify取消の戻り値：以外の場合。
            ...
            //1.4(*7) 約定取消の場合のみ処理実施
            else if (WEB3DealedTypeDef.CANCEL.equals(hostOptionOrderExecNotifyRow.getDealedType()))
            {
                log.debug("約定取消の場合のみ処理");
                //1.4.1 notify約定取消(OrderUnit, Timestamp, double, double)
                //  [notify約定取消()に指定する引数]
                //  注文単位：　@注文単位
                //  約定日時：　@出来通知キュー.約定日時
                //  約定数量：　@出来通知キュー.約定数量
                //  約定単価：　@出来通知キュー.約定単価
                log.debug("============= notify約定取消 開始=============");
                l_orderExecNotifyUnitService.notifyExecuteCancel(
                    l_orderUnit,
                    l_tsExecTimestamp,
                    l_dblExecQuantity,
                    l_dblExecPrice);
                log.debug("============= notify約定取消 終了=============");
            }

            //1.5(*8) 処理対象出来通知キューレコード.処理区分を以下の通りセットし更新する。
