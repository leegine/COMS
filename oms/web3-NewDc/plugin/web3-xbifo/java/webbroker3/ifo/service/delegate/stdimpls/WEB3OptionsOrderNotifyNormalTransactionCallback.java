...
filename	WEB3OptionsOrderNotifyNormalTransactionCallback.java;
...
File Name        : OP注文通知正常処理一件TransactionCallback(WEB3OptionsOrderNotifyNormalTransactionCallback.java)
 ...
 * @@version 1.0
 */
public class WEB3OptionsOrderNotifyNormalTransactionCallback implements TransactionCallback
{
    /**
     * ログ出力ユーティリティ。<BR>
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(WEB3OptionsOrderNotifyNormalTransactionCallback.class);
     ...
     */
    public WEB3OptionsOrderNotifyNormalTransactionCallback(
        ...
        throws DataQueryException, DataNetworkException, DataCallbackException
    {
        ...
        try
        {
            ...
            //1.2.3 getOP取引口座タイプ( )(顧客::getOP取引口座タイプ)
            SubAccountTypeEnum l_subAccountTypeEnum = l_mainAccount.getOpSubAccountType();

            //1.2.4 getSubAccount(補助口座タイプ : SubAccountTypeEnum)
            //  [getSubAccount()に指定する引数]
            //  補助口座タイプ：　@顧客.getOP取引口座タイプ()の戻り値
            SubAccount l_subAccount = l_mainAccount.getSubAccount(l_subAccountTypeEnum);

            //  OP注文通知UnitServiceImpl
            WEB3OptionsOrderNotifyUnitService l_orderNotifyUnitService =
                (WEB3OptionsOrderNotifyUnitService)Services.getService(WEB3OptionsOrderNotifyUnitService.class);
            ...
            {
                //1.2.5.1 notify新規建注文(先物OP注文通知キューParams, 補助口座)(OP注文通知UnitServiceImpl::notify新規建注文)
                //  [notify新規建注文()に指定する引数]
            ...
            {
                //1.2.6.1 notify返済注文(先物OP注文通知キューParams, 補助口座)(OP注文通知UnitServiceImpl::notify返済注文)
                //  [notify返済注文()に指定する引数]
                //  先物OP注文通知キューParams：　@取得した処理対象先物OP注文通知キューテーブルの1レコード
                //  補助口座：　@顧客.getSubAccount()の戻り値
