...
File Name        :  株価指数オプション出来通知サービス実装(WEB3AsynOptionOrderExecNotifyServiceImpl.java)
Author Name      : Daiwa Institute of Research
Revesion History : 同期版履歴：
Revesion History : 2004/06/15 鄒鋭 (中訊) 新規作成
Revesion History : 非同期対応版履歴：
Revesion History : 2005/03/10 李志強　@非同期実行対応（新規クラス）
    ...
    implements Runnable
{
    ...
    public void run()
    {
        final String STR_METHOD_NAME =
            "WEB3AsynOptionOrderExecNotifyServiceImpl：run()";
        ...
        implements TransactionCallback
    {
            ...
            throws DataNetworkException, DataQueryException, DataCallbackException
        {
            ...
            //1.1 (*1) 先物OP出来通知キューテーブル読込
            //  先物OP出来通知キューテーブルを、行ロック（select for update）オプションにて読み込む。
            //  [検索条件]
            //  先物OP出来通知キューテーブル.データコード == ”株価指数オプション出来通知”（EI811）
            //  先物OP出来通知キューテーブル.処理区分 == ”未処理”
            String l_strWhere = " request_code = ? ";       //識別コード = ?
            ...
            l_strBindValues[0] = WEB3HostRequestCodeDef.OPTION_EXEC_NOTICE;//注文単位.識別コード
