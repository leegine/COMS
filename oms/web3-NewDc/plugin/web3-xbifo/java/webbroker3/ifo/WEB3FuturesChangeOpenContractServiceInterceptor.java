...
1.1
date	2011.03.16.03.43.56;	author zhang-tengyu;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	WEB3FuturesChangeOpenContractServiceInterceptor.java;
...
File Name        : 株価指数先物訂正新規建サービスインタセプタ(WEB3FuturesOpenMarginChangeServiceInterceptor.java)
Author Name      : Daiwa Institute of Research
Revesion History : 2004/07/22 李強 (中訊) 新規作成
Revesion History : 2007/01/29 何文敏 (中訊) 仕様変更　@モデルNO.617
*/
...
/**
 * (株価指数先物訂正新規建サービスインタセプタ)<BR>
 * @@author 李強
 * @@version 1.0
 */
public class WEB3FuturesChangeOpenContractServiceInterceptor implements Interceptor 
{
    /**
     * ログ出力ユーティリティ。
     */
    private static WEB3LogUtility log =
        WEB3LogUtility.getInstance(
            WEB3FuturesChangeOpenContractServiceInterceptor.class);
            
    /**
     * @@roseuid 40F7B0E301E4
     */
    public WEB3FuturesChangeOpenContractServiceInterceptor() 
    {
     ...
     * <BR>
     * １）　@リクエストデータの型が「株価指数先物訂正新規建完了リクエスト」の場合のみ、口座をロックする。<BR> 
    ...
    public Object onCall(Method l_method, Object[] l_serviceParam) 
    {
        ...
        {            
            ...
            {
                WEB3FuturesOpenMarginChangeConfirmRequest  l_request0 =
            ...
            {
                WEB3FuturesOpenMarginChangeCompleteRequest  l_request0 =
            ...
            // １）　@リクエストデータの型が「株価指数先物訂正新規建完了リクエスト」の場合のみ、口座をロックする。 
            //  －拡張アカウントマネージャ.lock口座(証券会社コード, 部店コード, 口座コード)をコールする。 
            //  ※引数はOpLoginSecurityServiceより編集。
            if (l_request instanceof WEB3FuturesOpenMarginChangeCompleteRequest)
            {
