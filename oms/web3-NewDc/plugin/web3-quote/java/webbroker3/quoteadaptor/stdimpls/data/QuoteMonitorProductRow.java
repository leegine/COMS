...
1.1
date	2011.03.14.09.44.21;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	QuoteMonitorProductRow.java;
...
/** 
 * QuoteMonitorProductRowインタフェースは変更不可でリードオンリーである<em>quote_monitor_product</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link QuoteMonitorProductRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface QuoteMonitorProductRow extends Row {


  /** 
   * この{@@link QuoteMonitorProductRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "quote_monitor_product", "session" );
