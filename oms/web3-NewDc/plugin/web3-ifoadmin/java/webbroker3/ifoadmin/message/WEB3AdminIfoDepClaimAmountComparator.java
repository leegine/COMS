...
File Name        : (証拠金)請求額Comparator(WEB3AdminIfoDepClaimAmountComparator.java)
...
/**
 * ((証拠金)請求額Comparator)<BR>
 * (証拠金)請求額Comparatorクラス<BR>
 * <BR>
 * @@author 李玉玲(中訊)
 * @@version 1.0
 */
public class WEB3AdminIfoDepClaimAmountComparator implements Comparator
{
     ...
     * compareの実装<BR>
     * <BR>
     * 昇順、降順の指定にもとづく請求額の比較を行う。 <BR>
     * <BR>
     * １）パラメータを証拠金不足状況情報にキャストする。 <BR>
     * <BR>
     * ２）比較 <BR>
     * 　@１）でキャストした明細1、明細2の請求額を比較し、結果を返却する。 <BR>
     * <BR>
     * 　@昇順指定の場合、 <BR>
     * 　@　@明細1の請求額が、明細2の請求額より小さい場合、負の整数（-1）を返却する。 <BR>
     * 　@　@両方が等しい場合は0を返却する。 <BR>
     * 　@　@明細1の請求額が、明細2の請求額より大きい場合、正の整数（1）を返却する。 <BR>
     * <BR>
     * 　@降順指定の場合、 <BR>
     * 　@　@明細1の請求額が、明細2の請求額より小さい場合、正の整数（1）を返却する。 <BR>
     * 　@　@両方が等しい場合は0を返却する。 <BR>
     * 　@　@明細1の請求額が、明細2の請求額より大きい場合、負の整数（-1）を返却する。 <BR>
     * <BR>
    ...
    public int compare(Object l_unit1, Object l_unit2)
    {
        ...
        //比較
        //でキャストした明細1、明細2の請求額を比較し、結果を返却する。
        int l_intReturn = 0;
