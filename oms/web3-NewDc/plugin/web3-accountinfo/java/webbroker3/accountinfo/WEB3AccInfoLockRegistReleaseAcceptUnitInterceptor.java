...
File Name        : ロック登録解除受付１件サービスインタセプタ(WEB3AccInfoLockRegistReleaseAcceptUnitInterceptor)
...
public class WEB3AccInfoLockRegistReleaseAcceptUnitInterceptor implements Interceptor
{
     ...
     * 　@－サービスの引数[0]をロック客Y客登録解除受付キューParamsオブジェクトにキャストする。<BR> 
     * 　@－ロック客Y客登録解除受付キューParamsオブジェクトの内容より、<BR>
     *    取引時間コンテキストのプロパティをセットする。<BR> 
     * <BR>
     * 　@取引カレンダコンテキスト.証券会社コード = ロック客Y客登録解除受付キューの同名項目 <BR>
     * 　@取引カレンダコンテキスト.部店コード =ロック客Y客登録解除受付キューの同名項目 <BR>
     ...
     * <BR>
     * ３）　@口座をロックする。<BR> 
     * 　@－拡張アカウントマネージャ.lock口座(証券会社コード, 部店コード, 口座コード（※１）)をコールする。 <BR>
     * 　@　@（※１）引数はロック客Y客登録解除受付キューから編集。 <BR>
     * @@param l_method
    ...
    public Object onCall(Method l_method, Object[] l_serviceParam) 
    {
        ...
        try
        {
             ...
             *      －サービスの引数[0]をロック客Y客登録解除受付キューParamsオブジェクトにキャストする。
             *      －ロック客Y客登録解除受付キューParamsオブジェクトの内容より、
            ...
            //取引カレンダコンテキスト.証券会社コード = ロック客Y客登録解除受付キューの同名項目
            l_context.setInstitutionCode(l_strInstitutionCode);
                
            //取引カレンダコンテキスト.部店コード = ロック客Y客登録解除受付キューの同名項目
