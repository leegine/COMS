...
1.1
date	2011.03.10.02.17.05;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	HostLockRegiAcceptRow.java;
...
/** 
 * HostLockRegiAcceptRowインタフェースは変更不可でリードオンリーである<em>host_lock_regi_accept</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link HostLockRegiAcceptRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface HostLockRegiAcceptRow extends Row {


  /** 
   * この{@@link HostLockRegiAcceptRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "host_lock_regi_accept", "session" );
