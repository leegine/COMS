...
/** 
 * host_sell_cancelテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link HostSellCancelRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link HostSellCancelParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link HostSellCancelParams}が{@@link HostSellCancelRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
...
public class HostSellCancelParams extends Params implements HostSellCancelRow {
  ...
  /** 
   * 値が未設定のHostSellCancelParamsオブジェクトを作成します。 
   */
  ...
  /** 
   * 指定のHostSellCancelRowオブジェクトの値を利用してHostSellCancelParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するHostSellCancelRowオブジェクト 
   */
   ...
   * @@param row 比較対照のHostSellCancelRow 
