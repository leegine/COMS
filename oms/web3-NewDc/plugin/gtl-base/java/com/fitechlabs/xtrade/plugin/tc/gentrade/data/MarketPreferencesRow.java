...
1.1
date	2011.03.14.02.37.37;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MarketPreferencesRow.java;
...
/** 
 * MarketPreferencesRowインタフェースは変更不可でリードオンリーである<em>market_preferences</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link MarketPreferencesRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface MarketPreferencesRow extends Row {


  /** 
   * この{@@link MarketPreferencesRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "market_preferences", "master" );
