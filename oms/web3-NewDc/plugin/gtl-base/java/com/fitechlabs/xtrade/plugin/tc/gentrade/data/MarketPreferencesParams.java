...
1.1
date	2011.03.14.02.39.16;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MarketPreferencesParams.java;
...
/** 
 * market_preferencesテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MarketPreferencesRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MarketPreferencesParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MarketPreferencesParams}が{@@link MarketPreferencesRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class MarketPreferencesParams extends Params implements MarketPreferencesRow {
  ...
  public static final RowType TYPE = MarketPreferencesRow.TYPE;
  ...
  /** 
   * 値が未設定のMarketPreferencesParamsオブジェクトを作成します。 
   */
  public MarketPreferencesParams() {
  ...
  /** 
   * 指定のMarketPreferencesRowオブジェクトの値を利用してMarketPreferencesParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMarketPreferencesRowオブジェクト 
   */
  public MarketPreferencesParams( MarketPreferencesRow p_row )
  {
  ...
  {
    if ( !( other instanceof MarketPreferencesRow ) )
       return false;
    return fieldsEqual( (MarketPreferencesRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMarketPreferencesRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MarketPreferencesRow row )
  {
  ...
  {
    return new MarketPreferencesPK(market_id, name, name_serial_no);
  }
