...
1.1
date	2011.03.14.02.35.04;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	TradedProductParams.java;
...
/** 
 * traded_productテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link TradedProductRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link TradedProductParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link TradedProductParams}が{@@link TradedProductRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class TradedProductParams extends Params implements TradedProductRow {
  ...
  public static final RowType TYPE = TradedProductRow.TYPE;
  ...
  /** 
   * 値が未設定のTradedProductParamsオブジェクトを作成します。 
   */
  public TradedProductParams() {
  ...
  /** 
   * 指定のTradedProductRowオブジェクトの値を利用してTradedProductParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するTradedProductRowオブジェクト 
   */
  public TradedProductParams( TradedProductRow p_row )
  {
  ...
  {
    if ( !( other instanceof TradedProductRow ) )
       return false;
    return fieldsEqual( (TradedProductRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のTradedProductRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( TradedProductRow row )
  {
  ...
  {
    return new TradedProductPK(traded_product_id);
  }
