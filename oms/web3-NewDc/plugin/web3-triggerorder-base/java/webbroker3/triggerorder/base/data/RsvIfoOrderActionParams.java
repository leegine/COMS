...
/** 
 * rsv_ifo_order_actionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RsvIfoOrderActionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RsvIfoOrderActionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RsvIfoOrderActionParams}が{@@link RsvIfoOrderActionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
...
public class RsvIfoOrderActionParams extends Params implements RsvIfoOrderActionRow {
  ...
  /** 
   * 値が未設定のRsvIfoOrderActionParamsオブジェクトを作成します。 
   */
  ...
  /** 
   * 指定のRsvIfoOrderActionRowオブジェクトの値を利用してRsvIfoOrderActionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRsvIfoOrderActionRowオブジェクト 
   */
   ...
   * @@param row 比較対照のRsvIfoOrderActionRow 
