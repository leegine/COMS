...
1.1
date	2011.03.15.02.20.15;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RsvEqOrderActionParams.java;
...
/** 
 * rsv_eq_order_actionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RsvEqOrderActionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RsvEqOrderActionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RsvEqOrderActionParams}が{@@link RsvEqOrderActionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see RsvEqOrderActionRow 
 */
public class RsvEqOrderActionParams extends Params implements RsvEqOrderActionRow {
  ...
  public static final String PTYPE = "rsv_eq_order_action";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = RsvEqOrderActionRow.TYPE;
  ...
  public RowType getRowType() {
    return RsvEqOrderActionRow.TYPE;
  }
  ...
  /** 
   * 値が未設定のRsvEqOrderActionParamsオブジェクトを作成します。 
   */
  public RsvEqOrderActionParams() {
  }
  ...
  /** 
   * 指定のRsvEqOrderActionRowオブジェクトの値を利用してRsvEqOrderActionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRsvEqOrderActionRowオブジェクト 
   */
  public RsvEqOrderActionParams( RsvEqOrderActionRow p_row )
  {
  ...
  {
    if ( !( other instanceof RsvEqOrderActionRow ) )
       return false;
    return fieldsEqual( (RsvEqOrderActionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRsvEqOrderActionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RsvEqOrderActionRow row )
  {
  ...
  {
    throw new UnsupportedOperationException("Table rsv_eq_order_action has no primary key.");
  }
