...
/** 
 * RsvEqOrderActionRowインタフェースは変更不可でリードオンリーである<em>rsv_eq_order_action</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RsvEqOrderActionRow}インタフェースの実装オブジェクトを検索します。 
 * 
...
public interface RsvEqOrderActionRow extends Row {
  ...
  /** 
   * この{@@link RsvEqOrderActionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
