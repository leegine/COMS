...
1.1
date	2011.03.15.02.20.00;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RsvEqClosingContractSpecRow.java;
...
/** 
 * RsvEqClosingContractSpecRowインタフェースは変更不可でリードオンリーである<em>rsv_eq_closing_contract_spec</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link RsvEqClosingContractSpecRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface RsvEqClosingContractSpecRow extends Row {


  /** 
   * この{@@link RsvEqClosingContractSpecRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "rsv_eq_closing_contract_spec", "account" );
