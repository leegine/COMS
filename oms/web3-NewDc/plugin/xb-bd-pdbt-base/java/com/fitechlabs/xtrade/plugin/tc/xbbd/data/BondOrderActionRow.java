...
1.1
date	2011.03.22.08.57.40;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BondOrderActionRow.java;
...
/** 
 * BondOrderActionRowインタフェースは変更不可でリードオンリーである<em>bond_order_action</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link BondOrderActionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface BondOrderActionRow extends Row {


  /** 
   * この{@@link BondOrderActionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "bond_order_action", "account" );
