...
1.1
date	2011.03.22.08.58.01;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BondTradedProductParams.java;
...
/** 
 * bond_traded_productテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link BondTradedProductRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link BondTradedProductParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link BondTradedProductParams}が{@@link BondTradedProductRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class BondTradedProductParams extends Params implements BondTradedProductRow {
  ...
  public static final RowType TYPE = BondTradedProductRow.TYPE;
  ...
  /** 
   * 値が未設定のBondTradedProductParamsオブジェクトを作成します。 
   */
  public BondTradedProductParams() {
  ...
  /** 
   * 指定のBondTradedProductRowオブジェクトの値を利用してBondTradedProductParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するBondTradedProductRowオブジェクト 
   */
  public BondTradedProductParams( BondTradedProductRow p_row )
  {
  ...
  {
    if ( !( other instanceof BondTradedProductRow ) )
       return false;
    return fieldsEqual( (BondTradedProductRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のBondTradedProductRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( BondTradedProductRow row )
  {
  ...
  {
    return new BondTradedProductPK(traded_product_id);
  }
