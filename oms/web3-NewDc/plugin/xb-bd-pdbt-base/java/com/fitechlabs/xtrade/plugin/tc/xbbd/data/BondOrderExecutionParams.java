...
1.1
date	2011.03.22.08.59.46;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BondOrderExecutionParams.java;
...
/** 
 * bond_order_executionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link BondOrderExecutionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link BondOrderExecutionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link BondOrderExecutionParams}が{@@link BondOrderExecutionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class BondOrderExecutionParams extends Params implements BondOrderExecutionRow {
  ...
  public static final RowType TYPE = BondOrderExecutionRow.TYPE;
  ...
  /** 
   * 値が未設定のBondOrderExecutionParamsオブジェクトを作成します。 
   */
  public BondOrderExecutionParams() {
  ...
  /** 
   * 指定のBondOrderExecutionRowオブジェクトの値を利用してBondOrderExecutionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するBondOrderExecutionRowオブジェクト 
   */
  public BondOrderExecutionParams( BondOrderExecutionRow p_row )
  {
  ...
  {
    if ( !( other instanceof BondOrderExecutionRow ) )
       return false;
    return fieldsEqual( (BondOrderExecutionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のBondOrderExecutionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( BondOrderExecutionRow row )
  {
  ...
  {
    return new BondOrderExecutionPK(order_execution_id);
  }
