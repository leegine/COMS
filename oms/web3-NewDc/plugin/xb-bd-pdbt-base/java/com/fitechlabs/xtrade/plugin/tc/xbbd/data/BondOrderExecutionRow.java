...
1.1
date	2011.03.22.08.57.44;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BondOrderExecutionRow.java;
...
/** 
 * BondOrderExecutionRowインタフェースは変更不可でリードオンリーである<em>bond_order_execution</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link BondOrderExecutionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface BondOrderExecutionRow extends Row {


  /** 
   * この{@@link BondOrderExecutionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "bond_order_execution", "account" );
