...
1.1
date	2011.03.22.08.57.23;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	BondFinTransactionRow.java;
...
/** 
 * BondFinTransactionRowインタフェースは変更不可でリードオンリーである<em>bond_fin_transaction</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link BondFinTransactionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface BondFinTransactionRow extends Row {


  /** 
   * この{@@link BondFinTransactionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "bond_fin_transaction", "account" );
