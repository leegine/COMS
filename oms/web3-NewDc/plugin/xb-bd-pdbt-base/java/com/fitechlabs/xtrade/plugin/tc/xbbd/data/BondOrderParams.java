...
1.1
date	2011.03.22.08.58.43;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
/** 
 * bond_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link BondOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link BondOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link BondOrderParams}が{@@link BondOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 * @@see BondOrderRow 
 */
public class BondOrderParams extends Params implements BondOrderRow {
  ...
  public static final String PTYPE = "bond_order";


  /** クエリ内で利用される場合にrowをあらわすRowType */
  public static final RowType TYPE = BondOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のBondOrderParamsオブジェクトを作成します。 
   */
  public BondOrderParams() {
  ...
  /** 
   * 指定のBondOrderRowオブジェクトの値を利用してBondOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するBondOrderRowオブジェクト 
   */
  public BondOrderParams( BondOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof BondOrderRow ) )
       return false;
    return fieldsEqual( (BondOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のBondOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( BondOrderRow row )
  {
  ...
  {
    return new BondOrderPK(order_id);
  }
