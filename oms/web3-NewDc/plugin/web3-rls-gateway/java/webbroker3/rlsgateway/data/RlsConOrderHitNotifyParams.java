...
1.1
date	2011.03.22.08.24.26;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	RlsConOrderHitNotifyParams.java;
...
/** 
 * rls_con_order_hit_notifyテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link RlsConOrderHitNotifyRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link RlsConOrderHitNotifyParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link RlsConOrderHitNotifyParams}が{@@link RlsConOrderHitNotifyRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class RlsConOrderHitNotifyParams extends Params implements RlsConOrderHitNotifyRow {
  ...
  public static final RowType TYPE = RlsConOrderHitNotifyRow.TYPE;
  ...
  /** 
   * 値が未設定のRlsConOrderHitNotifyParamsオブジェクトを作成します。 
   */
  public RlsConOrderHitNotifyParams() {
  ...
  /** 
   * 指定のRlsConOrderHitNotifyRowオブジェクトの値を利用してRlsConOrderHitNotifyParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するRlsConOrderHitNotifyRowオブジェクト 
   */
  public RlsConOrderHitNotifyParams( RlsConOrderHitNotifyRow p_row )
  {
  ...
  {
    if ( !( other instanceof RlsConOrderHitNotifyRow ) )
       return false;
    return fieldsEqual( (RlsConOrderHitNotifyRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のRlsConOrderHitNotifyRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( RlsConOrderHitNotifyRow row )
  {
  ...
  {
    return new RlsConOrderHitNotifyPK(account_id, sub_account_id, order_type, order_id, product_type);
  }
    ...
    public Object getColumn( String name ) {
        ...
        switch ( name.charAt(0) ) {
            ...
            case 'c':
                ...
                break;
            case 'h':
                if ( name.equals("hit_tick_timestamp") ) {
