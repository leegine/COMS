...
1.1
date	2011.03.22.09.13.22;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderRow.java;
...
/** 
 * MutualFundOrderRowインタフェースは変更不可でリードオンリーである<em>mutual_fund_order</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link MutualFundOrderRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface MutualFundOrderRow extends Row {


  /** 
   * この{@@link MutualFundOrderRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "mutual_fund_order", "account" );
