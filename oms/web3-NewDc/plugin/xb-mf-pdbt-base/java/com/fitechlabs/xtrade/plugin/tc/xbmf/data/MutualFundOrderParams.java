...
1.1
date	2011.03.22.09.13.30;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderParams.java;
...
/** 
 * mutual_fund_orderテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MutualFundOrderRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MutualFundOrderParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MutualFundOrderParams}が{@@link MutualFundOrderRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class MutualFundOrderParams extends Params implements MutualFundOrderRow {
  ...
  public static final RowType TYPE = MutualFundOrderRow.TYPE;
  ...
  /** 
   * 値が未設定のMutualFundOrderParamsオブジェクトを作成します。 
   */
  public MutualFundOrderParams() {
  ...
  /** 
   * 指定のMutualFundOrderRowオブジェクトの値を利用してMutualFundOrderParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMutualFundOrderRowオブジェクト 
   */
  public MutualFundOrderParams( MutualFundOrderRow p_row )
  {
  ...
  {
    if ( !( other instanceof MutualFundOrderRow ) )
       return false;
    return fieldsEqual( (MutualFundOrderRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMutualFundOrderRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MutualFundOrderRow row )
  {
  ...
  {
    return new MutualFundOrderPK(order_id);
  }
