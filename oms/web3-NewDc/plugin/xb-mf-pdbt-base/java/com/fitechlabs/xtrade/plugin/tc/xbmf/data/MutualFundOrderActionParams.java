...
1.1
date	2011.03.22.09.11.37;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderActionParams.java;
...
/** 
 * mutual_fund_order_actionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MutualFundOrderActionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MutualFundOrderActionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MutualFundOrderActionParams}が{@@link MutualFundOrderActionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 * @@author xTradeコードジェネレータ 
 * 
 * @@see com.fitechlabs.dbind.Row 
 * @@see com.fitechlabs.dbind.PrimaryKey 
 * @@see MutualFundOrderActionPK 
 * @@see MutualFundOrderActionRow 
 */
public class MutualFundOrderActionParams extends Params implements MutualFundOrderActionRow {
  ...
  public static final RowType TYPE = MutualFundOrderActionRow.TYPE;
  ...
  /** 
   * 値が未設定のMutualFundOrderActionParamsオブジェクトを作成します。 
   */
  public MutualFundOrderActionParams() {
  ...
  /** 
   * 指定のMutualFundOrderActionRowオブジェクトの値を利用してMutualFundOrderActionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMutualFundOrderActionRowオブジェクト 
   */
  public MutualFundOrderActionParams( MutualFundOrderActionRow p_row )
  {
  ...
  {
    if ( !( other instanceof MutualFundOrderActionRow ) )
       return false;
    return fieldsEqual( (MutualFundOrderActionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMutualFundOrderActionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MutualFundOrderActionRow row )
  {
  ...
  {
    return new MutualFundOrderActionPK(order_action_id);
  }
