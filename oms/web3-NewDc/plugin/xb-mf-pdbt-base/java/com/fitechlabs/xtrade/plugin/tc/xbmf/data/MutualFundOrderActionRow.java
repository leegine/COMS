...
1.1
date	2011.03.22.09.13.43;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderActionRow.java;
...
/** 
 * MutualFundOrderActionRowインタフェースは変更不可でリードオンリーである<em>mutual_fund_order_action</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link MutualFundOrderActionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface MutualFundOrderActionRow extends Row {


  /** 
   * この{@@link MutualFundOrderActionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "mutual_fund_order_action", "account" );
