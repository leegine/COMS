...
1.1
date	2011.03.22.09.12.44;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderExecutionParams.java;
...
/** 
 * mutual_fund_order_executionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MutualFundOrderExecutionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MutualFundOrderExecutionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MutualFundOrderExecutionParams}が{@@link MutualFundOrderExecutionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class MutualFundOrderExecutionParams extends Params implements MutualFundOrderExecutionRow {
  ...
  public static final RowType TYPE = MutualFundOrderExecutionRow.TYPE;
  ...
  /** 
   * 値が未設定のMutualFundOrderExecutionParamsオブジェクトを作成します。 
   */
  public MutualFundOrderExecutionParams() {
  ...
  /** 
   * 指定のMutualFundOrderExecutionRowオブジェクトの値を利用してMutualFundOrderExecutionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMutualFundOrderExecutionRowオブジェクト 
   */
  public MutualFundOrderExecutionParams( MutualFundOrderExecutionRow p_row )
  {
  ...
  {
    if ( !( other instanceof MutualFundOrderExecutionRow ) )
       return false;
    return fieldsEqual( (MutualFundOrderExecutionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMutualFundOrderExecutionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MutualFundOrderExecutionRow row )
  {
  ...
  {
    return new MutualFundOrderExecutionPK(order_execution_id);
  }
