...
1.1
date	2011.03.22.09.11.54;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundFinTransactionParams.java;
...
/** 
 * mutual_fund_fin_transactionテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MutualFundFinTransactionRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MutualFundFinTransactionParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MutualFundFinTransactionParams}が{@@link MutualFundFinTransactionRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class MutualFundFinTransactionParams extends Params implements MutualFundFinTransactionRow {
  ...
  public static final RowType TYPE = MutualFundFinTransactionRow.TYPE;
  ...
  /** 
   * 値が未設定のMutualFundFinTransactionParamsオブジェクトを作成します。 
   */
  public MutualFundFinTransactionParams() {
  ...
  /** 
   * 指定のMutualFundFinTransactionRowオブジェクトの値を利用してMutualFundFinTransactionParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMutualFundFinTransactionRowオブジェクト 
   */
  public MutualFundFinTransactionParams( MutualFundFinTransactionRow p_row )
  {
  ...
  {
    if ( !( other instanceof MutualFundFinTransactionRow ) )
       return false;
    return fieldsEqual( (MutualFundFinTransactionRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMutualFundFinTransactionRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MutualFundFinTransactionRow row )
  {
  ...
  {
    return new MutualFundFinTransactionPK(fin_transaction_id);
  }
