...
1.1
date	2011.03.22.09.11.49;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MfTradedProductUpdqParams.java;
...
/** 
 * mf_traded_product_updqテーブルのデータベースレコードの値変更に利用するパラメータを作成するユーティリティクラスです。 
 * <p> 
 * 通常、コンストラクタのうちの1つを利用して既存の{@@link MfTradedProductUpdqRow}オブジェクトからパラメータを作成します。 
 * 続いて、setterメソッドを利用して希望するフィールドの値を変更します。 
 * そして、{@@link MfTradedProductUpdqParams}オブジェクトを引数として渡し、新しいレコードを作成または存在するレコードを更新します。 
 * <p> 
 * {@@link MfTradedProductUpdqParams}が{@@link MfTradedProductUpdqRow}インタフェースを実装するので、このクラスでは各フィールドに対しgetterおよびsetterメソッドを提供しています。 
 * <p> 
 ...
 */
public class MfTradedProductUpdqParams extends Params implements MfTradedProductUpdqRow {
  ...
  public static final RowType TYPE = MfTradedProductUpdqRow.TYPE;
  ...
  /** 
   * 値が未設定のMfTradedProductUpdqParamsオブジェクトを作成します。 
   */
  public MfTradedProductUpdqParams() {
  ...
  /** 
   * 指定のMfTradedProductUpdqRowオブジェクトの値を利用してMfTradedProductUpdqParamsオブジェクトを作成します。 
   * @@param p_row テンプレートとして利用するMfTradedProductUpdqRowオブジェクト 
   */
  public MfTradedProductUpdqParams( MfTradedProductUpdqRow p_row )
  {
  ...
  {
    if ( !( other instanceof MfTradedProductUpdqRow ) )
       return false;
    return fieldsEqual( (MfTradedProductUpdqRow) other );
  }


  /** 
   * 指定のRowオブジェクトとフィールド値がすべて一致しているかどうか調べます。 
   * @@param row 比較対照のMfTradedProductUpdqRow 
   * @@return すべてのフィールド値が一致の場合true、そうでなければfalse 
   */
  public boolean fieldsEqual( MfTradedProductUpdqRow row )
  {
  ...
  {
    return new MfTradedProductUpdqPK(traded_product_id, valid_for_biz_date);
  }
