...
1.1
date	2011.03.22.09.12.27;	author che-jin;	state Exp;
branches;
next	;
deltatype	text;
kopt	kv;
...
filename	MutualFundOrderExecutionRow.java;
...
/** 
 * MutualFundOrderExecutionRowインタフェースは変更不可でリードオンリーである<em>mutual_fund_order_execution</em>データベーステーブルレコードのスナップショットを表現します。 
 * <p> 
 * 通常、{@@link com.fitechlabs.xtrade.kernel.data.QueryProcessor}オブジェクトのメソッドを利用して{@@link MutualFundOrderExecutionRow}インタフェースの実装オブジェクトを検索します。 
 * 
 ...
 */
public interface MutualFundOrderExecutionRow extends Row {


  /** 
   * この{@@link MutualFundOrderExecutionRow}クラスに紐付くテーブルをあらわす{@@link com.fitechlabs.dbind.RowType}オブジェクト 
   */
   public static final RowType TYPE = new RowType( "mutual_fund_order_execution", "account" );
